...
export function DeclaredConnectionsCard({
  ...
  const visibleKinds = GENERIC_ADDABLE_KINDS;
  const listed = controller.listed.filter((entry) => !isDeclaredUnityCatalogConnection(entry.connection));
  const chosenKind = visibleKinds.find((entry) => entry.id === kindChoice);
    ...
    <>
      ...
      {allowMutations && adding ? (
        <div className="plane-form" id={`${formId}-form`} data-testid="add-connection-form">
          <div className="plane-kind-field">
            ...
            <AppSelect
              ...
              options={[
                ...
                ...visibleKinds.map((entry) => ({ value: entry.id, label: entry.label })),
              ]}
          ...
          {chosenKind && picker ? (
            <div className="plane-picker">
              <AssetPicker
                ...
                current={value}
                rowState={(candidate) => {
                  const connected = controller.listed.some(
                    (entry) =>
                      entry.connection.state === 'declared' &&
                      entry.connection.resourceType === chosenKind.id &&
                      entry.connection.value.trim().toLocaleLowerCase() === candidate.trim().toLocaleLowerCase()
                  );
                  return connected
                    ? { label: 'Connected', selectable: false }
                    : { label: 'Available', selectable: true };
                }}
          ...
          {chosenKind && !picker ? (
            <div className="plane-manual-fields">
              <label className="plane-field-label" htmlFor={`${formId}-connection-value`}>
                {chosenKind.id === 'mcp-server' ? 'MCP server URL or identifier' : `${chosenKind.label} identifier`}
              </label>
              <input
                id={`${formId}-connection-value`}
                ...
                onChange={(event) => {
                  ...
                  setValue(next);
                  setLabel((current) => current || next);
                  setError('');
                }}
              />
              <label className="plane-field-label" htmlFor={`${formId}-connection-label`}>
                Display name
              </label>
              <input
                id={`${formId}-connection-label`}
                className="plane-field"
                type="text"
                value={label}
                disabled={busy}
