...
const PROSE = 'GTA Online leads the last 30 days.\n\nThe gap widened after the summer update.';
...
describe('the answer for a prose-only reply', () => {
  ...
  it('takes the takeaway from the agent, not from a template', () => {
    expect(proseOnlyAnswer('msg-1', PROSE).takeaway).toBe('GTA Online leads the last 30 days.');
  });

  it('skips leading blank lines rather than showing an empty takeaway', () => {
    expect(proseOnlyAnswer('msg-1', `\n\n   \n${PROSE}`).takeaway).toBe('GTA Online leads the last 30 days.');
  });
 ...
 * analysis: a card headed "This question was not answered." with the model's own
...
describe('the orchestrator’s internal package', () => {
  ...
  it('does not headline unanswered once the findings already hold a table', () => {
    ...
    expect(answer.narrative).toContain('| GTA Online | 9575 |');
  });
