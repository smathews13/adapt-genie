...
import { showsAdminSurfaces, useOptionalRole } from './role';

/**
 * The six pages that are fetched when somebody opens them, not when the app
 * opens.
 *
 * ADAPT ASK IS DELIBERATELY NOT ONE OF THEM. It is the page nearly every visit
...
const GeniePage = lazy(() => import('./GeniePage').then((loaded) => ({ default: loaded.GeniePage })));
...
/**
 * A refused admin route is a complete startup result, just like a loaded page.
 *
 * Lazy pages mark themselves ready after their chunk resolves. A consumer never
 * mounts that child, so the route gate owns readiness for the refusal panel.
 */
function AdminRoute({ children }: { children: ReactNode }) {
  const role = useOptionalRole();
  const { markReady } = useStartupReadiness();
  useLayoutEffect(() => {
    if (role && role.state !== 'resolving' && !showsAdminSurfaces(role.state)) markReady();
  }, [markReady, role]);
...
applyColorScheme(appliedColorScheme() ?? DEFAULT_COLOR_SCHEME);
