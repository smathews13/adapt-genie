...
describe('the admin cancellation control', () => {
  it('labels the compact admin-only control and says only what it preserves', () => {
    ...
    expect(markup).toContain('class="sr-only">Stop all active runs</span>');
...
function health(overrides: Partial<OpsHealthPayload> = {}): OpsHealthPayload {
  return {
    ...
    app: {
      ...
      variable: 'ADAPT_TELEMETRY_SCHEMA',
      table: '',
      grant: null,
      insightsHref: 'https://example.test/apps/adapt/insights',
      requestsPerHour: [],
      lastServedAt: '',
      recordingSince: '',
      signInsPerDay: [],
...
describe('the health block', () => {
  it('uses the branded panel loader for the first health check', () => {
    const markup = markupOf(<HealthBody block={block<OpsHealthPayload>(null, { busy: true })} />);
    expect(markup).toContain('adapt-loader--panel');
    expect(markup).toContain('Checking application health');
  ...
  it('keeps one answer-path serving endpoint and no judge resource', () => {
    const markup = markupOf(<HealthBody block={block(health())} />);
    expect(markup).toContain('aria-label="Orchestrator serving endpoint · a-model connection status: Connected"');
    expect(markup.match(/Orchestrator serving endpoint · a-model/g)).toHaveLength(2);
    expect(markup).not.toContain('Benchmark judge');
  ...
  it("carries the store's own words when it is not answering", () => {
            ...
            platform: [
              {
                ...
                read: true,
                rows: [],
                reason: 'permission denied for schema adapt_metrics',
              },
            ],
          })
        )}
    ...
    expect(markup).toContain('permission denied for schema adapt_metrics');
  });
  ...
  it('links out to the platform record instead of paraphrasing it', () => {
    ...
    expect(markup).toContain('https://example.test/apps/adapt/insights');
  });

  it('shows telemetry being off without deployment-variable narrative', () => {
    const markup = render(<HealthBody block={block(health())} />);
    expect(markup).toContain('App telemetry is off');
    expect(markup).not.toContain('ADAPT_TELEMETRY_SCHEMA');
   ...
   * there is one.
   *
   * Only configured ADAPT dependencies carry Connections entity ids.
   */
  it('links a dependency to its Connections row, and only where there is one', () => {
    ...
    // Three configured probe rows carry an id. App and Lakebase are readings.
  ...
  it('says a check that did not run in its Notes cell', () => {
    ...
    const row = { ...health().dependencies[0], kind: 'catalog', label: 'Catalog', result: 'not-checked' as const };
    expect(render(<HealthBody block={block(health({ dependencies: [row], platform: [] }))} />)).toContain(
      'Unity Catalog container · not checked'
    );
  });
  ...
  it('draws a neutral resource mark for an unfamiliar probe kind', () => {
    const markup = markupOf(
      <HealthBody
        block={block(
          health({
...
describe('the cost block', () => {
  it('shows an explicit loading icon while the first cost read is pending', () => {
  ...
  it('carries no caption under any figure', () => {
    const payload = cost({
      tiles: (['serving-endpoint', 'sql-warehouse', 'genie', 'legacy-addon', 'app-compute'] as const).map((id) => ({
        ...cost().tiles[0],
        id,
      })),
    });
  ...
  it('leaves range totals unlabelled and keeps daily rates explicit', () => {
    const payload = cost({
      tiles: [{ ...cost().tiles[0], id: 'app-compute', label: 'App compute', basis: 'per-day' }],
    });
  ...
  it('keeps attribution methodology out of the simplified tile area', () => {
    ...
    expect(render(<CostBody block={block(cost())} />)).not.toContain("custom_tags['adapt']");
  });
  ...
  it('averages attributed serving and SQL spend and removes the per-run table', () => {
    ...
    expect(markup).toContain('Average cost / question Estimated 2.00 USD');
    expect(markup).not.toContain('Marginal serving + foundation tokens + Ask SQL ÷ completed interactive Asks');
  ...
  it('keeps the resource tile grid when billing has no matching rows', () => {
    ...
    expect(markup).not.toContain('Legacy add-on');
    ...
    expect((markup.match(/ops-genie-card/g) ?? []).length).toBe(1);
  });

  it('draws one box for the configured Genie space when billing is empty', () => {
    const empty = {
        ...
        block={block(
          cost({
            ...
            tiles: [
              ...
              { ...empty, id: 'app-compute', label: 'App compute', resourceId: 'adapt', resourceKind: 'app' },
            ],
            reason: 'No billing rows matched the ADAPT tag in this range.',
          })
        )}
      />
    );
    expect(markup).toContain('Data Genie');
    expect(markup).not.toContain('Dictionary');
    expect(markup).not.toContain('Legacy add-on');
  ...
  it('renders Serving and the one configured Data Genie without excluded resources', () => {
    const payload = cost({
      tiles: [
        {
          ...cost().tiles[0],
    ...
    expect(visible).not.toMatch(/ADAPT (?:requests|queries)/);
  ...
  it('keeps internal row-count evidence out of the simplified SQL card', () => {
    const payload = cost({
      tiles: [
        {
          ...cost().tiles[0],
    ...
    expect(markup).not.toMatch(/ADAPT quer|warehouse quer/);
  ...
  it('lets an operator set monthly app and resource budgets without replacing saved values', () => {
    ...
    expect(markup).not.toContain('aria-label="Legacy add-on monthly budget in USD"');
    expect(editor).toContain('data-columns="4"');
    expect(editor.match(/ops-number-ticker ops-forecast-number-control/g)).toHaveLength(4);
    expect(markup).toContain('Resource budgets (monthly)');
    for (const label of ['Agent serving budget', 'Ask SQL budget', 'Data Genie budget', 'App compute budget']) {
    ...
    expect(markup).not.toContain('Legacy add-on monthly budget');
    ...
    expect(markup.match(/class="[^"]*ops-budget-apply[^"]*"/g)).toHaveLength(2);
    expect(markup).toContain('>Apply<');
    expect(markup).toContain('>Apply resource budgets<');
  ...
  it('uses the shared Cost-period date badge on component cards', () => {
  ...
  it('does not render an excluded historical cost tile', () => {
    const vector = {
      ...cost().tiles[1],
      id: 'legacy-addon',
      label: 'Legacy add-on',
  ...
  it('renders exactly Free and Charged for the one configured Genie card', () => {
    ...
    expect(grid).not.toContain('Dictionary Genie');
    ...
    expect(dbu).not.toContain('Dictionary Genie');
  ...
  it('renders the production Cost hierarchy without exposing unmatched workspace Genie spend', () => {
    ...
    const primary = [
      ...
      },
      {
        ...cost().tiles[0],
        id: 'legacy-addon',
        label: 'Legacy add-on · example.resource',
        resourceId: 'catalog.schema.index',
      ...
      },
      {
        ...cost().tiles[0],
        id: 'app-compute',
        label: 'App compute · adapt-production-app',
        resourceId: 'adapt-production-app',
        quality: 'rate' as const,
        amount: 11.12,
        dbus: 55.6,
        basis: 'per-day' as const,
    ...
    expect((primaryGrid.match(/ops-tile ops-primary-cost-card ops-genie-card/g) ?? []).length).toBe(1);
    ...
    expect(primaryGrid).not.toContain('Dictionary Genie');
    expect(primaryGrid).not.toContain('Legacy add-on');
    expect(primaryGrid).not.toMatch(/Free usage|Allowance|Promotional|effective DBU|Configured-space/);
    expect(primaryGrid.indexOf('Data Genie')).toBeLessThan(primaryGrid.indexOf('Average cost / question'));
  ...
  it('keeps budget fields when billing has no rows and when the grant is missing', () => {
    ...
    expect(empty).not.toContain('Dictionary Genie budget');
...
describe('the traffic block', () => {
  it('uses the branded panel loader for the first traffic read', () => {
    const markup = markupOf(<TrafficBody block={block<OpsTrafficPayload>(null, { busy: true })} />);
    expect(markup).toContain('adapt-loader--panel');
    expect(markup).toContain('Loading traffic');
    expect(markup).not.toContain('ops-skeleton');
  });
  ...
  describe('the tool-calls chart', () => {
    it('names each tool in full and prints its count', () => {
          ...
          block={block(
            traffic({
              toolCalls: [
                ...
                { key: 'search', label: 'Called query_metrics', count: 15 },
              ],
            })
          )}
        />
      ...
      expect(markup).toContain('Called query_metrics');
...
describe('the latency block', () => {
  ...
  it('keeps baseline filters between search and Refresh on the right control rail', () => {
    ...
    expect(css).toMatch(
      /\.ops-latency-trend-filters\s*\{[^}]*display:\s*inline-flex;[^}]*align-items:\s*center;[^}]*gap:\s*6px;/
    );
    expect(css).toMatch(
  ...
  it('shows no redundant route or date-window caption in its subheader', () => {
    const markup = markupOf(<LatencyBody block={block(latency())} />);
