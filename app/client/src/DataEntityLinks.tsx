...
export function EntityLink({ entity, children }: { entity: string; children: ReactNode }) {
  return (
    <Link
      ...
      className="entity-table entity-table-link font-medium underline decoration-dotted decoration-1 underline-offset-2 hover:decoration-solid"
    >
      {children}
    </Link>
  );
 ...
 */
export function VisitInDatabricks({
  name,
  object,
  children,
  className,
  ...
  if (!href) return null;
  return (
    <VisitLink href={href} name={name} className={className}>
      {children}
    </VisitLink>
  );
}
...
export function VisitLink({
  ...
  return (
    <a
      className={['visit-in-databricks', className].filter(Boolean).join(' ')}
      href={href}
      rel="noopener noreferrer"
      target="_blank"
      title={label}
  ...
  return (
    <>
      {nodes.map((node, index) => {
        switch (node.kind) {
          case 'text': {
            // `**Labels:** Rockstar, 2K` is a lead-in and its value, and the
