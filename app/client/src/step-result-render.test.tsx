...
import { LEGACY_ANALYSIS_STAGE_ID } from '../../shared/stage-lexicon';
...
const DATA_GENIE = [
  ...
  '',
  'Query result: distinct_player_count',
  '12000',
  '',
...
const SEMANTIC = [
  ...
  'Table example_catalog.sales_demo.data_dictionary. Business definitions and usage guardrails.',
  'Columns:',
  '- table_name (string)',
  '',
  'What appears above was filtered by a cached snapshot of grants from when this governed dataset was built.',
].join('\n');
...
const DECLARED_TABLES = [
  ...
  'example_catalog.sales_demo.gold_customer_180d_summary',
...
const DECLARED_TABLE_LIST = [
  ...
  `${DECLARED_TABLES[1]} [franchise: Rockstar]`,
  `${DECLARED_TABLES[2]} [franchise: 2K]`,
...
describe('a data_genie step', () => {
  ...
  it('names the space that answered on the Result row, instead of burying it in the prose', () => {
    ...
    // reading a sentence.
    expect(result(markup)).toContain('Revenue Metrics');
  ...
  it('draws a one-row result as a grid with its column name over it', () => {
    expect(result(markup)).toContain('<th scope="col">distinct_player_count</th>');
    ...
    const many = panel({
      ...
      output: DICTIONARY_GENIE.replace(
        ...
        [
          ...
          'silver_player_profiles | cdp_customer_id | Cross-label key. | Aggregate only.',
        ].join('\n')
      ),
    });
...
describe('rendered markdown results', () => {
  it('renders stored discovery tables with the shared semantic entity treatment', () => {
    const table = 'example_catalog.sales_demo.gold_product_daily_summary';
    const markup = result(
      panel({
        id: 'inventory',
  ...
  it.each([
    ['Orchestrator data analysis', LEGACY_ANALYSIS_STAGE_ID, 'agent'],
    ...
    expect(markup).toContain('entity-table-list-meta">franchise: Rockstar</span>');
    expect(markup).toContain('entity-table-list-meta">franchise: 2K</span>');
...
describe('a governed asset search step', () => {
  const markup = panel({
    id: 'step-2-1-search_tagged_assets',
  ...
  it('draws each match as a row that says what it is and how wide it is', () => {
    expect(result(markup)).toContain('silver_customer_profiles');
    ...
    expect(result(markup)).toContain('data-entity-part="schema">sales_demo</span>');
    expect(result(markup)).toContain('title="example_catalog.sales_demo.silver_customer_profiles"');
  });
...
describe('the step panel and the shapes in it are on one palette', () => {
  ...
  it('spends no colour, size or radius that is not the ADAPT palette’s', () => {
...
describe('an agent step that wrote up its findings', () => {
  ...
  it('degrades to markdown when the step wrote nothing but a sentence', () => {
    const plain = panel({ id: 'step-1-agent', kind: 'agent', output: 'search_tagged_assets' });
    expect(result(plain)).toContain('search_tagged_assets');
