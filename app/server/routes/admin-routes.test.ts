...
const ACCESS_GUIDE_URL = new URL('../../../docs/ADAPT_Access_Guide.md', import.meta.url);
...
describe('reading the list', () => {
  it('shows a seed row as not removable, and runs no statement to do it', async () => {
    ...
    expect(body.entries).toEqual([
      ...
      {
        email: 'emily.huang@take2games.com',
        origin: 'seed',
        addedBy: '',
        addedAt: '',
        isYou: false,
      ...
      {
        email: 'rida.qureshi@take2games.com',
        origin: 'seed',
        addedBy: '',
        addedAt: '',
        isYou: false,
...
describe('removing an administrator', () => {
  it('takes back only what an earlier version of the app granted', async () => {
    ...
    expect(revokes[0].statement).toContain('`example_catalog`.`adapt_telemetry`');
