...
REQUIRED_SCOPES = {
    ...
    "dashboards.genie",
    # DELIBERATELY NOT OPTIONAL RESOURCE-BROWSING SCOPES. This set is the hard
    # floor for EVERY target -- a recovery aborts
    # without one -- so it holds only scopes whose absence is unrecoverable. The
    # catalog and workspace browse scopes are optional for the
...
app = apps.get("adapt_app")
if app is None:
    sys.exit(
        "the bundle resolved no app named adapt_app.\n"
        "resources/adapt_app.app.yml is what defines it."
    )
...
if missing:
    problems.append(
        ...
        "  sending it would drop them. Fix resources/adapt_app.app.yml."
    )
