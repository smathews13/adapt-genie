 ...
 * deployment settings on the way through; those are
...
describe('a deployed app with a forwarded identity', () => {
  ...
  it('records a heartbeat under the proxy-authenticated caller only', async () => {
    ...
    try {
      const response = await app.fetch('/api/activity/heartbeat', {
        method: 'POST',
        headers: { 'x-forwarded-email': 'Analyst@CMEG.Example', 'content-type': 'application/json' },
    ...
    expect(write?.params).toEqual(['analyst@cmeg.example']);
  ...
  it('scopes the read to the caller, and to nobody else', async () => {
    ...
    try {
      const response = await app.fetch('/api/conversations', {
        headers: { 'x-forwarded-email': 'analyst@cmeg.example' },
      });
    ...
    expect(read?.params).toEqual(['analyst@cmeg.example']);
  });
  ...
  it('gives two people two different tenancy keys', async () => {
    ...
    try {
      await app.fetch('/api/conversations', { headers: { 'x-forwarded-email': 'first@cmeg.example' } });
      await app.fetch('/api/conversations', { headers: { 'x-forwarded-email': 'second@cmeg.example' } });
    } finally {
      await app.close();
    }
    ...
    expect(
      tenancyQueries(store.queries)
        ...
        .filter(Boolean)
    ).toEqual(['first@cmeg.example', 'second@cmeg.example']);
  });
  ...
  it('reports the caller as signed in, not the app owner', async () => {
    ...
    try {
      ...
      expect(body.signedInAs).toBe('analyst@cmeg.example');
  ...
  it('resolves a local-only proxy identity through one unique canonical roster row', async () => {
    ...
    try {
      ...
      expect(body).toMatchObject({
        ...
        organization: { id: 'take-two-interactive' },
      });
  ...
  it('does not brand an ambiguous local-only identity from a token claim or guessed domain', async () => {
    const app = await startApp(rosterStore(['shared.user@databricks.com', 'shared.user@take2games.com']).lakebase);
    try {
      const tokenPayload = Buffer.from(JSON.stringify({ email: 'shared.user@databricks.com' })).toString('base64url');
      const response = await app.fetch('/api/identity', {
        headers: {
  ...
  it('returns sanitized user, app, and application-principal metadata', async () => {
    ...
    const reader: ControlPlaneReader = (path) => {
      if (path === SCIM_USERS_PATH) {
        return Promise.resolve({
          Resources: [{ id: '1122334455667788', userName: 'analyst@cmeg.example', displayName: 'CMEG Analyst' }],
        });
      }
      return Promise.resolve({
...
const ALICE = 'alice@cmeg.example';
const BOB = 'bob@cmeg.example';
/**
...
describe('an answer remains available when optional Postgres context cannot be read', () => {
  ...
  beforeEach(() => {
    ...
    process.env.DATABRICKS_SERVING_ENDPOINT_NAME = 'adapt-orchestrator';
  });
  afterEach(() => {
