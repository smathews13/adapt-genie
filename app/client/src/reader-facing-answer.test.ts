...
const DUMP = 'data_genie({"question": "For the title \\"Red Dead Redemption 2\\", distinct players by platform"})';
...
describe('the takeaway a reader is shown', () => {
  ...
  it('replaces a Returned transport label with a one-sentence answer', () => {
    expect(readerFacingTakeaway('Returned:', 'Asking Genie space abc123.\n\nRefunds fell 4.2% week over week.')).toBe(
      'Refunds fell 4.2% week over week.'
    );
    expect(readerFacingTakeaway('Returned:', 'Asking Genie space abc123.')).toBe(
      'The assessed data did not contain a direct answer to the question.'
    );
  });

  it('removes the returned transport suffix before promoting a finding', () => {
    const narrative = `${DUMP} returned:\n| Title | Players |\n| GTA Online | 9575 |\n\nGTA Online led the window.`;
    expect(stripToolCallDumps(narrative)).not.toContain('returned:');
    expect(readerFacingTakeaway('The analysis completed from assessed sources.', narrative)).toBe(
      'GTA Online led the window.'
    );
  });
...
describe('internal Genie transcript removal', () => {
  it('removes transport labels and duplicated relevant-table inventories', () => {
    const dirty = [
      'Asking Genie space ADAPT.',
      'Query interpretation: You want revenue by title.',
      'Relevant tables in this Genie space are:',
      'catalog.schema.sales,',
      'catalog.schema.wishlist',
      '',
      'Revenue increased **15.3%** for Grand Theft Auto V.',
    ].join('\n');

    expect(stripToolCallDumps(dirty)).toBe('Revenue increased **15.3%** for Grand Theft Auto V.');
  });

  it('keeps answer prose immediately after a table inventory', () => {
    const dirty = [
      'Relevant tables in this Genie space are:',
    ...
    expect(stripToolCallDumps(dirty)).toBe('**Bookings increased 12%.**');
  });
});
...
describe('whether the section may call itself a final answer', () => {
  ...
  it('keeps a 12-table catalog listing Complete when optional analysis detail was clipped', () => {
    const listing = [
      ...
      '',
      '- **Package note:** Optional detail was clipped at the analysis handoff bound.',
    ].join('\n');
    const honesty = answerHonesty({
      truncated: false,
      caveats: [],
  ...
  it('does not call a words-only degraded reply a refused final answer', () => {
    ...
    const honesty = answerHonesty({
      truncated: false,
      caveats: [degraded],
      narrative: 'GTA Online leads the last 30 days on distinct players in the window.',
    });
