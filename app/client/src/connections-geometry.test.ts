...
describe('resource picker geometry', () => {
  ...
  it('does not retain the retired generic picker spinner', () => {
...
describe('the parts a reader has to be able to use', () => {
  ...
  it('draws the declared Unity Catalog objects as a readable table', () => {
    ...
    // Same seating as the other resource cards. An 8px header and
  ...
  it('keeps the shared outlined add button compact', () => {
    expect(rule('.plane-add-connection')).toMatch(/flex:\s*none/);
    expect(rule('.plane-add-row')).toMatch(/justify-content:\s*flex-start/);
  });
  ...
  it('paints the affordance that leads somewhere in the action colour, and the padlock not at all', () => {
    ...
    // The ADAPT compatibility spelling of the one blue. `--ast-blue` and `--db-blue-600`
    // are the same #2272B4 and the token test holds them equal; this
...
describe('two radii, and no third', () => {
  it('writes every corner as one of the two tokens', () => {
    ...
    // Either spelling of the same two corners: the ADAPT token test holds
