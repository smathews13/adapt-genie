...
const runs: UserRunSpendEvidence[] = [
  {
    ...
    tokenCoveredRuns: 1,
    totalTokens: 100,
    resources: [{ tool: 'data_genie', resourceId: 'space-data', calls: 1 }],
  },
  {
    ...
    tokenCoveredRuns: 1,
    totalTokens: 300,
    resources: [{ tool: 'data_genie', resourceId: 'space-data', calls: 3 }],
  },
];
...
describe('individual user spend attribution', () => {
  ...
  it('never enters excluded workspace Genie usage into user or app totals', () => {
    ...
    expect(payload.reconciliation.dbu.appTotal).toBeNull();
    ...
    expect(evidence).toEqual([
      {
        ...
        totalTokens: 0,
        resources: [{ tool: 'data_genie', resourceId: 'space-data', calls: 2 }],
      },
    ]);
  });
  ...
  it('excludes aggregate identities outside the authoritative roster', () => {
  ...
  it('lists people who asked questions even when the roster stored a workspace username without @', () => {
    const page = buildUserMonitoringPage({
      spend: build(),
      runs: [],
      activity: [],
      interactions: [
        {
          ...
          questions: 2,
          runs: 1,
          firstActive: '2026-09-04T10:00:00Z',
          lastActive: '2026-09-04T11:00:00Z',
        },
      ],
      roles: new Map([['manish.namburi@example.test', 'admin']]),
      unit: 'USD',
    });
    expect(page.users.map((row) => row.email).sort()).toEqual(['manish.namburi@example.test', 'sam.mathews']);
  });
