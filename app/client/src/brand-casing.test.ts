 ...
 * what must remain exact is the ADAPT wordmark and the lockups it leads, such as
 * the ask hero chip "ADAPT digital sales intelligence".
 *
 * Two ways a lowercase lockup shows up capitalised, and this file guards both.
 * One is the obvious one: somebody types a title-cased variant into the
...
function body(selector: string, css: string) {
  ...
  return (
    css.replace(/\/\*[\s\S]*?\*\//g, ' ').match(new RegExp(`(?:^|[{}])\\s*${escaped}\\s*\\{([^{}]*)\\}`))?.[1] ?? ''
  );
}
...
describe('the ask hero chip is the exact brand lockup', () => {
  const homeSource = withoutComments(HOME_PAGE);

  it('renders "ADAPT digital sales intelligence", the acronym upper and the rest lower', () => {
    // ADAPT is an acronym, so it is uppercase; the descriptor beside it stays lowercase.
    expect(homeSource).toContain('ADAPT digital sales intelligence');
  });

  it('does not title-case or fully lowercase the descriptor', () => {
    expect(homeSource).not.toContain('ADAPT Digital Sales Intelligence');
    expect(homeSource).not.toContain('adapt digital sales intelligence');
    expect(homeSource).not.toMatch(/ADAPT\s+Digital\s+Sales\s+Intelligence/);
  });
  ...
  it('has no ask-hero rule anywhere that transforms case', () => {
    const askHeroRules = STYLESHEET.replace(/\/\*[\s\S]*?\*\//g, ' ').match(/\.ask-hero[\w-]*\s*\{[^{}]*\}/g);
