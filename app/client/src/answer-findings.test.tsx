...
describe('finding layout', () => {
  ...
  it('turns a dense paragraph into bullets and keeps figures and chips', () => {
    const markup = render(
      'The profile base is **12,000** distinct players in `silver_player_profiles`. Identity scores range **0.8365–0.8905**. Civilization VII sessions average **108** minutes.'
    );
    expect(markup).toContain('<ul class="answer-list">');
  ...
  it('promotes a short bold lead-in to the finding label', () => {
    const markup = render(
      '**Sessions** Average session length is **108** minutes on Civilization VII. NBA 2K25 sits at **41** minutes.'
    );
    expect(markup).toContain('class="answer-finding-label"');
  ...
  it('splits an unlabeled essay into Who / Identity / Sessions blocks', () => {
    const markup = render(
      [
        ...
        'Identity confidence scores range **0.8365–0.8905**. GTAV leads on email addressability at **60.0%**.',
        'Civilization VII sessions average **108** min. NBA 2K25 sits at **41** min. GTAO net bookings are **$42,650**.',
        '**11** countries appear in the `country_code` column. Codes follow ISO 3166-1 alpha-2.',
        'The `label` column splits titles between Rockstar and 2K. Rockstar bookings are **$42,650** for GTAO.',
