...
const NARRATIVE =
  ...
  'business definition of is_recurrent_consumer_spending. Sessions were flat over the window, and ' +
...
const ANSWER = {
  ...
  sources: [
    ...
    { name: 'ADAPT Sales Genie', freshness: 'Current demo seed' },
  ],
...
test('a table the answer declared becomes a link; nothing else does', async ({ page }) => {
  ...
  await page
    .locator('.answer-card')
    .first()
    .screenshot({ path: join(SHOTS, 'answer-with-links.png') });
});
...
test('a link to an entry the report no longer has says so', async ({ page }) => {
  ...
  await page.route('**/api/preflight', (route) =>
    route.fulfill({
      json: { ...PREFLIGHT, checks: [tableCheck(PURCHASES)], counts: { ok: 1, failed: 0, unverified: 0 } },
    })
  );
