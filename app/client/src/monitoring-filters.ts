...
export interface MonitoringFilters {
  ...
  table: string;
  /** App group id, or '' for all groups. */
  appGroup: string;
  /**
...
export function withUserBrowserFilters(
  search: string,
  state: Pick<UserBrowserState, 'search' | 'role' | 'organizations' | 'unit'>
): string {
  const next = new URLSearchParams(search);
  const entries = [
...
export function closedUserMonitoring(search: string): string {
  const next = new URLSearchParams(search);
  for (const name of ['users', 'userSearch', 'userRole', 'userOrganization', 'userUnit', 'userCursor']) {
    next.delete(name);
  }
  next.delete(PERSON_PANEL_PARAM);
  next.delete(QUESTION_PARAM);
