...
describe('every admin route is wrapped, and the router is where that is decided', () => {
  it('wraps each path ADMIN_PAGE_NAMES knows about', () => {
    ...
    for (const path of ADMIN_PATHS) {
      ...
      expect(route![0], `App.tsx gates ${path}`).toContain('<AdminRoute>');
    }
  });
  ...
  it('wraps nothing else, so a page nobody decided was admin cannot become one', () => {
    const wrapped = [...APP.matchAll(/path: '([^']+)',[\s\S]*?errorElement:/g)]
      .filter((match) => match[0].includes('<AdminRoute>'))
      .map((match) => match[1]);
    expect(wrapped.sort()).toEqual([...ADMIN_PATHS].sort());
  });

  it('lets every settled refusal clear startup without mounting a lazy page', () => {
    expect(APP).toMatch(/function AdminRoute[\s\S]*useOptionalRole\(\)/);
    expect(APP).toMatch(
      /if \(role && role\.state !== 'resolving' && !showsAdminSurfaces\(role\.state\)\) markReady\(\)/
    );
    const layout = readFileSync(new URL('Layout.tsx', import.meta.url), 'utf8');
    expect(layout).not.toContain('useStartupReadiness');
  });
});
