...
/**
 * The component that renders them, shared by ADAPT Ask and the Run Explorer's Final
...
describe('the caveats under an answer', () => {
  ...
  it('still labels the block, which is what tells a reader what the bullets are', () => {
    ...
    expect(PANEL_SOURCE).toContain('<section className={`keep-in-mind${collapsed');
    expect(PANEL_SOURCE).toContain('aria-label="Caveats">');
