 ...
 * as many words. `declaredScopes` comes from the server identity report.
 * A source-only Git artifact gets the universal ask-path scopes from
 ...
 * Reading the list is what stops this screen from inventing or preserving stale
 * entitlement names.
 */
...
/** Optional entitlements ADAPT exposes in the client. */
export const ADAPT_OPTIONAL_USER_API_SCOPES = OPTIONAL_USER_API_SCOPES.filter((scope) =>
  /^(catalog\.|workspace\.|postgres)/.test(scope)
);
const HIDDEN_OPTIONAL_USER_API_SCOPES = new Set<string>(
  OPTIONAL_USER_API_SCOPES.filter((scope) => !ADAPT_OPTIONAL_USER_API_SCOPES.includes(scope))
);
 ...
 *
 * `APP_TITLE` used to be a long name set as an h1. The ADAPT lockup now supplies
 * the mark and wordmark under the Databricks logo, and the dialog takes its
 * accessible name from it.
 */
 ...
 *
 * Optional names that ARE declared keep their place in declaration order (cmeg
...
export function scopeRows(
  ...
  const isVisibleScope = (scope: string): boolean => !HIDDEN_OPTIONAL_USER_API_SCOPES.has(scope);
  const cleanScopes = (value: readonly unknown[] | null | undefined): string[] =>
    Array.isArray(value)
      ...
      : [];
  const declaredList = cleanScopes(declared).filter(isVisibleScope);
  const missingList = cleanScopes(missing).filter(isVisibleScope);
  const heldScopes = tokenScopes == null ? null : cleanScopes(tokenScopes).filter(isVisibleScope);
  ...
  for (const name of ADAPT_OPTIONAL_USER_API_SCOPES) {
...
/**
 * End this browser tab's ADAPT session without touching Databricks sign-in.
 *
