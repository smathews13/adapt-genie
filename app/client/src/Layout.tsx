...
export function NavLinks({
  ...
  // Carry the open conversation from ADAPT Ask into Run Explorer. Clicking through
  // used to drop it, so the Explorer opened on whoever's run was newest instead
  // of the conversation the reader was just looking at. Only while on ADAPT Ask
      ...
      {navEntries(role.state, features).map((entry) => {
        ...
        return (
          <NavLink
            ...
            // ADAPT Ask only. Without it the root path matches every route and the
    ...
    <div className="app-sky-host">
      <div className="min-h-screen flex flex-col app-frame">
        ...
        <header className="app-header ast-top-chrome border-b flex items-center">
          ...
          />
          {/* Navigation comes from the role contract: Ask for consumers and the
            operational surfaces for administrators. The breakpoint stays fixed:
            below it the nav and the brand alone over-subscribe the
          ...
          {/* The way into settings, and now into settings rather than towards them.
            ...
            AVAILABLE TO EVERY SIGNED-IN USER. Consumers can customize their own
            Appearance and Insights preferences; privileged panes stay visible
            but disabled inside Settings.
        ...
        {/* The role is handed down because this modal is a sibling of Outlet,
          ...
          consumers get Environment, Appearance and Insights while privileged
          panes remain disabled. */}
        {settingsVisible ? (
