...
# fails the deploy, so prune idle entities first when already at the ceiling
# and refuse if that still cannot make room.
...
if [[ "$ALLOW_UNATTRIBUTED_FIGURES" == true ]]; then
  ...
  note "                      estate with incomplete attribution. Protected-column"
...
#
# Not cleared: the data Genie space id above, which is a documented override for
# the pre-deploy case.
...
if [[ "$SKIP_LOG" != true ]]; then
  ...
  LOG_STDOUT="$(mktemp "${TMPDIR:-/tmp}/adapt-log-model.XXXXXX")"
  on_exit "rm -f '$LOG_STDOUT'"
  (cd "$BUNDLE_ROOT/agent" && uv run --python 3.13 python log_model.py "${LOG_ARGS[@]+"${LOG_ARGS[@]}"}" > "$LOG_STDOUT")
  LOG_SUMMARY="$(mktemp "${TMPDIR:-/tmp}/adapt-log-summary.XXXXXX")"
  on_exit "rm -f '$LOG_SUMMARY'"
  SUMMARY_READER="$BUNDLE_ROOT/bundle/read-log-summary.py"
  [[ -f "$SUMMARY_READER" ]] || die "bundle/read-log-summary.py is missing.
The release cannot safely distinguish a successful log from trailing warnings."
  MODEL_VERSION="$(python3 "$SUMMARY_READER" "$LOG_STDOUT" --write "$LOG_SUMMARY")" \
    || die "log_model.py did not print a readable model-version summary.
The model may already be registered. Inspect $LOG_STDOUT before retrying."
...
if [[ ! -f "$BUNDLE_ROOT/bundle/drift-check.py" \
  ...
  die "The generated model scope contract is incomplete.
bundle/drift-check.py, bundle/scope-contract.py, and bundle/scope-contract.json
are all required. A missing release check is not a pass."
fi
