...
export function shouldDropAppkitCacheSchema(tableOwners, appRole) {
  const ownedByApp = tableOwners.length > 0 && tableOwners.every((owner) => owner === appRole);
  return !ownedByApp;
}
...
function required(name, how) {
  ...
  console.error(`${how}\n`);
  console.error(
    'All of DATABRICKS_CONFIG_PROFILE, PGHOST, PGDATABASE, PGUSER and APP_PG_ROLE\n' +
...
function resolveAppSchema() {
  ...
  if (override && override !== declared) {
    ...
    console.log(`  note  PLAYER_INSIGHTS_APP_SCHEMA='${override}' (compiled default is '${declared}')`);
  }
...
async function main() {
  ...
  for (const schema of SCHEMAS) {
    ...
    if (rowCount === 0) {
      console.log(`ok: ${schema} does not exist; database CREATE is granted and the app will create and own it`);
    }
  }
