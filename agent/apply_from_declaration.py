...
THE SOLUTION. One resolver that merges those two documents into the compatibility
environment variables `log_model.py` / `bundle/agent-release.sh` already
understand, then a sibling script that exports them and calls the existing
release path. Nothing here silently re-logs.
...
#: Keys Apply may promote into a re-log. Matches the companion app's
#: `needs-model-version` declaration, minus refused ones.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "into compatibility exports for a model re-log."
        )
    )
    parser.add_argument(
        "--i-am-deploying",
