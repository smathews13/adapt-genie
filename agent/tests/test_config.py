...
The serving container inherits none of the compatibility variables the release
shell exports, so two things need proving: a value set at log time survives into
the loaded model, and a value nobody set stops the process instead of quietly
becoming some other deployment's.
"""
...
def test_the_artifact_carries_configuration_into_a_bare_environment():
    """The serving case: none of the compatibility variables are present."""
...
def test_the_environment_variable_no_longer_reaches_anything():
    ...
    The retired synthetic-data switch was a supported way to turn the disclosure
    on, so it will survive in shell history and in half-remembered runbooks. It
