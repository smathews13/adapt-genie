...
describe('User Monitoring modal filter isolation', () => {
  it('owns range, role, search, unit, and paging without rewriting the parent URL', () => {
    ...
    expect(SOURCE).not.toMatch(/updateUserBrowser\('user(?:Search|Role|Unit|Cursor)'/);
  });
...
describe('User Monitoring filter menus', () => {
  it('uses the shared body-level non-modal popover without fighting the dialog focus trap', () => {
    ...
    expect(SOURCE.match(/contentClassName="monitoring-users-filter-menu"/g)).toHaveLength(1);
