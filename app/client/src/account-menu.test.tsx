...
describe('account menu', () => {
  it('uses the official Databricks organization mark in the trigger and panel', () => {
    const identity: Identity = {
      signedInAs: 'employee@databricks.com',
      organization: organizationForEmail('employee@databricks.com'),
      executionMode: 'user-verified',
    };
    ...
    expect(trigger).toContain('aria-label="Signed in as employee@databricks.com"');
    expect(trigger).toContain('title="employee@databricks.com"');
  ...
  it('opens with the live identity and the fixed menu order', () => {
    ...
    expect(markup).not.toMatch(/Slack[^<]*ADAPT|ADAPT[^<]*Slack/);
  });
  ...
  it('does not send Slack messages through the ADAPT server', () => {
  ...
  it('matches the selected conversation colour with the ADAPT primary token', () => {
  ...
  it('uses the same primary family for menu actions with a stronger keyboard focus ring', () => {
    expect(ACCOUNT_CSS).toMatch(
  ...
  it('keeps sign-out concise without session explanations or disclosure UI', () => {
    const markup = renderToStaticMarkup(<AccountMenuPanel identity={IDENTITY} role="super_admin" onClose={() => {}} />);
    expect(markup).toContain('Sign out of ADAPT');
