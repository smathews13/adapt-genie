...
describe('the Lakebase app schema', () => {
  it('uses an ADAPT-owned schema for a direct Git deployment', () => {
    expect(DEFAULT_APP_SCHEMA).toBe('adapt');
    const gitDeploy = { [LAKEBASE_ENDPOINT_ENV]: 'projects/example/branches/production' };
    expect(resolveAppSchema(gitDeploy)).toBe('adapt');
    expect(resolveAppSchema({ ...gitDeploy, [APP_SCHEMA_ENV]: '' })).toBe('adapt');
    expect(resolveAppSchema({ ...gitDeploy, [APP_SCHEMA_ENV]: '   ' })).toBe('adapt');
