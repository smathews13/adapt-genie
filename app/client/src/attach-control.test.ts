...
describe('the composer wires the control to that one decision', () => {
  it('drives the label, the glyph, the disabled attribute and aria-busy from it', () => {
    ...
    expect(HOME_PAGE).toContain('<AdaptBusyButtonContent');
    expect(HOME_PAGE).toContain('busy={attachControl.pending}');
    expect(HOME_PAGE).toContain('busyLabel={attachControl.label}');
    expect(HOME_PAGE).not.toContain('Loader2');
  });
