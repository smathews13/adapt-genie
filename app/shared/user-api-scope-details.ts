/**
 * Human-readable Databricks descriptions for scopes shown by ADAPT.
 *
 ...
 */
export const PLATFORM_DEFAULT_USER_API_SCOPES = ['iam.access-control:read', 'iam.current-user:read'] as const;
...
export const USER_API_SCOPE_DETAILS: Readonly<Record<string, string>> = {
  ...
  'workspace.workspace:read': 'Allows the app to read workspace objects (folders, notebooks, and files).',
};

export function userApiScopeDetail(scope: string): string {
  return USER_API_SCOPE_DETAILS[scope] ?? 'Allows the app to use this Databricks permission.';
}
