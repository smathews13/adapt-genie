 ...
 *
 * OFF for ADAPT. The registry entry, settings route, declaration reader and
 ...
 * does not. An unset optional route is a valid state, and a row reading `Not
 * checked` over an empty value promises a verdict that will never arrive.
 *
...
export function hasRemoteEnd(row: ResourceRow, check?: PreflightCheck): boolean {
  ...
  const { resource } = row;
  if (!resource.namesRemoteObject && !resource.actualFromCheck) return false;
    ...
    .filter(
      (row) =>
        (row.resource?.namesRemoteObject ||
          Boolean(row.resource?.actualFromCheck) ||
          Boolean(checkFor(row.resource, checksById))) &&
...
/**
 * Stable scan order for the normal Connections list.
 *
 * Actionable blocked and drifted rows keep their sections above this list.
 * Inside a section, connected rows lead, unsettled rows follow, and settled
 * disconnected rows come last. The authored registry order breaks every tie.
 */
export function orderConnectionsByCanonicalStatus(
  readings: readonly ConnectionReading[],
  isChecking: (reading: ConnectionReading) => boolean = () => false
): ConnectionReading[] {
  const rank = (reading: ConnectionReading): number => {
    if (!isChecking(reading) && reading.status === 'reachable') return 0;
    if (isChecking(reading) || reading.marker === 'pending') return 1;
    return 2;
  };
  return readings
    .map((reading, authoredIndex) => ({ reading, authoredIndex, rank: rank(reading) }))
    .sort((left, right) => left.rank - right.rank || left.authoredIndex - right.authoredIndex)
    .map(({ reading }) => reading);
}
...
export function groupConnections(
  ...
  return GROUP_ORDER.filter((group) => (byKey.get(group.key) ?? []).length > 0).map((group) => {
    ...
    return {
      ...
      title: group.title,
      aside: '',
      readings: group.key === 'reachable' ? orderConnectionsByCanonicalStatus(readings, isChecking) : readings,
    };
  });
}
