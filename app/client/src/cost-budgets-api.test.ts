...
describe('cost budget API responses', () => {
  it('loads stored budgets when the store answered', async () => {
    ...
    const audit = { appliedAt: '2026-09-02T16:51:00.000Z', appliedBy: 'sam.mathews@example.com' };
...
describe('Cost budget Apply copy', () => {
  it('renders persisted apply metadata without exposing the full actor in visible text', () => {
    expect(
      budgetAuditView(
        { appliedAt: '2026-09-02T16:51:00.000Z', appliedBy: 'sam.mathews@example.com' },
        () => 'Sep 2, 10:51 AM'
      )
    ).toEqual({
      text: 'Last applied Sep 2, 10:51 AM by sam.mathews',
      title: 'sam.mathews@example.com',
    });
  ...
  it('shows truthful per-button states and the ADAPT mark while applying', () => {
    ...
    expect(markup(SETTINGS_SAVE_IDLE)).toContain('data-busy="false"');
    expect(markup(SETTINGS_SAVE_IDLE)).toContain('>Apply<');
    expect(markup({ kind: 'saving' })).toContain('Applying');
    expect(markup({ kind: 'saving' })).toContain('adapt-button-mark');
    expect(markup({ kind: 'saving' })).toContain('data-busy="true"');
    expect(markup({ kind: 'saving' })).toContain('disabled');
    const saved = markup({ kind: 'saved' });
    expect(saved).toContain('Applied');
    ...
    expect(markup({ kind: 'failed', message: 'no' })).toContain('data-busy="false"');
  });
