...
describe('the list of assets the agent may consider', () => {
  ...
  it('puts removal feedback directly after each relevant add action', () => {
    expect(CARD_SOURCE).toMatch(
      /className="plane-add-connection"[\s\S]*?Add connection[\s\S]*?<\/Button>\s*<ConnectionRemovalStatus notice=\{removalNotice\}/
    );
  ...
  it('puts the compact outlined add control after the declared asset rows', () => {
    const markup = renderToStaticMarkup(
      <DeclaredConnectionsCard entries={[entry()]} allowMutations onChanged={() => {}} />
    );
...
describe('every addable kind browses', () => {
  it.each([
    ...
    ['agent-endpoint', 'serving-endpoints'],
  ...
  it('leaves only MCP servers on the explicit manual identifier path', () => {
    for (const option of ADDABLE_KINDS) {
      if (!option.browse) {
        expect(option.id).toBe('mcp-server');
        continue;
      }
  ...
  it('starts with no resource type and reveals browse or manual fields only after selection', () => {
    ...
    expect(CARD_SOURCE).toMatch(/chosenKind && !picker \? \([\s\S]*plane-manual-fields/);
    expect(CARD_SOURCE).toContain('Display name');
...
describe('how a listed asset reads on its row', () => {
  it('knows a minted id from a name', () => {
    ...
    expect(isOpaqueAssetId('9cd919d96b11bf1c')).toBe(true);
