...
describe('SSE terminal response projection', () => {
  it('parks an approval response without leaving it Live', () => {
    ...
    expect(terminalSettlementForResponse(plan, { ...plan, runId: 'run-plan-1' })).toEqual({
  ...
  it('matches a clarification to its persisted message id', () => {
    ...
    expect(
      terminalSettlementForResponse(clarification, { ...clarification, runId: 'run-clarification-1' })
    ).toMatchObject({
