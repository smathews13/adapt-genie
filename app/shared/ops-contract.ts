...
export type CostResourceKind = 'serving-endpoint' | 'sql-warehouse' | 'app' | 'genie-space';
...
export interface CostTile {
   ...
   *
   * Warehouse id, serving-endpoint name, app name, or Genie space id. Empty when this deployment cannot name the
   * object. The page builds a Databricks link from this and the live workspace
   * host; it does not invent a host or a path.
   */
  resourceId: string;
  /** A second configured identifier shown in the title. */
  ...
  evidence?: {
    ...
    /** Executable compatibility field consumed by deployed Ops clients. */
...
export interface HealthDependency {
   ...
   * Some are not: table checks are one probe over a manifest. A link
...
/**
 * One thing the platform says about itself, as opposed to something ADAPT probed.
 *
...
/**
 * What the platform records about the app itself, as opposed to what ADAPT probes.
 *
