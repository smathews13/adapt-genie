...
describe('streamed progress geometry', () => {
  ...
  it('contains long names, timing, code, and descriptions inside reserved tracks', () => {
    ...
    expect(rule('.live-step-head')).toMatch(/grid-template-columns:\s*minmax\(0,\s*1fr\) max-content 168px 80px/);
    expect(rule('.live-step-type')).not.toMatch(/\b(?:min-)?width\s*:/);
  ...
  it('follows inside the action list without scrolling page ancestors', () => {
    ...
    expect(rule('.ast-splash-run')).toMatch(/display:\s*flex/);
    expect(rule('.ast-splash-run')).toMatch(/flex:\s*1 1 auto/);
    ...
    expect(rule('.ast-splash-run')).toMatch(/min-height:\s*0/);
    expect(rule('.ast-splash-run')).toMatch(/overflow:\s*hidden/);

    expect(PANEL).toContain('list.scrollTop = list.scrollHeight - list.clientHeight');
    expect(PANEL).toContain('step.detail');
    expect(PANEL).toContain('step.result');
    expect(PANEL).toContain('onScroll=');
    expect(PANEL).not.toContain('.scrollIntoView(');
  });

  it('adds the animated ADAPT mark only to a currently running badge', () => {
    const running = progress(2, { status: 'running', duration: 0 });
    expect(running).toContain('live-step-busy-mark');
    expect(running).toContain('adapt-button-mark__ring');
