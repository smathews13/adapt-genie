...
const SOURCE = {
  name: 'main.sales_demo.silver_sales_activity',
...
describe('the Overview Final Answer module', () => {
  ...
  it('titles a finished catalog listing Complete when optional analysis detail was clipped', () => {
    const html = markup({
      takeaway: 'All 12 declared tables live in **`example_catalog.sales_demo`** and are available to query.',
    ...
    expect(html).toMatch(/final-answer-takeaway[\s\S]*<strong[\s\S]*<code[\s\S]*example_catalog/);
  ...
  it('lists each source as a bullet with one Open control, not a wrapping tangle', () => {
    ...
    expect(html).toContain('silver_sales_activity');
