 ...
 */
const NAME = 'main.sales_demo.data_dictionary';
const FRESHNESS = 'Read during this run';
/** The tables a run's figures actually came from, beside the dictionary above. */
const QUERIED = ['main.sales_demo.gold_product_daily_summary', 'main.sales_demo.gold_revenue_daily'];
...
function renderCard(raw: WireAnswer, collapseSupportingDetails = false): string {
...
describe('the source list', () => {
  it('keeps Ask data sources and caveats in separate closed panels', () => {
    const answer = normalizeAnswer({ ...wireAnswer(), caveats: ['Watch the reporting window.'] }) as Answer;
...
describe('the row that names a source', () => {
  it('uses the shared entity parts and marks the table segment for emphasis', () => {
    const parts = renderToStaticMarkup(
      <EntityParts text="sales_demo.gold_product_daily_summary" entity={QUERIED[0]} />
    );
