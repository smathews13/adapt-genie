...
export function explorerEventLabel(
  ...
  // Unclassified agent rows keep their projected name rather than inventing a
  // rival vocabulary for the Orchestrator's data-analysis flow.
  return row.name;
}
 ...
 * optional analysis clip on a finished listing) is shown as complete, so this panel cannot
 * disagree with Ask.
 */
