...
describe('lining up configured against actual', () => {
  ...
  it('does not treat an unmeasured value as agreeing', () => {
    ...
    const all = states({
      report: report({
        configuration: [configured({ key: 'catalog', value: 'taketwo_catalog' })],
  ...
  it('shows the ADAPT foundation-model default when nothing named one', () => {
    const all = states({ report: report({ configuration: [] }), environment: {}, stored: stored() });
    expect(state(all, 'llm-endpoint').configured).toBe('databricks-claude-sonnet-4-6');
    expect(state(all, 'llm-endpoint').configuredFrom).toBe('app-default');
  });
  ...
  it('reads the namespace in use from the tables the orchestrator reached', () => {
    ...
    const all = states({
      report: report({
        checks: [
          check({ id: 't1', kind: 'table', name: 'taketwo_catalog.player_insights.players' }),
          check({ id: 't2', kind: 'table', name: 'taketwo_catalog.player_insights.sessions' }),
        ],
      }),
      environment: {},
      stored: stored(),
    ...
    expect(state(all, 'catalog').actual).toBe('taketwo_catalog');
...
describe('what the page refuses to call healthy', () => {
  ...
  it('does not call an optional setting nobody set a provenance failure', () => {
     ...
     * turns the whole page's status to `blocked`. cmeg read as misconfigured while
  ...
  it('does not treat a provenance it could not read as a route the artifact did not sanction', () => {
    ...
    const all = states({
      report: report({
        configuration: [unreadable({ key: 'warehouse_id', value: 'wh-take-two', source: { kind: 'artifact' } })],
      }),
      environment: {},
      stored: stored(),
    });
  ...
  it('accepts a value that came from the artifact', () => {
    const all = states({
      report: report({
        configuration: [configured({ key: 'warehouse_id', value: 'wh-take-two' })],
      }),
      environment: {},
      stored: stored(),
    });
