...
function wireAnswer(caveats: string[]) {
  return {
    ...
    mode: 'live',
    provenance: 'live',
    takeaway: 'GTA Online led on active players over the window.',
    ...
    sources: [{ name: 'main.sales_demo.gold_product_daily_summary', freshness: 'Updated daily' }],
    caveats,
...
/** The card as ADAPT Ask mounts it, as markup. */
...
describe('the caveats an answer arrives with', () => {
  ...
  it('tags a caveat with the one table it is about, and only then', () => {
    const scoped = caveatBullets(renderCard(['gold_product_daily_summary counts a player once per day.']));
    expect(caveatsSection(renderCard(['gold_product_daily_summary counts a player once per day.']))).toContain(
      'caveat-scope'
    );
    // Short name, because the catalog and schema are the same on every row and
    // are already said in full above.
    expect(scoped[0]).toContain('gold_product_daily_summary');
    expect(scoped[0]).not.toContain('main.sales_demo.gold_product_daily_summary');
