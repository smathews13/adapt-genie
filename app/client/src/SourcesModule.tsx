...
export function SourcesModule({
   ...
   *
   * Passed in rather than fetched, because ADAPT Ask lifts a degradation out into
  ...
  /** Ask keeps provenance and caveats in separate closed panels. */
  ...
  const provenance =
    leftover.length > 0 || unmatched.length > 0 ? (
      <section
        className={`sources-module${collapseSupportingDetails ? ' answer-support-panel' : ''}`}
        aria-label="Data sources and provenance"
      >
        <div className={collapseSupportingDetails ? 'answer-support-head' : undefined}>
          <p className="source-list-heading">{collapseSupportingDetails ? 'Data sources' : 'Sources'}</p>
          {collapseSupportingDetails ? (
            ...
            >
              {sourcesOpen ? 'show less' : 'show more'}
              <ChevronDown className={sourcesOpen ? 'rotate-180 transition-transform' : 'transition-transform'} />
            </Button>
          ) : null}
        </div>
        {!collapseSupportingDetails || sourcesOpen ? (
  ...
  return (
    <>
      {provenance}
      <KeepInMind caveats={caveats} sources={rows} sql={sql} collapsed={collapseSupportingDetails} />
    </>
  );
}
