...
export function runStatusFor({
   ...
   */
  verdict?: RunVerdict;
  /** A complete answer that succeeded after a governed-data retry. */
  recoveredWithRetries?: boolean;
  ...
  if (answered) {
    ...
    if (recoveredWithRetries) {
      return { label: 'Recovered · complete with retries', tone: 'is-ready', alive: false, finished: true };
    }
