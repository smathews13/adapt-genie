...
describe('Ops Query History attribution', () => {
  ...
  it('paginates the complete range and allocates only exact ADAPT tags', async () => {
    const listQueries = vi
      .fn()
      .mockResolvedValueOnce({
        res: [row('app-1', 25, 'ADAPT'), row('other-1', 75, 'Other')],
        next_page_token: 'page-2',
        has_next_page: true,
      })
      .mockResolvedValueOnce({
        res: [row('app-2', 100, 'ADAPT')],
        has_next_page: false,
      });
    const result = await readWarehouseQueryAttribution({
      warehouseId: 'warehouse-1',
  ...
  it('fails closed when the denominator lacks execution time but preserves the ADAPT count', async () => {
    const transport: WarehouseQueryHistoryTransport = {
      listQueries: vi.fn().mockResolvedValue({
        res: [row('app-1', 25, 'ADAPT'), row('other-1', null, 'Other')],
        has_next_page: false,
      }),
    };
    const result = await readWarehouseQueryAttribution({
    ...
    expect(result.adaptQueries).toBe(1);
  ...
  it('fails closed on broken pagination instead of calling a partial denominator complete', async () => {
    ...
    expect(result).toMatchObject({ complete: false, adaptQueries: 1, totalQueries: 1 });
  });
  ...
  it('splits periods longer than the Query History 30-day limit and keeps one complete denominator', async () => {
    const listQueries = vi
      .fn()
      .mockResolvedValueOnce({ res: [row('app-1', 10, 'ADAPT')], has_next_page: false })
      .mockResolvedValueOnce({ res: [row('other-1', 20, 'Other')], has_next_page: false })
      .mockResolvedValueOnce({ res: [row('app-2', 30, 'ADAPT')], has_next_page: false });
  ...
  it('falls back to query duration when execution metrics are absent', async () => {
    const result = await readWarehouseQueryAttribution({
      ...
      transport: {
        listQueries: vi.fn().mockResolvedValue({
          res: [{ ...row('app-1', null, 'ADAPT'), duration: 25 }],
          has_next_page: false,
        }),
      },
    });
  ...
  it('separates generated SQL by Genie space and never also counts it as ADAPT SQL', async () => {
    const genieRow = (id: string, spaceId: string, executionMs: number) => ({
      ...row(id, executionMs, 'ADAPT'),
  ...
  it('aggregates large pages as they arrive and retains no raw private fields in the result', async () => {
    const pages = Array.from({ length: 3 }, (_, page) =>
      Array.from({ length: 999 }, (_, index) => row(`${page}-${index}`, 1, index % 2 ? '' : 'ADAPT'))
    );
    const listQueries = vi
      .fn()
