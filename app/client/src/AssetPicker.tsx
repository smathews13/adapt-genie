...
export function AssetPickerRow({
  ...
  const connected = state?.selectable === false;
    ...
    return (
      ...
      >
        <label className="warehouse-picker-choice">
          ...
          {connected ? <span className="asset-picker-connected-label">Connected</span> : null}
        </label>
      </li>
    );
  }
      ...
      {!loading && response?.status === 'ok' && items.length > 0 && response.next_page_token ? (
        <div className="asset-picker-more">
          <Button
            ...
            onClick={onMore}
          >
            <AdaptBusyButtonContent busy={loadingMore} label="Load more" busyLabel="Loading" />
          </Button>
        </div>
      ) : null}
    </div>
