...
describe('a failed run’s process', () => {
  ...
  it('does not paint a successful table listing as Request refused', () => {
    ...
    const markup = card(
      answer({
        takeaway: 'This deployment declares 12 tables in the sales schema.',
        caveats: [
  ...
  it('renders nested bold+code in the takeaway so the schema name has no asterisks', () => {
    ...
    expect(markup).toMatch(/answer-takeaway[\s\S]*<strong[\s\S]*<code[\s\S]*example_catalog\.sales_demo/);
