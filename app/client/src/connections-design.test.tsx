...
function groupsFor(
  resources: SettingsPayload['resources'],
  checks: PreflightCheck[] = [],
  isChecking: NonNullable<Parameters<typeof groupConnections>[1]> = () => false
) {
  return groupConnections(readConnections(payload({ resources, checks }), []), isChecking);
}
...
describe('the sections the rows are grouped into', () => {
  ...
  it('keeps actionable verdict groups while each row states its exact status', () => {
    ...
    expect(groups.map((group) => group.key)).toEqual(['reachable', 'blocked']);
  ...
  it('orders the main list by canonical status while preserving authored ties', () => {
    const groups = groupsFor(
      [
        row('genie-data', { configured: 'space-data' }),
        row('sql-warehouse', { configured: 'warehouse' }),
        row('llm-endpoint', { configured: 'databricks-claude-sonnet-4-6' }),
        row('catalog', { configured: 'catalog' }),
      ],
      [check('sql-warehouse', 'ok'), check('llm-endpoint', 'ok')],
      (reading) => reading.resource.id === 'genie-data'
    );
    expect(groups[0].readings.map((reading) => reading.resource.id)).toEqual([
      'sql-warehouse',
      'llm-endpoint',
      'genie-data',
      'catalog',
  ...
  it('keeps actionable disconnected rows after the status-sorted main list', () => {
    const groups = groupsFor(
      [
        row('genie-data', { configured: 'space-data' }),
        row('sql-warehouse', { configured: 'warehouse' }),
      ...
      [check('genie-data', 'failed'), check('sql-warehouse', 'ok')]
    );
    expect(groups.map((group) => group.key)).toEqual(['reachable', 'blocked']);
    expect(groups[0].readings.map((reading) => reading.resource.id)).toEqual(['sql-warehouse', 'catalog']);
    expect(groups[1].readings.map((reading) => reading.resource.id)).toEqual(['genie-data']);
  });
});
...
describe('the product marks on the rows', () => {
   ...
   * an unavailable product -- because the official artwork was not in the repository. A
...
describe('long values in a fixed column', () => {
  ...
  it('drops the head, so what distinguishes two values survives', () => {
    const cut = truncateHead('main.player_insights_demo.fact_sessions', 20);
