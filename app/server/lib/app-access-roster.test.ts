...
describe('Databricks App access roster', () => {
  it('uses explicit App users as the roster and overlays ADAPT roles', () => {
  ...
  it('keeps stored roles visible when the App ACL cannot be read', () => {
    const aligned = alignRosterWithAppAccess(roster, {
      available: false,
      principals: [],
  ...
  it('keeps user, group, and service-principal access separate', () => {
  ...
  it('reads the live App ACL with the forwarded user token', async () => {
    ...
    expect(call.mock.calls[0]?.[0]).toBe('https://workspace.example/api/2.0/permissions/apps/adapt');
