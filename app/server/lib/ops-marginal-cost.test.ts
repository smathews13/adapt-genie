...
describe('Aug 26–Sep 1 component-total and marginal Ask audit fixture', () => {
  it('shows the dedicated endpoint total while retaining its request-active share only for efficiency', () => {
    ...
    const tiles = buildTiles(
      ...
      ],
      {
        ...
        adaptQueries: 26,
        totalQueries: 1_024,
        adaptExecutionMs: ASK_EXECUTION_MS,
  ...
  it('keeps persisted Ask tokens visible when system usage evidence is unavailable', () => {
    const interactive = runs();
    const tile = foundationCostTile(IDS, null, 'system table unavailable', interactive);
    expect(tile).toMatchObject({
      amount: null,
      unavailable: 'system table unavailable',
      evidence: {
        tokens: {
          input: 2_600,
          output: 520,
  ...
  it('reconciles marginal serving, token, and Ask SQL to users exactly once', () => {
    ...
    const spend = buildSpendByUser({
      ...
      queryComplete: true,
      queryUsers: [
        { email: 'a@example.test', adaptExecutionMs: ASK_EXECUTION_MS / 2, genieSpaces: [] },
        { email: 'b@example.test', adaptExecutionMs: ASK_EXECUTION_MS / 2, genieSpaces: [] },
      ],
      runs: userRuns,
