 ...
 * the ADAPT operations-tab handoff.
 *
      ...
      <BlockBody>
        {block.busy && !payload ? (
          <div className="ops-loading-state">
            <AdaptLoader variant="panel" label="Checking application health" />
          </div>
        ) : (
          <>
            {payload?.reason ? (
  ...
  return (
    <section className="ops-block" aria-labelledby="ops-traffic-heading">
      ...
      <BlockBody>
        {block.busy && !payload ? (
          <div className="ops-loading-state">
            <AdaptLoader variant="panel" label="Loading traffic" />
          </div>
        ) : payload?.reason ? (
...
export function StopAllActiveRuns() {
  ...
  const stop = async () => {
    const confirmed = window.confirm('Stop all active ADAPT runs? No data or history is deleted.');
    if (!confirmed) return;
    setBusy(true);
    setError('');
    setResult(null);
  ...
  return (
    <section className="ops-stop-all" aria-labelledby="ops-stop-all-heading">
      ...
      <div className="ops-admin-action-main">
