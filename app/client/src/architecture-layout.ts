 ...
 * same direction, and a card is as tall as the copy inside it. Two things followed, and the checks in this
 ...
 *   - THE BOTTOM OF THE RIGHT COLUMN WAS CUT OFF. A lower card extended beyond
 *     the canvas, and `.arch-canvas-scroll` is `overflow-y: hidden`, so those pixels were not
 ...
 *
 * It was 760, then 832, then 939, then 861, and is 846 now that unused Vector
 * Search cards are gone and the storage row is the bottom of the drawing.
...
export const CANVAS_HEIGHT = 846;
 ...
 * in a column's own vertical gap instead. Two are: "plan + prose" and
 * "resolve + SQL".
 */
 ...
 * counted here for every DEPENDENCY, which is the upper bound. A dependency
 * without a workspace path comes out shorter than this says. Counting a row that may not appear
 * costs a card some air underneath it; not counting one that does costs a title.
 *
 * The role paragraph is the row that actually varies, so its wrapped height is
 * measured rather than estimated once for every card.
 */
export function nodeHeight(id: string): number {
  ...
  const pills = node.presence !== 'connection' ? [PILL_CHARS.status] : [PILL_CHARS.status, PILL_CHARS.drift];
 ...
 * over it. It is also enough to seat the `conversation` and `trace` captions,
 * which are placed inside their columns and are only clear of the cards because
 * they sit in the space between two of them.
 *
 ...
 * the agent itself, a Genie space, governed data, or storage that is written
 * during a run and never read to answer one. Status
 ...
 *
 * The remaining `--db-*` values are the drawing's own relationship vocabulary
 * rather than palette status: teal for a Genie space and grey for storage off
 * the answer path. None is a status rung, so they are not ADAPT token families
 ...
 * below.
 *
 * THE STORAGE ROW IS A ROW AGAIN. Lakebase and the experiment share one top, so
 * the bottom of the canvas reads as a single band rather than as cards that each
 * drifted low by a different amount. Their tops have to be below
...
const NODE_PLACEMENTS: Readonly<Record<string, Omit<NodeBox, 'height'>>> = {
  ...
  'sql-warehouse': { left: 1054, top: 330, width: 198, accent: 'governed' },
};
 ...
 * a check on the anchor point alone cannot see: a middle-anchored caption reaches
 * to both sides of its point, and whether that is clear of the cards beside it is
 * a question about the width of the words rather than about the point.
 */
