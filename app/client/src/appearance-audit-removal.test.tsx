...
describe('removed Appearance contrast audit', () => {
  ...
  it('keeps Entity colors clean before the timezone field in default and custom states', () => {
    for (const initialSettings of [DEFAULT_RUNTIME_SETTINGS, CUSTOM_SAFE_SETTINGS, CUSTOM_LOW_CONTRAST_SETTINGS]) {
      const markup = renderToStaticMarkup(
        <RuntimeSettingsPanel section="appearance" initialSettings={initialSettings} />
      );
      const paletteAt = markup.indexOf('appearance-palette-section');
      const timezoneAt = markup.indexOf('runtime-timezone-field');

      expect(markup).toContain('appearance-palette-section');
      expect(paletteAt).toBeGreaterThan(-1);
      expect(timezoneAt).toBeGreaterThan(paletteAt);
    }
  });
