...
export function foundationCostTile(
  ...
  const storedTokenRuns = runs.filter(
    (run) => typeof run.totalTokens === 'number' && Number.isFinite(run.totalTokens) && run.totalTokens > 0
  );
  const storedInputRuns = storedTokenRuns.filter(
    (run) => typeof run.inputTokens === 'number' && Number.isFinite(run.inputTokens) && run.inputTokens >= 0
  );
  const storedOutputRuns = storedTokenRuns.filter(
    (run) => typeof run.outputTokens === 'number' && Number.isFinite(run.outputTokens) && run.outputTokens >= 0
  );
  const storedTokens =
    runs.length > 0
      ? {
          input:
            storedInputRuns.length > 0 ? storedInputRuns.reduce((sum, run) => sum + (run.inputTokens ?? 0), 0) : null,
          output:
            storedOutputRuns.length > 0
  ...
  return {
    ...
    evidence: {
      ...
      // The completed Ask ledger is the app's authoritative token population.
      // System usage tables are still required to allocate billing dollars, but
      // a delayed or unreadable system table must not erase tokens already
      // persisted on these runs.
      tokens:
        storedTokens ??
        (result
          ? {
