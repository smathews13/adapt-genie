...
export interface ForecastBaseline {
  ...
  fixedDailyCosts: Array<{ id: string; label: string; amount: number | null; unavailable: string }>;
...
export function deriveForecastBaseline(
  ...
  const completeInteractiveRuns =
    marginalInteractive &&
    !cost.perQuestion.limited &&
    cost.perQuestion.runs.length === cost.perQuestion.runsInRange &&
    cost.perQuestion.runs.every((run) => Boolean(run.user && run.completedAt));
  if (completeInteractiveRuns && cost.perQuestion.runs.length > 0) {
    const usersByDay = new Map<string, Set<string>>();
    const questionsByDay = new Map<string, number>();
    for (const run of cost.perQuestion.runs) {
      const runDay = run.completedAt.slice(0, 10);
      if (!inWindow(runDay, cost.range.from, cost.range.to)) continue;
      const runUser = run.user;
      if (!runUser) continue;
      const usersForDay = usersByDay.get(runDay) ?? new Set<string>();
      usersForDay.add(runUser.toLowerCase());
      usersByDay.set(runDay, usersForDay);
      questionsByDay.set(runDay, (questionsByDay.get(runDay) ?? 0) + 1);
    }
    const interactiveUserDays = [...usersByDay.values()].reduce((sum, users) => sum + users.size, 0);
    const interactiveQuestions = [...questionsByDay.values()].reduce((sum, count) => sum + count, 0);
    if (interactiveUserDays > 0) {
      baseline.defaults = normalizeForecastAssumptions({
        ...baseline.defaults,
        averageDailyUsers: interactiveUserDays / days,
        questionsPerUserPerDay: interactiveQuestions / interactiveUserDays,
      });
      baseline.evidence.averageDailyUsers = {
        calculation: `${interactiveUserDays.toLocaleString()} completed-Ask user-days ÷ ${days} complete ${
          days === 1 ? 'day' : 'days'
        }`,
        period,
        range: rangeOf(
          [...usersByDay.values()].map((users) => users.size),
          'daily interactive users'
        ),
      };
      baseline.evidence.questionsPerUserPerDay = {
        calculation: `${interactiveQuestions.toLocaleString()} completed interactive Asks ÷ ${interactiveUserDays.toLocaleString()} user-days`,
        period,
        range: rangeOf(
          [...questionsByDay].map(([day, count]) => count / Math.max(usersByDay.get(day)?.size ?? 0, 1)),
          'daily interactive Asks/user'
        ),
      };
    }
  }
  ...
  const servingTotal = marginalInteractive
    ? unit === 'DBU'
      ? (serving?.marginalDbus ?? null)
      : (serving?.marginalAmount ?? null)
    : totalInWindow(serving, days, unit);
  ...
  baseline.foundationConfigured = Boolean(foundation?.resourceId.trim());
  if (foundation && foundationTotal !== null && completedRuns > 0 && averageTokens !== null) {
    ...
    baseline.foundationUnavailable = '';
    ...
    baseline.exclusions.push({
      ...
      reason: baseline.foundationUnavailable,
    });
  }
  ...
  const sqlQuestions = marginalInteractive ? completedRuns : questionCount;
  if (sqlTotal !== null && sqlQuestions > 0) {
    baseline.observed.sqlCostPerQuestion = sqlTotal / sqlQuestions;
    if (!queryHistoryComplete) {
      pushUnique(baseline.caveats, 'Ask SQL uses the available Query History duration; coverage is partial.');
    }
  } else {
    baseline.exclusions.push({
      component: 'ADAPT SQL',
  ...
  const known = new Set(['serving-endpoint', 'foundation-model', 'sql-warehouse', 'app-compute', 'genie:data']);
  for (const tile of cost.tiles) {
    if (known.has(tile.id) || tile.id.startsWith('genie:')) {
      continue;
    }
    const amount = dailyInWindow(tile, days, unit);
    if (amount !== null) {
      baseline.fixedDailyCosts.push({ id: tile.id, label: tile.label, amount, unavailable: '' });
    } else {
      const reason = tileReason(tile, 'No priced, deployment-attributable daily baseline is available.', unit);
      if (tile.resourceId.trim()) {
        baseline.fixedDailyCosts.push({ id: tile.id, label: tile.label, amount: null, unavailable: reason });
      }
      baseline.exclusions.push({
  ...
  const hasMeasuredRate =
    ...
    baseline.fixedDailyCosts.some((cost) => cost.amount !== null);
...
export function calculateForecast(baseline: ForecastBaseline, assumptions: ForecastAssumptions): ForecastResult {
  ...
  } else if (baseline.foundationConfigured) {
    components.push({
      id: 'foundation-model',
      label: 'Foundation model tokens',
      dailyAmount: null,
      formula: 'requires attributable model usage and list-price coverage',
      unavailable: baseline.foundationUnavailable || 'No priced foundation-model token baseline is available.',
    });
  }
  if (baseline.observed.sqlCostPerQuestion !== null) {
    components.push({
      id: 'sql-warehouse',
      label: baseline.marginalInteractive ? 'Ask SQL' : 'ADAPT SQL',
