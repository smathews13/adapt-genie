 ...
 *
 * ADAPT has no static table fallback: its one Genie workflow supplies the live
 * manifest at model-log time. Later readers may fill a missing app-container
 * declaration from the served model version's baked `model_config`.
 */
...
export function configurationFromRelease(
  ...
  if (!hasManifest) {
    const qualified = qualifyDataContractTables(
 ...
 * Compatibility booleans are not promoted into resource names.
 */
