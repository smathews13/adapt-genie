...
describe('every product resolves to its own published mark', () => {
  ...
  it('carries the two corporate marks the chrome and the gate need', () => {
    ...
    // discovered late: `assets/brand/README.md` says these two exist, the ADAPT
  ...
  it('recolours the official geometry rather than redrawing it', () => {
    // THE ONE MECHANICAL GUARD THE RULING HAS. Sam Mathews settled on 2026-08-17
    // that recolouring a product mark into the ADAPT palette is permitted and
  ...
  it('paints the recoloured cuts out of the ADAPT palette and nothing else', () => {
...
describe('the mark is decorative, because the name is already beside it', () => {
  it('hides itself from a screen reader by default', () => {
    ...
    // what the ADAPT mark already does, for the same reason.
...
describe('the stylesheet preserves geometry and applies ADAPT ink', () => {
  ...
  it('maps inherited product blues to the ADAPT teal and turquoise tokens', () => {
    const css = partial('brand.css').replace(/\/\*[\s\S]*?\*\//g, ' ');
    expect(css).toMatch(/\[fill='#2272B4'\]\s*\{[^}]*fill:\s*var\(--db-blue-600\)/);
    expect(css).toMatch(/\[fill='#6FAEDD'\]\s*\{[^}]*fill:\s*var\(--ast-blue-on-dark\)/);
    expect(css).toMatch(/\[fill='#B7D6EE'\]\s*\{[^}]*fill:\s*var\(--ast-ice-accent\)/);
    expect(css).not.toMatch(/(?:^|[;{\s])(?:stroke|background)\s*:/);
  });
});
...
describe('one map, and it is this one', () => {
  ...
  it('files each tool under one product, and leaves an unknown tool alone', () => {
    ...
    expect(productForTool('search_tagged_assets')).toBe('mosaic-ai');
