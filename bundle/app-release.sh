...
while [[ $# -gt 0 ]]; do
  ...
  --rollback-to /Workspace/Users/you@corp.com/adapt-genie-src
...
Dry run. Nothing was built or deployed. Re-run with --apply to:
  ...
  7. verify the active deployment runs from Databricks' separate SNAPSHOT path,
     then safety-check that $SRC_PATH is adapt-genie's staging directory directly
     below /Workspace/Users/<current actor>/ with basename adapt-genie-src
...
[[ -f "$RELEASE_GATE" ]] || die "bundle/release-gate.sh is missing.
A missing customer release gate is not a pass and there is no flag past it."
...
# Postgres schema the app owns inside Lakebase. Default adapt; the
...
DATA_GENIE_ID="$(bundle_var_or_empty genie_data_space_id)"
WATCHLIST_TABLE="$(bundle_var_or_empty watchlist_table)"
note "foundation model     ${LLM_ENDPOINT:-(app default)}"
note "data genie space     ${DATA_GENIE_ID:-(unset)}"
note "watchlist table      ${WATCHLIST_TABLE:-(unset; rail reports unavailable)}"
...
import json,sys
app=json.load(sys.stdin).get("resources",{}).get("apps",{}).get("adapt_app",{})
...
#
# No count is given here on purpose. The number is a function of a target's
# variable and goes stale every time a scope is added. `bundle summary -t
# <target>` prints the resolved list, and bundle/scope-contract.json carries it
# per target, generated.
#
...
(cd "$APP_DIR" \
  && PLAYER_INSIGHTS_TARGET="$TARGET" \
     ...
     PLAYER_INSIGHTS_WATCHLIST_TABLE="$WATCHLIST_TABLE" \
     ...
     ADAPT_ADMIN_GROUP="$ADAPT_ADMIN_GROUP" \
...
# three counts across two files that became stale as optional scopes changed.
# bundle/scope-contract.json is generated from databricks.yml,
...
SCOPE_CONTRACT="$BUNDLE_ROOT/bundle/scope-contract.py"
SCOPE_CONTRACT_JSON="$BUNDLE_ROOT/bundle/scope-contract.json"
[[ -f "$SCOPE_CONTRACT" && -f "$SCOPE_CONTRACT_JSON" ]] || die "The generated scope contract is incomplete.
Both bundle/scope-contract.py and bundle/scope-contract.json are required."
CONTRACT_APP_JSON="$(mktemp "${TMPDIR:-/tmp}/adapt-app-json.XXXXXX")"
printf '%s' "$APP_JSON" > "$CONTRACT_APP_JSON"
CONTRACT_STATUS=0
python3 "$SCOPE_CONTRACT" --check --live "$CONTRACT_APP_JSON" || CONTRACT_STATUS=$?
...
restart above, not a contract problem."
  ...
  die "The scope contract could not run (exit $CONTRACT_STATUS).
An unanswered release check is not a pass. Read the COULD NOT RUN line above."
fi
