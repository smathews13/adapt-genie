...
describe('environment tabs', () => {
  ...
  it('keeps hover, focus, active, and forced-color states distinct in ADAPT blue', () => {
    expect(STYLES).toMatch(/\.environment-tabs button:hover:not\(\.active\)\s*\{[^}]*color:\s*var\(--foreground\)/);
    expect(STYLES).toMatch(
      /\.environment-tabs button:focus-visible\s*\{[^}]*outline:\s*2px solid var\(--ring\)[^}]*color:\s*var\(--foreground\)/
    );
    expect(STYLES).toMatch(/\.environment-tabs button\.active\s*\{[^}]*color:\s*var\(--ast-blue\)/);
    expect(STYLES).toMatch(
      /@media \(forced-colors:\s*active\)[\s\S]*\.environment-tabs button\.active::after\s*\{[^}]*background:\s*Highlight/
    );
  });
});
