...
describe('a session short of a permission the app declares', () => {
   ...
   * Uses a required ask-path scope so the fixture exercises the notice.
   */
  ...
  it('ignores optional browse shortfalls for the sign-in remedy', () => {
    expect(
      staleSignInNotice({
        ...STALE,
        missingScopes: ['catalog.catalogs:read', 'workspace.workspace:read'],
      })
    ).toBeNull();
  });
});
...
describe('the states that must not produce a sign-in prompt', () => {
  ...
  it('refuses to prompt on a stale verdict with nothing signed in behind it', () => {
    expect(staleSignInNotice({ ...CURRENT, state: 'stale', signedIn: false, missingScopes: ['sql'] })).toBeNull();
