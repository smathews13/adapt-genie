...
def test_table_tags_are_paged_filtered_to_the_manifest_and_baked_as_keyed_pairs():
    first = SimpleNamespace(
        status=SimpleNamespace(state=StatementState.SUCCEEDED),
        statement_id="statement-1",
        result=SimpleNamespace(
            data_array=[
                ["test_catalog", "test_schema", "sales", "Franchise", "GTA"],
                ["other", "schema", "hidden", "franchise", "Other"],
            ],
            next_chunk_index=1,
        ),
    )
    second = SimpleNamespace(
        data_array=[["test_catalog", "test_schema", "players", "franchise", "NBA"]],
        next_chunk_index=None,
    )
    statements: list[tuple[str, str, str]] = []
    workspace = SimpleNamespace(
        statement_execution=SimpleNamespace(
            execute_statement=lambda statement, warehouse_id, wait_timeout: (
                statements.append((statement, warehouse_id, wait_timeout)) or first
            ),
            get_statement_result_chunk_n=lambda statement_id, chunk_index: second,
        )
    )
    configured = settings(
        declared_manifest=(
            "test_catalog.test_schema.sales",
            "test_catalog.test_schema.players",
        )
    )

    pairs, notes = resolve_table_tags(configured, workspace)

    assert pairs == (
        "test_catalog.test_schema.players=franchise=NBA",
        "test_catalog.test_schema.sales=franchise=GTA",
    )
    assert notes == ["Table tags: 2 of 2 declared table(s) tagged."]
    assert statements[0][1:] == ("wh-123", "50s")
    assert "information_schema.table_tags" in statements[0][0]
...
#: A stand-in set of curated tables for the fixtures below. The production
#: deployment declares no static data contract (preflight.DECLARED_TABLES is
#: empty by design), so tests that need "some curated tables" name their own
#: sample here rather than importing an empty tuple.
DECLARED_TABLES = (
...
def test_genie_mode_declares_a_customers_own_tables_without_asking_for_ours():
    ...
    workspace = FakeGenie({"space-data": list(THEIRS)})
    ...
    # The single data space is read, and our vocabulary never comes up.
    assert workspace.asked == ["space-data"]
...
def test_genie_mode_cannot_silently_declare_nothing():
    ...
    cases = (
        genie_settings(data_genie_space_id=""),
        genie_settings(),
    )
...
def test_a_space_that_curates_nothing_stops_the_release():
    ...
    with pytest.raises(ScopeError) as raised:
        resolve_declared_manifest(genie_settings(), FakeGenie({"space-data": []}))

    message = str(raised.value)
    assert "data" in message and "space-data" in message
...
def test_the_dry_run_says_that_in_prose_rather_than_raising_a_traceback(monkeypatch, capsys):
    ...
    for key in ("data_genie_space_id",):
...
def test_a_curated_name_that_is_not_three_part_is_dropped_rather_than_guessed_at():
    ...
    manifest, _ = resolve_declared_manifest(genie_settings(), workspace)
...
def test_the_denylist_still_vetoes_a_curated_table():
    ...
    manifest, notes = resolve_declared_manifest(
        genie_settings(catalog_denylist=("*.fact_transaction",)),
...
def test_everything_curated_being_excluded_is_a_refusal_not_an_empty_manifest():
    with pytest.raises(ScopeError) as raised:
        resolve_declared_manifest(
            genie_settings(catalog_denylist=("*",)),
            FakeGenie({"space-data": list(THEIRS)}),
        )
...
def test_an_app_catalogs_unwritten_schemas_do_not_stop_a_release():
    ...
    workspace = FakeCatalog(
        {
            "app_catalog.adapt_data": ["gold_player_180d_summary"],
            "app_catalog.adapt_telemetry": [],
            "app_catalog.adapt_app_assets": [],
        }
    )

    manifest, _ = resolve_declared_manifest(settings(catalog_allowlist=("app_catalog",)), workspace)

    assert manifest == ("app_catalog.adapt_data.gold_player_180d_summary",)
...
def test_the_payload_table_is_recognised_by_shape_not_by_our_name_for_it():
    ...
    Matching our own endpoint's literal payload-table name would exclude nothing
    on their workspace, which is where this defect actually costs something:
    their users' questions, in a table the agent can read, on their first deploy.
    """
...
def test_log_model_declares_and_bakes_the_same_manifest():
    ...
    assert "table_tags, table_tag_notes = resolve_table_tags(" in source
    ...
    assert "table_tags=table_tags" in source
...
def test_a_model_logged_before_the_manifest_existed_still_loads():
    ...
    narrower than that version's real reach, never wider. This deployment
    declares no static data contract, so the fallback is empty rather than a set
    of demo names -- still a load rather than a crash.
    """

    served = settings(declared_manifest=())

    assert served.readable_tables == ()
...
def test_git_beats_a_stale_environment_variable():
    ...
    A compatibility build-stamp value left over in some shell from an earlier
    release would otherwise stamp this artifact with a commit it was not built
    from, which is the failure the stamp exists to catch, reintroduced by the
    stamp.
    """
