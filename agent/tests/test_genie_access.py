...
def test_format_genie_space_joins_title_and_id():
    ...
    assert format_genie_space(SPACE, "ADAPT Data") == f"ADAPT Data ({SPACE})"
...
def test_a_refused_genie_space_names_its_baked_title_in_the_caveat():
    ...
    tools.settings = agent_settings(data_genie_space_title="ADAPT Data")
    llm = ScriptedLlm(
...
def test_a_refusal_is_not_reported_as_a_governance_control_firing():
    ...
    A governance refusal is the product working, and the ADAPT demo highlights
    that behavior. Crediting a control that never fired for a setup step nobody
