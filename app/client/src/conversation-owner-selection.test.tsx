...
const ME = 'sam.mathews@example.com';
...
describe('owner selection semantics', () => {
  ...
  it('normalizes, deduplicates, drops stale owners, and caps forged state', () => {
    ...
    expect(normalizeOwnerSelection([' SAM.MATHEWS@EXAMPLE.COM ', ME, 'gone@example.com'], available)).toEqual([ME]);
...
describe('the admin owner dropdown', () => {
  ...
  it('is admin-only and defaults to All users', () => {
    ...
    expect(HOME).toContain('const [ownerFilters, setOwnerFilters] = useState<readonly string[]>([])');
    expect(HOME).toContain('const [organizationFilters, setOrganizationFilters] = useState<readonly string[]>([])');
    expect(HOME).toContain('<ConversationFilters');
    expect(HOME).toContain('organizations={railOrganizations}');
    expect(HOME).toContain('organizationOptionsForEmails(emails, emails, identity.organizations)');
    expect(HOME).toContain('organizationForEmail(entry.owner, identity.organizations)');
  });
