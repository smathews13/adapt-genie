...
describe('firstOpenReport', () => {
  ...
  it('never reports zero required scopes for a normal ADAPT deployment', () => {
    const report = firstOpenReport(identity());
    ...
    expect(optionalScopeRows(report.scopes).map((scope) => scope.name)).toEqual([...ADAPT_OPTIONAL_USER_API_SCOPES]);
  });
  ...
  it('lists declared scopes first, then optional catalog scopes not on the deploy', () => {
    ...
    expect(report.scopes.map((s) => s.name)).toEqual([...declared, ...ADAPT_OPTIONAL_USER_API_SCOPES]);
  });
  ...
  it('marks undeclared optional scopes granted when the effective token carries them', () => {
    const report = firstOpenReport(
      identity({
        session: session({
          tokenScopes: [...DECLARED, 'unity-catalog', 'workspace', 'postgres'],
        }),
      })
    );

    expect(optionalScopeRows(report.scopes).map((scope) => scope.status)).toEqual(
      ADAPT_OPTIONAL_USER_API_SCOPES.map(() => 'granted')
    );
  });
  ...
  it('marks undeclared optional scopes not requested when the effective token does not carry them', () => {
    ...
    expect(optionalScopeRows(report.scopes).map((scope) => scope.status)).toEqual(
      ADAPT_OPTIONAL_USER_API_SCOPES.map(() => 'not_declared')
    );
  });
...
describe('scopeRows', () => {
  it('reports nothing as granted when the check did not run', () => {
    expect(scopeRows(['sql', 'dashboards.genie'], [], false).every((r) => r.status === 'unchecked')).toBe(true);
  });

  it('still lists optional catalog scopes when nothing was declared', () => {
    const rows = scopeRows(null, [], true);
    expect(rows.map((r) => r.name)).toEqual([...ADAPT_OPTIONAL_USER_API_SCOPES]);
