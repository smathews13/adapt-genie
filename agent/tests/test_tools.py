...
class FakeWarehouse:
    ...
    def _execute(
        ...
        self.query_tags.append(query_tags)
...
def test_ask_sql_is_tagged_for_cost_attribution():
    warehouse = FakeWarehouse(["n"], [["1"]])
    tools = build(warehouse, manifest=MANIFEST)
    tools.run_sql(f"SELECT count(*) AS n FROM {ACTIVITY}")

    tags = {tag.key: tag.value for tag in warehouse.query_tags[0]}
    assert tags["application"] == "ADAPT"
    assert tags["surface"] == "ask"
    assert tags["tool"] == "run_sql"
...
def test_the_cross_label_key_is_refused_in_a_projection():
    ...
    reason = refused(
        f"SELECT crm_customer_ref, partner_player_ref, email FROM {PROFILES} LIMIT 50",
        "crm_customer_ref",
    )
...
def test_the_key_is_refused_everywhere_not_only_where_it_is_returned():
    ...
    refused(
        f"SELECT count(*) FROM {PROFILES} WHERE crm_customer_ref IS NOT NULL",
        "not filtered on",
    )
...
def test_the_genie_budget_fits_inside_the_turn():
    ...
    assert tools_module.GENIE_TIMEOUT_SECONDS * 2 <= 180.0
...
def test_a_starting_warehouse_returns_a_skippable_result_instead_of_failing_the_step(monkeypatch):
    ...
    result = build(genie).data_genie("what does active player mean")
...
def test_the_data_genie_tool_description_names_the_space_when_titled():
    from tools import data_genie_tool

    data = data_genie_tool("ADAPT Data")["function"]["description"]
    assert '"ADAPT Data"' in data
...
def test_a_text_only_answer_fetches_no_rows():
    ...
    result = build(genie).data_genie("what is an active player")
...
def test_ranking_players_is_refused_through_genie_exactly_as_it_is_through_sql():
    ...
    "Top spenders" is a plausible thing to ask this analytical agent, and the
