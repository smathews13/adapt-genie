...
const payload: RosterPayload = {
  entries: [
    {
      email: 'fictional.deployer@databricks.com',
      isDeploymentOwner: true,
      role: 'super_admin',
      seedFloor: 'super_admin',
      setBy: 'fictional.deployer@databricks.com',
...
function roster(busy = false) {
  return renderToStaticMarkup(<RosterRows payload={payload} busy={busy} onChange={() => {}} onRemove={() => {}} />);
}
...
describe('Identity table polish', () => {
  it('reserves control columns and owns overflow at the screenshot and narrower desktop widths', () => {
    ...
    // platform scrollbar; the table keeps explicit room for identity metadata. At 900px the table
  ...
  it('keeps immutable Super admin and Owner badges complete and separated by columns', () => {
    ...
    expect(markup).not.toContain('aria-label="Remove fictional.deployer@databricks.com"');
    expect(CSS).toMatch(
  ...
  it('renders the official Databricks mark and a derived domain mark without hiding either full email', () => {
    ...
    for (const entry of payload.entries) {
      ...
      expect(markup).not.toContain(`aria-label="Copy email ${entry.email}"`);
    }
  });
  ...
  it('aligns the Add row to the same columns and preserves disabled and focusable control states', () => {
    ...
    // Email, Organization, Role and Actions — the "Set by" column was removed.
    expect(enabled.match(/<td/g) ?? []).toHaveLength(4);
    ...
    expect(enabled).toContain('>Add<');
    expect(enabled).toContain('adapt-button-state');
