 ...
 * the object it does name. Genie links only a space id, never a workspace id.
 */
...
export function costTilesForDisplay(tiles: readonly CostTile[]): CostTile[] {
  ...
  const visibleIds = new Set(EMPTY_COST_TILES.map((tile) => tile.id));
  return tiles.filter((tile) => visibleIds.has(tile.id));
}
...
const EMPTY_COST_TILES: readonly CostTile[] = [
  ...
  { ...EMPTY_COST_TILE, id: 'lakebase', label: 'Lakebase', resourceKind: '' },
