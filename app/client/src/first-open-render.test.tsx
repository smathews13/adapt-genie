...
import {
  ADAPT_OPTIONAL_USER_API_SCOPES,
  DISCLAIMER_BODY,
  FIRST_OPEN_KEY,
 ...
 * any count.
 */
const OPTIONAL_ROWS = ADAPT_OPTIONAL_USER_API_SCOPES.length;
...
describe('all required scopes granted', () => {
  ...
  it('names the app and the address the questions will run under', () => {
    ...
    expect(markup).not.toContain('Legacy product name');
  ...
  it('lists every declared scope with a Granted pill and none missing', () => {
    ...
    expect(markup.match(/fo-scope-request/g)).toHaveLength(OPTIONAL_ROWS);
    ...
    expect(markup).not.toContain(
      'Questions still work without these; they unlock Connections browsing (catalogs, tables, and notebooks) and Lakebase, and a deployment can omit any of them.'
    );
...
/*
 * THE SCREEN THAT PROMPTED THIS. The cmeg deployment declares the three catalog
...
describe('once per session', () => {
  ...
  it('keeps ADAPT startup static and free of obsolete constellation artwork', () => {
    const opening = firstPaint();
    expect(opening).not.toContain('data-star-motion-field');
    expect(opening).not.toContain('constellation-field');
    expect(opening).not.toContain('star-field');
  });
