...
identity_population AS (
  ...
  FROM (
    ...
    FROM jsonb_to_recordset($14::jsonb) AS supplied(email text, role text)
    WHERE email <> ''
    ORDER BY lower(email)
  ) roster
  WHERE $13::boolean
    AND ($5::boolean OR lower(roster.email) = lower($6))
  UNION ALL
...
identity_revision AS (
  SELECT (SELECT MAX(added_at) FROM ${ADMIN_EMAILS_TABLE}) AS revision
),
filtered AS (
  ...
  WHERE ($7 = '' OR identity_population.display_email LIKE ('%' || lower($7) || '%'))
    ...
    AND (
      ...
      OR EXISTS (
        SELECT 1
        FROM unnest($9::text[]) AS organization_suffix
...
identity_revision AS (
  SELECT (SELECT MAX(added_at) FROM ${appTable("admin_emails")}) AS revision
),
filtered AS (
  SELECT identity_population.user_key, identity_population.display_email,
         COALESCE(aggregated.submitted_questions, 0) AS submitted_questions,
