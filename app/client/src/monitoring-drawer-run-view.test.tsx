...
/**
 * Opening a Monitoring question is a centered modal whose body is ADAPT Ask's
 * own `AnswerCard`.
 *
...
describe("the modal draws one run view, the card's own", () => {
  ...
  it('keeps the modal chrome around the shared card', () => {
    ...
    expect(markup).toContain('aria-label="Open user overview for User first.person@example.test');
    expect(rendered.indexOf('first.person')).toBeLessThan(rendered.indexOf('Run process'));
  ...
  it('stacks the token line inside the card, after the tables and before Sources', () => {
    ...
    const markup = drawer({
      answer: {
        ...
        content: ['| Franchise | Players |', '| --- | ---: |', '| GTA | 10 |'].join('\n'),
        sources: [
...
describe('the answer card still draws the run view where it is the only one', () => {
  ...
  /**
   * ADAPT Ask's transcript passes no `showRunProcess` at all, so this is the
