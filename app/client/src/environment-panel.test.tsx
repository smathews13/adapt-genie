...
const INFO: EnvironmentInfo = {
  ...
  variables: [
    { key: 'DATABRICKS_APP_NAME', value: 'adapt' },
...
describe('Environment panel', () => {
  ...
  it('filters either column without changing the source list', () => {
    expect(filterEnvironmentItems(INFO.variables, 'adapt')).toEqual([{ key: 'DATABRICKS_APP_NAME', value: 'adapt' }]);
  ...
  it('does not offer the ADAPT tag repair on this pane', () => {
    const markup = renderToStaticMarkup(<EnvironmentPanel initialData={INFO} />);
    expect(markup).not.toContain('Resource tags');
    expect(markup).not.toContain('system_billing=adapt');
