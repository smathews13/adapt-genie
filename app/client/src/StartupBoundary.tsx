...
export function StartupLoadingSurface({ phase }: { phase: StartupPhase }) {
  ...
  return (
    <main
      ...
      data-startup-loader="adapt-primary"
      aria-busy="true"
      aria-label={label}
    >
...
/** Give the ADAPT mark time to resolve instead of flashing into the login card. */
...
export function StartupBoundary({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!loginPathActive || loginDwellComplete) return;
