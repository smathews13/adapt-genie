...
describe('canonical signed-in identity', () => {
  it.each([
    ['employee@databricks.com', 'employee@databricks.com', 'employee'],
  ...
  it('keeps ambiguous and absent local parts unbranded instead of guessing a domain', () => {
    expect(
      resolveCanonicalUserIdentity('shared.user', [
        row('shared.user@databricks.com'),
        row('shared.user@studio2games.example'),
      ]).canonicalEmail
    ).toBeNull();
    expect(
      resolveCanonicalUserIdentity('missing.user', [row('someone.else@databricks.com')]).canonicalEmail
    ).toBeNull();
  });

  it('does not let roster data replace a complete proxy-authenticated address', () => {
    expect(
      resolveCanonicalUserIdentity('employee@databricks.com', [row('employee@studio2games.example')]).canonicalEmail
    ).toBe('employee@databricks.com');
  });
