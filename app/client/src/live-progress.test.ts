...
/**
 * ADAPT Ask, which seats this panel twice: the splash animation and the smaller
...
describe('describeStage', () => {
  ...
  it('says what a listing covered without inventing a scope it was not given', () => {
    ...
    expect(describeStage(stage({ id: 'step-1-1-list_data_assets', kind: 'tool', input: '{"catalog": "cmeg"}' }))).toBe(
      'Listed the tables it may read under catalog: cmeg'
    );
  });
  ...
  it('maps the historical analysis envelope to neutral live copy', () => {
    ...
    expect(live.name).toBe('Orchestrator data analysis');
    expect(live.detail).toBe(ORCHESTRATOR_ANALYSIS_TASK);
...
describe('the live timeline follows the newest action inside its card', () => {
  it('owns one bounded scroller without moving page ancestors', () => {
...
describe('a step announced before it finishes', () => {
  ...
  it('names the newest unfinished step rather than the envelope holding it', () => {
     ...
     *
     * A run announces its envelopes before it does anything -- the Orchestrator,
     * then its analysis envelope -- and reports neither until it is over, so the
    ...
    for (const [id, name] of [
      ...
      [LEGACY_ANALYSIS_STAGE_ID, 'Orchestrator data analysis'],
