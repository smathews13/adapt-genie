...
# Clean slate: this deployment declares no static data contract. In
# `manifest_source=genie` the manifest is derived entirely from the live Genie
# space's curated tables (see `_manifest_from_genie`), and `data_catalogs` /
# `scope_allows` remain the enforced read boundary. Empty tuples mean there is
# no contract to union in and no GOVERNANCE note to emit.
DATA_GENIE_TABLES: tuple[str, ...] = ()
DICTIONARY_GENIE_TABLES: tuple[str, ...] = ()
...
def resolve_build_stamp(
    ...
    GIT FIRST, environment second: a stale compatibility build-stamp variable
    left in a shell would otherwise stamp the artifact with a commit it was not
    built from. Returns the empty string rather than ``"unknown"``, so no reader
    can mistake absence for a match.
    """
...
def declared_tables(settings: Settings) -> list[str]:
    ...
    # The compatibility table-list variable is human-edited, and a fully
    # qualified entry is the obvious mistake to make in it: qualifying one again
    # yields a five-part name in the GRANT, which is unusable rather than merely
    # wrong.
    return [
...
def _manifest_from_genie(
    ...
    if not configured:
        raise ScopeError(
            "manifest_source=genie needs the data_genie_space_id to be set. The mode "
            "declares what the space curates, so with no space there is nothing to "
            "declare and the endpoint could read nothing. Set the id, or use "
