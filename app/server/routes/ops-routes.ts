...
/**
 * The Genie spaces Connections already lists.
 *
...
export async function costIdentifiersFor(
  ...
  return {
    ...
    ids: {
      ...
      lakebaseEndpoint: (process.env.LAKEBASE_ENDPOINT ?? '').trim(),
      genieSpaces: [
        {
          id: dataGenie?.id || '',
          label: 'Data Genie',
 ...
 * matters here is a lost grant on the compatibility schema, and a connection-level check
...
/**
 * What the platform says about itself, which is not what ADAPT probed.
 *
...
/**
 * Resource-scoped request counts from ADAPT's persisted traces.
 *
...
export const RESOURCE_ACTIVITY_QUERY = `
  ...
  observed AS (
    ...
    WHERE stage->>'id' ~ '(^|-)(data_genie|dictionary_genie)$'
    GROUP BY 1
  ),
  attributed AS (
  ...
  SELECT c.tile_id,
         (COALESCE(a.calls, 0) + COALESCE(l.calls, 0))::bigint AS attributed_calls,
         GREATEST(
           COALESCE(o.calls, 0),
...
async function readLifetimeSpendSnapshot(input: {
  ...
  const foundation = foundationOutcome.ok
    ? foundationCostTile(input.ids, readFoundationBillingRows(foundationOutcome.rows), '', runs)
    : foundationCostTile(input.ids, null, foundationOutcome.message, runs);
  ...
  appkit.server.extend((app: Application) => {
    ...
    app.get('/api/ops/cost', async (req: Request, res: Response) => {
      ...
      try {
        ...
        const foundation = foundationOutcome.ok
          ? foundationCostTile(ids, readFoundationBillingRows(foundationOutcome.rows), '', questionRunsRead.runs)
          : foundationCostTile(ids, null, foundationOutcome.message, questionRunsRead.runs);
        const genieRows = genieOutcome.ok ? readGenieAccountingRows(genieOutcome.rows) : [];
        ...
        if (split.components.length === 0 && (!split.meta || split.meta.billedDays === 0)) {
          ...
          const reason = unpropagated.length
            ? 'Matching usage exists without the ADAPT tag, but exact resource attribution remains available.'
            : delayed
        ...
        const spendByUser =
          ...
          buildSpendByUser({
            ...
            queryComplete: !spendWindow.partial && warehouseQueryAttributionUsable(queryAttribution),
