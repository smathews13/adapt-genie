...
/**
 * ADAPT's immutable operator floor.
 *
 * Unlike deployment seed values, this remains authoritative after Lakebase has
 * a roster so the named operator cannot be locked out by a stale or incomplete
 * roster. It grants only ADAPT application administration; analytical queries
 * still run with the caller's own Databricks permissions.
 */
export const ADAPT_SUPER_ADMIN_EMAILS = [
...
export type { GroupRoleLookup };

/** Resolve immutable and Lakebase-stored workspace-group mappings through SCIM. */
...
export function seedAdminEmails(): readonly string[] {
  return [...new Set([...ADAPT_SUPER_ADMIN_EMAILS, ...seedAdmins])];
}
...
export function seedSuperAdminEmails(): readonly string[] {
  return [...new Set([...ADAPT_SUPER_ADMIN_EMAILS, ...seedSuperAdmins])];
}
  ...
  try {
    ...
    console.error(
      `[admin] ROLE BOOTSTRAP SKIPPED: the Lakebase roster could not be read (code ${code}: ` +
        `${(error as Error).message}). ADAPT will keep serving with no stored roles available; ` +
...
async function resolveRoleFrom(
  ...
  const immutableFloor = seedFloorFor(seed, caller);
  if (immutableFloor === 'super_admin') {
    return { role: 'super_admin', addedAdminsReadable: true, seedAdminCount: seed.admins.length };
  }
  const groupRole = await readGroupRole(caller).catch(() => null);
  const floor = highestRole(immutableFloor, groupRole ?? 'consumer');
  if (floor === 'super_admin') {
    return { role: 'super_admin', addedAdminsReadable: true, seedAdminCount: seed.admins.length };
  }
  try {
    const { rows } = await read();
    const storedRole = caller ? effectiveRole({ seed, stored: rows, email: caller }) : 'consumer';
    const role = highestRole(floor, storedRole);
...
export async function rolePayload(
  store: AdminStore,
  email: string,
  readGroupRole: GroupRoleLookup = groupRoleLookupForStore(store)
): Promise<RolePayload> {
  const { role, addedAdminsReadable, seedAdminCount } = await resolveRoleFrom(
    email,
    () => readRoster(store),
    readGroupRole
  );
...
export function requireAdmin(
  ...
  return function refuseNonAdmins(req: Request, res: Response, next: NextFunction) {
    ...
    resolveRoleForRequest(store, req, () => caller, readGroupRole)
      .then((resolution) => {
