...
export const LATER_MIGRATIONS: readonly Migration[] = [
  {
    ...
    statements: [
       ...
       * and other governed tool calls and the MLflow trace. See
  ...
  },
  {
     ...
     * table, or a secondary endpoint from a configured resource. Keep legacy rows valid
  ...
  },
  {
    ...
    statements: [
      ...
      `CREATE INDEX IF NOT EXISTS saved_queries_user_idx
         ON ${APP_SCHEMA}.saved_queries (user_email, created_at DESC)`,
    ],
    down: [
      `DROP INDEX IF EXISTS ${APP_SCHEMA}.saved_queries_user_idx`,
      `DROP TABLE IF EXISTS ${APP_SCHEMA}.saved_queries`,
    ],
  },
  {
    version: 42,
    name: 'repair run label overrides',
    /**
     * Some ADAPT databases were initialized from a version ledger that already
     * recorded v10 without carrying its table. Replaying that idempotent DDL
     * under a new version repairs those deployments without rewriting history.
     */
    statements: [
      `CREATE TABLE IF NOT EXISTS ${APP_SCHEMA}.run_label_overrides (
         run_id TEXT PRIMARY KEY,
  ...
  {
    version: 43,
    name: 'governed watchlist settings',
    statements: [WATCHLIST_SETTINGS_DDL],
    down: [`DROP TABLE IF EXISTS ${WATCHLIST_SETTINGS_TABLE}`],
  },
  {
    version: 44,
