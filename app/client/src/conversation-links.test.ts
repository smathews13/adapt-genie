 ...
 * `/?conversation=<id>`. ADAPT Ask reads `?c=`. Both halves were written from a
 ...
 *
 * `conversation` is a real parameter in the OTHER direction -- ADAPT Ask to Run
...
describe('the link from a run back to its conversation', () => {
  it('names the conversation under the parameter the ADAPT Ask screen reads', () => {
...
describe('the pages at each end of that link', () => {
  ...
  it('leaves no hand-built link to ADAPT Ask in the run explorer', () => {
    // The whole defect, as a string. `/?conversation=` is the spelling ADAPT Ask
    // has never read.
  ...
  it('reads both halves of the link on the ADAPT Ask side from the same definitions', () => {
...
describe('the way back into the Run Explorer', () => {
  it('carries the open conversation from ADAPT Ask into the run list', () => {
    expect(LAYOUT).toContain("searchParams.get('c')");
    expect(LAYOUT).toMatch(/\/runs\?conversation=\$\{encodeURIComponent\(openConversation\)\}/);
  });

  it('only reads that conversation while ADAPT Ask is the page on screen', () => {
