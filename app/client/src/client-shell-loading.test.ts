...
describe('the client shell import graph', () => {
  ...
  it('loads the Settings body only after the gear or deep link asks for it', () => {
    ...
    expect(layout).toContain('<Suspense fallback={<SettingsModalFallback />}>');
    expect(source('SettingsModalFallback.tsx')).toContain('data-testid="settings-loading"');
  });
  ...
  it('keeps route-only Architecture data behind its boundary without dropping shell motion', () => {
    ...
    expect(source('HomePage.tsx')).not.toContain("from './benchmark-summary'");
    expect(source('HomePage.tsx')).not.toContain("from './benchmark-format'");
  ...
  it('styles the Settings loading seat before its lazy CSS resolves', () => {
    const fallback = source('SettingsModalFallback.tsx');
    expect(fallback).toContain('settings-overlay fixed inset-0 z-50 grid place-items-center');
    expect(source('styles/adapt-loading.css')).toContain('.settings-loading-seat');
    expect(fallback).toContain('rounded-lg border bg-background');
  });
