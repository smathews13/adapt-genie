 ...
 *   3. `--ast-pane`, which is what the answer card and the question bubble are
 *      drawn on, is 94% white -- so whatever was behind them showed through, and
 *      in light mode what was behind them was (2). (The composer was on this pane
 *      too until it became a docked footer; it is solid `--background` now, since
 *      a footer sits over scrolling prose rather than sky.)
 *
...
const inFlight: TraceStage[] = [
  ...
  stage({ id: 'step-1-1-search_tagged_assets', name: 'Searched governed assets', kind: 'tool', duration: 604 }),
...
describe('the night sky belongs to dark mode alone', () => {
  it('has removed the app-wide starfield, in daylight and in dark', () => {
    /*
     * The stars theme is gone: no `<AppSky>` layer mounts, no StarField source
     * remains, and there are no `.app-sky` display rules gating a layer that no
     * longer exists. What is left is a flat sky the surfaces paint themselves.
     */
    expect(source('Layout.tsx')).not.toContain('<AppSky');
    const baseCode = BASE.replace(/\/\*[\s\S]*?\*\//g, ' ');
    const darkCode = DARK.replace(/\/\*[\s\S]*?\*\//g, ' ');
    expect(baseCode).not.toMatch(/\.app-sky\s*[,{]/);
    expect(darkCode).not.toMatch(/\.app-sky\s*[,{]/);
  });

  it('paints neither navy nor spackle on the two surfaces that carry the sky in dark', () => {
    /*
     * The base rules paint a FLAT navy on both surfaces in dark (the starfield
     * spackle they used to carry is gone with the theme). What is asserted here is
     * the light half: each surface repaints to its daylight fill and references no
     * dark-only token.
     */
    expect(rule(RAIL, '.ask-layout')).not.toMatch(/--ast-sky-spackle/);
    expect(rule(RAIL, '.trace-inspector')).not.toMatch(/--ast-sky-spackle/);
