...
const span = (id: string, kind: string, start: number, duration: number, parent_id: string, name = id): TraceStage => ({
  id,
  kind,
  start,
  duration,
...
/**
 * The shape of a historical run: one analysis span with the model turns and the tool
 ...
 * stage that reads as data work gives 160.0s, because the analysis span is
...
const NESTED: TraceStage[] = [
  ...
  span(LEGACY_ANALYSIS_STAGE_ID, 'agent', 120, 96_000, 'orchestrator', 'Orchestrator data analysis'),
  span('step-1', 'agent', 200, 40_000, LEGACY_ANALYSIS_STAGE_ID, 'Choosing the next step'),
  ...
  span('step-2', 'agent', 41_000, 50_000, LEGACY_ANALYSIS_STAGE_ID, 'Choosing the next step'),
...
describe('Run Explorer feedback', () => {
  it('sums tool, historical analysis, and SQL durations instead of showing 0.0s', () => {
    expect(
      toolStageDurationMs([
        stage('orchestrator', 'agent', 10_000, 'Orchestrator'),
        stage(LEGACY_ANALYSIS_STAGE_ID, 'agent', 2_000, 'Orchestrator data analysis'),
  ...
  it('counts a nested tool call once, not once per stage that contains it', () => {
    ...
    // analysis envelope's whole span plus a second copy of the calls inside it, and lands
    // above the wall clock it is read against.
    const naive = NESTED.filter(
      (item) =>
        item.kind === 'tool' || item.id === LEGACY_ANALYSIS_STAGE_ID || /\bsql\b/i.test(`${item.id} ${item.name}`)
  ...
  it('never reports more data work than the run it happened inside', () => {
    ...
    // that happened.
    const unplaced = [stage('step-1-1-data_genie', 'tool', 100_000), stage('step-1-2-run_sql', 'tool', 100_000)];
  ...
  it('offers a conversation that has a run even when the conversation list missed it', () => {
    const run: Run = {
      ...
      kind: 'conversation',
      prompt: 'How is GTA doing this week?',
      stakeholder: 'jay.mehta@cmeg.example',
      status: 'complete',
      duration_ms: 1,
      rating: null,
    };
    expect(conversationFilterOptions([], [run])).toEqual([{ id: 'conv-jay', label: 'How is GTA doing this week?' }]);
  });
  ...
  it('lists only usernames that have runs, and never invents one', () => {
    ...
    expect(
      usernameFilterOptions([
        run('a', 'sam.mathews@cmeg.example'),
        run('b', 'jay.mehta@cmeg.example'),
        run('c', 'sam.mathews@cmeg.example'),
        run('d', 'Another team member'),
        run('e', ''),
      ])
    ).toEqual([
      { value: 'jay.mehta', label: 'jay.mehta' },
      { value: 'sam.mathews', label: 'sam.mathews' },
    ]);
  });
  ...
  it('narrows the list by username and still honours search', () => {
    ...
    const rows = [
      run('sam-gta', 'sam.mathews@cmeg.example', 'is GTA bringing in more users than Red Dead 2?'),
      run('jay-gta', 'jay.mehta@cmeg.example', 'How is GTA doing this week?'),
      run('sam-rdr', 'sam.mathews@cmeg.example', 'Red Dead retention'),
    ];

    expect(matchingRuns(rows, { username: 'jay.mehta' }).map((item) => item.id)).toEqual(['jay-gta']);
    expect(matchingRuns(rows, { username: 'sam.mathews', search: 'GTA' }).map((item) => item.id)).toEqual(['sam-gta']);
    expect(matchingRuns(rows, { search: 'GTA' }).map((item) => item.id)).toEqual(['sam-gta', 'jay-gta']);
  });
});
