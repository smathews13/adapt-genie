...
const ADAPT_FORECAST_COMPONENTS = new Set([
  'serving-endpoint',
  'foundation-model',
  'sql-warehouse',
  'app-compute',
...
function adaptForecastResult(result: ForecastResult): ForecastResult {
  const components = result.components.filter((component) => ADAPT_FORECAST_COMPONENTS.has(component.id));
  return {
    ...result,
    components,
    horizons: result.horizons.map((horizon) => {
      const projected = horizon.components.filter((component) => ADAPT_FORECAST_COMPONENTS.has(component.id));
      ...
      return {
        ...
        total: available.length > 0 ? available.reduce((sum, amount) => sum + amount, 0) : null,
      };
    }),
  };
}
...
export function ProjectionBreakdown({
  ...
  const result = adaptForecastResult(rawResult);
  const complete =
...
export function ForecastingBody({
  ...
  const result = adaptForecastResult(calculateForecast(baseline, assumptions));
  const unavailable =
  ...
  return (
    <section className="ops-block ops-forecast" aria-labelledby="ops-forecast-heading" data-testid="ops-forecasting">
      ...
      <div className="ops-block-body">
        {waiting ? (
          <OpsLoadingState label="Loading cost forecast" />
        ) : unavailable ? (
