...
import{PLAN_APPROVAL_MESSAGE,feedbackDirectionSql,organizationForEmail,parseOrganizationMappings}from"./chunk-YS6R5DXU.mjs";import{markResponse,noSubstitution,readStored}from"./chunk-HX27HHAK.mjs";import"./chunk-VBGT52IV.mjs";import"./chunk-CEUYMJTW.mjs";import"./chunk-GRUXLPKC.mjs";import"./chunk-A3UHBMOK.mjs";import"./chunk-LIDXDLZN.mjs";import{ADDED_ADMINS_TABLE,ROLE_COLUMN,isRole,seedAdminEmails,seedSuperAdminEmails}from"./chunk-PU5OFDI2.mjs";import"./chunk-EK5YSKB2.mjs";import"./chunk-66G4LGYE.mjs";import"./chunk-3SZ5WCG5.mjs";import"./chunk-3LJPB2Y3.mjs";import"./chunk-ALFCGEZB.mjs";import"./chunk-KLY3H7SW.mjs";import"./chunk-Y3GFGB3N.mjs";import"./chunk-ECGDSIAS.mjs";import"./chunk-LOICDKUA.mjs";import"./chunk-FYRVKI3Q.mjs";import"./chunk-JX2D4OX6.mjs";import"./chunk-DC3NGZXW.mjs";import"./chunk-JJVCIDXB.mjs";import"./chunk-ZSALFZGP.mjs";import"./chunk-OP7OL6MA.mjs";import{APP_SCHEMA}from"./chunk-YDSOP3SS.mjs";import"./chunk-A7SHUGSC.mjs";var MONITORING_FEEDBACK_SCHEMA_REVISION=3;var MONITORING_FEEDBACK_PAGE_SIZE=5;var MONITORING_FEEDBACK_PAGE_MAX=100;var MONITORING_FEEDBACK_SEARCH_MAX=200;var MONITORING_FEEDBACK_ROUTE="/api/monitoring/feedback";function stringParam(value){return typeof value==="string"?value:""}function text(value){return typeof value==="string"?value:""}function integer(value){const parsed=typeof value==="number"?value:Number.parseInt(text(value),10);return Number.isFinite(parsed)&&parsed>=0?Math.round(parsed):0}function stamp(value){if(value instanceof Date)return value.toISOString();const parsed=Date.parse(text(value));return Number.isFinite(parsed)?new Date(parsed).toISOString():""}function records(value){if(Array.isArray(value)){return value.filter(entry=>Boolean(entry)&&typeof entry==="object")}if(typeof value!=="string")return[];try{return records(JSON.parse(value))}catch{return[]}}function encodeCursor(cursor){return Buffer.from(JSON.stringify(cursor),"utf8").toString("base64url")}function decodeCursor(raw){if(!raw)return null;try{const parsed=JSON.parse(Buffer.from(raw,"base64url").toString("utf8"));const feedbackAt=typeof parsed.feedbackAt==="string"?new Date(parsed.feedbackAt).toISOString():"";const id=typeof parsed.id==="string"?parsed.id.trim():"";return feedbackAt&&id?{feedbackAt,id}:null}catch{return null}}function monitoringFeedbackCursor(feedbackAt,id){return encodeCursor({feedbackAt:new Date(feedbackAt).toISOString(),id})}function monitoringFeedbackRequest(req,now=Date.now()){const rawFrom=Date.parse(stringParam(req.query.from));const rawTo=Date.parse(stringParam(req.query.to));const from=Number.isFinite(rawFrom)?new Date(rawFrom).toISOString():new Date(now-7*864e5).toISOString();const to=Number.isFinite(rawTo)&&rawTo>Date.parse(from)?new Date(rawTo).toISOString():new Date(now).toISOString();const rawLimit=Number.parseInt(stringParam(req.query.limit),10);const limit=Number.isFinite(rawLimit)&&rawLimit>0?Math.min(rawLimit,MONITORING_FEEDBACK_PAGE_MAX):MONITORING_FEEDBACK_PAGE_SIZE;const rawCursor=stringParam(req.query.cursor).trim();const cursor=decodeCursor(rawCursor);const search=stringParam(req.query.q).trim();const feedback=stringParam(req.query.feedback).trim();const role=stringParam(req.query.role).trim();const user=stringParam(req.query.user).trim().toLowerCase();const organization=stringParam(req.query.organization).trim().toLowerCase();let error="";if(search.length>MONITORING_FEEDBACK_SEARCH_MAX){error=`Search is limited to ${MONITORING_FEEDBACK_SEARCH_MAX} characters.`}else if(rawCursor&&!cursor){error="The feedback cursor is invalid. Start again without a cursor."}else if(Number.parseInt(stringParam(req.query.offset),10)>0){error="Use the opaque cursor from pagination.nextCursor instead of an offset."}else if(feedback&&feedback!=="up"&&feedback!=="down"){error="Feedback must be up or down."}else if(role&&!isRole(role)){error="Role must be super_admin, admin, or consumer."}else if(user.length>320||organization.length>253){error="One or more feedback filters are too long."}return{from,to,limit,cursor,filters:{search,feedback:feedback==="up"||feedback==="down"?feedback:"",user,role:isRole(role)?role:"",organization},error}}var MONITORING_FEEDBACK_QUERY=`
  WITH joined AS (
    SELECT f.id AS feedback_id,
           ...
           COALESCE(roster.added_at, 'epoch'::timestamptz) AS identity_updated_at
  ...
  filtered AS (
    SELECT *
     ...
     WHERE ($6 = '' OR
       ...
       AND ($10 = '' OR organization_domain = $10 OR organization_domain LIKE ('%.' || $10))
  ),
  totals AS (
