import { isSqlQueryTagApplication } from './sql-query-tags';

/**
 * A bounded, query-scoped cancellation sweep for ADAPT SQL.
 *
...
export interface CancelAdaptWarehouseQueriesInput {
  warehouseId: string;
...
function matchesScope(row: QueryHistoryRow, scope: WarehouseCancellationScope): boolean {
  ...
  if (!isSqlQueryTagApplication(tags.get('application'))) return false;
 ...
 * one-shot sweep.
 */
export async function cancelAdaptWarehouseQueries(
  input: CancelAdaptWarehouseQueriesInput
