...
export const SLACK_INSTALLATIONS_TABLE = appTable('slack_installations');
export const SLACK_USER_LINKS_TABLE = appTable('slack_user_links');
export const SLACK_CONVERSATION_BINDINGS_TABLE = appTable('slack_conversation_bindings');
...
export const SLACK_STATE_DDL = [
  ...
  `CREATE INDEX IF NOT EXISTS slack_event_dedup_expires_idx
     ON ${SLACK_EVENT_DEDUP_TABLE} (expires_at)`,
  `CREATE TABLE IF NOT EXISTS ${SLACK_DELIVERIES_TABLE} (
...
export const SLACK_SETTINGS_DDL = `CREATE TABLE IF NOT EXISTS ${SLACK_SETTINGS_TABLE} (
  id TEXT PRIMARY KEY,
...
/**
 * Append-only v50 convergence for the render/delivery slice.
 *
 * Kept separate from SLACK_STATE_DDL because that constant is the committed v48
 * migration. Fresh and existing databases must both reach this shape through
 * the same numbered migration.
 */
export const SLACK_RENDER_DELIVERY_MIGRATION_DDL = [
  `ALTER TABLE ${SLACK_USER_LINKS_TABLE}
  ...
  `ALTER TABLE IF EXISTS ${SLACK_DELIVERIES_TABLE}
     ALTER COLUMN run_id DROP NOT NULL`,
  `ALTER TABLE ${SLACK_DELIVERIES_TABLE}
