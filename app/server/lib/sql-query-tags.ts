...
export const SQL_QUERY_TAG_APPLICATION = 'ADAPT';
/** Executable compatibility value accepted when reading historical query rows. */
export const LEGACY_SQL_QUERY_TAG_APPLICATION = 'Astrolabe';

export function isSqlQueryTagApplication(value: string | undefined): boolean {
  return value === SQL_QUERY_TAG_APPLICATION || value === LEGACY_SQL_QUERY_TAG_APPLICATION;
}
...
type SqlQueryAttribution =
  ...
  | { surface: 'watchlist'; tool: 'sales_revenue_per_unit' | 'title_catalog'; operation: 'read' }
...
export function sqlQueryTags(input: SqlQueryAttribution & SqlQueryIdentifiers): SqlQueryTag[] {
  const tags: SqlQueryTag[] = [
    { key: 'application', value: SQL_QUERY_TAG_APPLICATION },
