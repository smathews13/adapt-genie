...
export function runUserSpendHourlyRefresh(
  ...
  const run = (async () => {
    ...
    const appScope = (options.appScope ?? process.env.DATABRICKS_APP_NAME ?? '').trim() || 'adapt';
...
export async function readUserSpendHourlyPage(
  ...
  input: {
    ...
    now?: number;
    rosterOnly?: boolean;
    roster?: readonly UserSpendRosterEntry[];
  }
): Promise<UserSpendReadModelPage> {
  const appScope = (input.appScope ?? process.env.DATABRICKS_APP_NAME ?? '').trim() || 'adapt';
