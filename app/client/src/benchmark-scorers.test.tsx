...
const PUBLISHED: Scorecard = {
  provenance: {
    ...
    labelReviewConsequence:
      'A high correctness rate means the answers agree with the repository, not that they are right.',
...
function markup(state: ScorecardState, revealNonApplicable = false) {
  return renderToStaticMarkup(<HeldOutEvaluation state={state} revealNonApplicable={revealNonApplicable} />);
}
...
describe('scorers that cannot report', () => {
  ...
  it('says why the identity mismatch check cannot run', () => {
    const html = markup({ published: true, scorecard: PUBLISHED }, true);
    expect(html).toContain('no second signed-in user credential');
  });

  it('contains only the identity mismatch scorer', () => {
