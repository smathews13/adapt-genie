  ...
  it('inventories every connected class and marks only billing-capable APIs supported', () => {
    const targets = resourceTagInventory({
      ...
      report: report([
        ...
        { key: 'configured_asset', value: 'catalog.schema.asset' },
      ]),
    });
    ...
    expect(targets.some((item) => item.kind === 'registered-model')).toBe(false);
    expect(targets.filter((item) => item.label.includes('Dictionary'))).toEqual([]);
...
describe('coverage semantics', () => {
  it('excludes organizational metadata and unsupported resources from the primary denominator', async () => {
    const summary = await applyAdaptTags({
      environment: {
        DATABRICKS_APP_NAME: 'adapt',
    ...
    expect(summary).toMatchObject({
      headline: '2 of 2 supported resources tagged',
      supportedTotal: 2,
      supportedCovered: 2,
      tagged: 2,
      unsupported: 0,
    ...
    expect(summary.results.filter((item) => item.status === 'unsupported')).toHaveLength(0);
    expect(summary.results.filter((item) => item.status === 'not-applicable')).toHaveLength(3);
    expect(summary.results.filter((item) => item.status === 'tagged').map((item) => item.detail)).toEqual([
      'Applied system_billing=adapt.',
      'Applied system_billing=adapt.',
    ]);
  });
  ...
  it('counts already-correct and newly-applied resources as covered without duplicate writes', async () => {
    ...
    const summary = await applyAdaptTags({
      ...
      token: TOKEN,
      platform: platform({
        getServingTags: vi.fn(() => Promise.resolve([{ key: 'system_billing', value: 'adapt' }])),
        patchServingTags,
        setWarehouseTags,
      }),
    });
...
describe('identity, permissions, and API paths', () => {
  it('uses only the server-provided OBO token for billing-tag APIs', async () => {
    ...
    const oboAdapter = source.slice(
      ...
      source.indexOf('export async function readAppBillingTag')
    );
  ...
  it('uses the documented endpoint, warehouse, and Lakebase update-mask paths', async () => {
...
describe('bounded retries, isolation, and cancellation', () => {
  ...
  it('a second Apply retries only unresolved resources; full mode rechecks all', async () => {
    ...
    const first = await applyAdaptTags({
      environment: {
        DATABRICKS_SERVING_ENDPOINT_NAME: 'agent',
        DATABRICKS_SQL_WAREHOUSE_ID: 'warehouse',
      },
    ...
    const second = await applyAdaptTags({
      environment: {
        DATABRICKS_SERVING_ENDPOINT_NAME: 'agent',
        DATABRICKS_SQL_WAREHOUSE_ID: 'warehouse',
      },
...
describe('organizational app tag diagnostics', () => {
  it('requires an explicit metadata reader and never treats absence as billing evidence', async () => {
    ...
    expect(await readAppBillingTag('app', { getAppTag: vi.fn(() => Promise.resolve('adapt')) })).toBe('matched');
