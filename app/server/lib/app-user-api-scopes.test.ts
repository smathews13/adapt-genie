...
import { MANAGER_GRANT_USER_API_SCOPES, allowAdaptUserApiScopes } from './app-user-api-scopes';
...
describe('allowAdaptUserApiScopes', () => {
  it('adds the required scopes and workspace browse without removing an existing extra', async () => {
    ...
    const result = await allowAdaptUserApiScopes({ ...options, fetchImpl: call });
    ...
    expect(new Headers(call.mock.calls[0][1]?.headers).get('authorization')).toBe('Bearer forwarded-user-token');
    expect(call.mock.calls[1][1]?.method).toBe('PATCH');
    expect(new Headers(call.mock.calls[1][1]?.headers).get('authorization')).toBe('Bearer forwarded-user-token');
  ...
  it('does nothing when every managed scope is already present', async () => {
    ...
    await expect(allowAdaptUserApiScopes({ ...options, fetchImpl: call })).resolves.toEqual({
      kind: 'unchanged',
      scopes: current,
    });
  ...
  it('can request the official Lakebase scope without adding unrelated optional scopes', async () => {
    ...
    const result = await allowAdaptUserApiScopes({
      ...options,
  ...
  it('drops hostile non-string values returned by the Apps API', async () => {
    ...
    await allowAdaptUserApiScopes({ ...options, additionalScopes: ['postgres'], fetchImpl: call });

    const patchBody = call.mock.calls[1][1]?.body;
    if (typeof patchBody !== 'string') throw new Error('PATCH carried no JSON body');
    const parsed = JSON.parse(patchBody) as { user_api_scopes?: unknown };
    expect(parsed.user_api_scopes).toEqual([
      'catalog.tables:read',
  ...
  it('uses only the forwarded user token and never creates a service-principal client', async () => {
    ...
    await allowAdaptUserApiScopes({ ...options, fetchImpl: call });
