...
import { GroupMappingAddRow, GroupRoleDefaults, RosterAddRow, RosterRows } from './UserRoleEditor';
...
describe('workspace group defaults', () => {
  it('lists the configured admin and consumer groups with their access', () => {
...
describe('the #24a roster row', () => {
  it('marks the reader, seed origin, super role, and immutable row explicitly', () => {
    ...
    // The "Set by" column was removed for room; the row now reads reader marker,
    // organization and role, with no Deployment set-by cell.
    expect(text(markup)).toContain('you EX example.invalid Super admin');
    expect(markup).not.toContain('title="Set at deployment. Edit the bundle variable to change it."');
...
describe('the #24a Roles geometry', () => {
  ...
  it('puts email, organization, role and actions in a real table', () => {
    ...
    // The "Set by" column was removed to give identities more room; the
    // provenance still lives on the row model, just not in its own column.
    expect(markup).not.toContain('<th scope="col">Set by</th>');
