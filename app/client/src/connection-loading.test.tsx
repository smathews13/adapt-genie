  ...
  it('loads every unresolved row before the first effect starts', () => {
    ...
    expect(connectionPlaceholderReadings().map((candidate) => candidate.resource.id)).toEqual(
      CONNECTED_RESOURCES.filter(
        (resource) =>
          resource.namesRemoteObject && resource.id !== 'notebook-declaration' && !isAdaptHiddenConnection(resource.id)
...
describe('canonical ADAPT loading geometry', () => {
  ...
  it('freezes on the static ADAPT rest mark for both motion vetoes', () => {
