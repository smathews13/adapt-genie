...
function payload(revision = USER_MONITORING_SCHEMA_REVISION) {
  return {
    ...
    userMonitoring: {
      ...
      organizations: [{ ...organizationForEmail('person@databricks.com'), count: 1 }],
      users: [
        {
...
describe('User Monitoring response decoding', () => {
  ...
  it('preserves canonical organization options and counts through the decoder', () => {
    const value = payload();
    value.userMonitoring.organizations = [{ ...organizationForEmail('person@databricks.com'), count: 4 }];
  ...
  it('rejects malformed organization facets instead of silently rendering All as zero', () => {
    const value = payload();
    value.userMonitoring.organizations = [{ ...organizationForEmail('person@databricks.com'), count: Number.NaN }];
