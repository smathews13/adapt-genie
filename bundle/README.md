...
The customer-visible resource identity is consistent across the bundle:
`adapt-genie` for the bundle and App, `adapt-orchestrator` for the serving
endpoint, `${app_catalog}.${app_schema}.adapt_orchestrator` for the registered
model, `/Shared/adapt-genie` for the MLflow experiment, and
...
app queries net revenue per unit sold as each signed-in reader.
...
```bash
ADAPT_CONFIRMED_NO_LIVE_DEPLOY=true TARGET=<target> PROFILE=<profile> \
...
| Script | What it does |
| --- | --- |
| `agent-release.sh` | Log the model, enforce the serving endpoint's three-entity ceiling before deployment, deploy, grant and verify `CAN_QUERY` for the configured app admin/user groups, wait for the traffic switch, then prune superseded entities. The group grant is required because app requests invoke Model Serving with each signed-in user's forwarded token; the app resource binding alone grants only the app service principal. At the ceiling it removes idle entities first and refuses if no slot can be made; `--served` lists versions and traffic without releasing, while `--no-prune` leaves idle entities and refuses a fourth. |
...
resource-only reconciliation, that command can remove a resource. The bundle's
`schemas.adapt_telemetry_schema` holds app history that does not backfill and
carries `prevent_destroy`, which stops `bundle destroy` and does **not** stop a
`replace`. The existing model schema is outside bundle ownership, so a bundle
reconciliation cannot update or delete it.
