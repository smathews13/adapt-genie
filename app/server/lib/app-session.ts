...
/** Executable compatibility cookie name shared with deployed clients. */
...
async function requireVerifiedSession(
  ...
  if (!sessionId) {
    refusal(res, 401, APP_SESSION_REQUIRED_CODE, 'Start a new ADAPT app session from the application entry page.');
    return false;
  }
  const subject = subjectFor(req, controls);
  if (!subject) {
  ...
  try {
    ...
    if (state === 'idle_expired' || state === 'absolute_expired' || state === 'revoked') {
      ...
      refusal(
        ...
        `This ADAPT app session ended after ${config.minutes} minutes without explicit user activity.`
      );
      return false;
    }
    ...
    refusal(
      ...
      503,
      APP_SESSION_UNAVAILABLE_CODE,
      'ADAPT cannot verify app-session state, so protected data is unavailable.'
    );
    return false;
  }
}
...
export function registerAppSessionControls(app: Application, controls: AppSessionControls): IdleTimeoutConfig {
  ...
  app.post('/api/app-session/bootstrap', async (req, res) => {
    ...
    try {
      ...
      refusal(
        ...
        503,
        APP_SESSION_UNAVAILABLE_CODE,
        'ADAPT cannot persist a new app session, so protected data remains unavailable.'
      );
    }
  });
  ...
  app.post('/api/app-session/activity', async (req, res) => {
    ...
    try {
      ...
      refusal(res, 503, APP_SESSION_UNAVAILABLE_CODE, 'ADAPT cannot verify or refresh app-session state.');
    }
  });
