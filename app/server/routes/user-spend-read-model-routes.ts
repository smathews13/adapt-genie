  ...
  appkit.server.extend((app: Application) => {
    ...
    app.get('/api/monitoring/user-spend', async (req: Request, res: Response) => {
      ...
      try {
        ...
        const countedEmails = entries
          .filter((entry) => (!search || entry.email.toLowerCase().includes(search)) && (!role || entry.role === role))
      ...
      const readRosterPage = () =>
        hourly
          ? readHourly(req, window, principal, true, limit, offset, true, organizationDomains, rosterEntries)
          : readDaily(req, range, principal, true, limit, offset, true, organizationDomains, rosterEntries);
    ...
    app.get('/api/monitoring/user-spend/:email', async (req: Request, res: Response) => {
      ...
      try {
        const readProfile = () =>
          hourly
            ? readHourly(req, window, email, false, 1, 0, true, [], rosterEntries)
            : readDaily(req, range, email, false, 1, 0, true, [], rosterEntries);
        recovery = await recoverInitialUserSpendRead({
          read: readProfile,
          refresh: hourly
            ? () => runUserSpendHourlyRefresh(appkit.lakebase, { from: window.from, to: window.to, now: clock() })
        ...
        ? {
            ...
            metrics: buildUserSpendMetrics({
              ...
              current: {
                ...
                questions: selected.row.completedQuestions,
