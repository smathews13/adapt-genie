...
export interface WarehouseQueryAttribution {
  ...
  complete: boolean;
  /** ADAPT Ask statements that were not generated by Genie. */
  adaptQueries: number;
  totalQueries: number;
...
/** Partial reads remain usable only when pagination and transport were complete. */
export function warehouseQueryAttributionUsable(attribution: WarehouseQueryAttribution): boolean {
  if (attribution.complete) return true;
  const reasons = attribution.coverage?.reasons ?? [];
  return (
    attribution.totalExecutionMs > 0 &&
    reasons.length > 0 &&
    reasons.every((reason) => reason === 'missing-execution-time' || reason === 'interactive-run-coverage')
  );
}
...
function interactiveRunId(row: QueryHistoryRow, runIds: ReadonlyMap<string, string>): string {
  ...
  if (!isSqlQueryTagApplication(tags.get('application')) || tags.get('surface') !== 'ask') return '';
...
function isAppOperation(row: QueryHistoryRow): boolean {
  const tags = parseQueryTags(row.query_tags);
  return isSqlQueryTagApplication(tags.get('application')) && tags.get('surface') === 'ask';
}
...
export async function readWarehouseQueryAttribution(input: {
  ...
  const aggregate = (row: QueryHistoryRow): void => {
    ...
    // the ADAPT bucket makes the two allocations mutually exclusive.
    const interactiveRun = !spaceId ? interactiveRunId(row, runIds) : '';
    const adapt = !spaceId && isAppOperation(row);
    if (adapt) adaptQueries += 1;
    if (spaceId) {
    ...
    if (adapt) adaptExecutionMs += duration;
    ...
    if (user) {
      const current = users.get(user) ?? { adaptExecutionMs: 0, genieSpaces: new Map<string, number>() };
      if (adapt) current.adaptExecutionMs += duration;
  ...
  return {
    ...
    users: [...users]
      ...
      .map(([email, values]) => ({
        email,
        adaptExecutionMs: values.adaptExecutionMs,
