...
import { generatePrivateKeyValue, issueGenieMcpCapability } from '../lib/genie-mcp-capability';
...
// Captured from a deployed compatibility endpoint so the app
...
async function startInsightsApp(
  ...
  return {
    ...
    async conversations(): Promise<{ conversations: { id: string; title: string }[] }> {
      const response = await fetch(`http://127.0.0.1:${port}/api/conversations`, { headers: headers() });
      return (await response.json()) as { conversations: { id: string; title: string }[] };
    },
    ...
    /**
     * Starts the same SSE request ADAPT chat uses and lets the test navigate away.
     *
...
describe('plan approval round trip through POST /api/insights/ask', () => {
  ...
  it('puts the approval on the wire as snake_case custom_inputs', async () => {
    ...
    expect(captured[1]?.path).toBe('/serving-endpoints/app/invocations');
...
describe('serving request body', () => {
  it('omits approval keys until the user actually approves', () => {
    ...
    expect(body.custom_inputs).toEqual({ conversation_id: 'conv-1', genie_transport: 'direct' });
  });
  ...
  it('cannot emit MCP without a signed capability', () => {
    const body = buildAskServingBody({
      history: [{ role: 'user', content: NONTRIVIAL_QUESTION }],
      prompt: NONTRIVIAL_QUESTION,
      conversationId: 'conv-1',
    ...
    expect(body.custom_inputs).toMatchObject({ genie_transport: 'direct' });
  });

  it('carries a server-signed capability only on the MCP transport', () => {
    ...
    const body = buildAskServingBody({
      ...
      requestId: 'req-1',
      genieTransport: 'mcp',
      genieCapability: capability ?? undefined,
    });
...
describe('Genie MCP capability', () => {
  it('drops client attempts to select the privileged transport', () => {
    ...
    expect(parsed).not.toHaveProperty('genieTransport');
    expect(parsed).not.toHaveProperty('genie_transport');
  });

  function appkitFor(genieCodeMcp: boolean, role: 'super_admin' | 'admin' | 'consumer' = 'admin'): InsightsAppKit {
    return {
      lakebase: {
        query: (sql: string) => {
          if (sql.includes('experimental_settings')) {
            return Promise.resolve({
              rows: [
                {
                  settings: {
          ...
          if (sql.includes('admin_emails')) {
            return Promise.resolve({
              rows: [
                {
                  email: role === 'consumer' ? 'consumer@example.test' : 'mcp-admin@example.test',
                  role,
                  added_by: 'seed',
                  added_at: new Date(),
                },
          ...
          return Promise.reject(new Error(`Unexpected capability SQL: ${sql}`));
        },
      },
      server: { extend: () => undefined },
    };
  }

  it('signs only after the durable flag and authoritative admin role both allow it', async () => {
    ...
    await expect(resolveGenieAuthorization(appkitFor(true), 'mcp-admin@example.test', input)).resolves.toMatchObject({
      transport: 'mcp',
      capability: { claims: { sub: 'mcp-admin@example.test', request_id: 'req-1' } },
    });
    await expect(
      resolveGenieAuthorization(appkitFor(true, 'super_admin'), 'mcp-admin@example.test', input)
    ).resolves.toMatchObject({ transport: 'mcp' });
    await expect(resolveGenieAuthorization(appkitFor(false), 'mcp-admin@example.test', input)).resolves.toEqual({
      transport: 'direct',
    });
    await expect(
      resolveGenieAuthorization(appkitFor(true, 'consumer'), 'consumer@example.test', input)
    ).resolves.toEqual({
      transport: 'direct',
    });
    await expect(
      resolveGenieAuthorization(appkitFor(true), 'mcp-admin@example.test', {
        ...input,
        identityMode: 'app_service_principal',
      })
    ).resolves.toEqual({ transport: 'direct' });
  });

  it('fails closed on a fresh settings read even after this process cached Genie MCP as enabled', async () => {
    const appkit = appkitFor(true);
    const input = {
      requestId: 'req-1',
      audience: 'adapt-orchestrator',
      identityMode: 'signed_in_user',
    ...
    await expect(resolveGenieAuthorization(appkit, 'mcp-admin@example.test', input)).resolves.toMatchObject({
      transport: 'mcp',
    });
    const query = appkit.lakebase.query.bind(appkit.lakebase);
    appkit.lakebase.query = (sql) =>
      sql.includes('experimental_settings') ? Promise.reject(new Error('Lakebase unavailable')) : query(sql);
    await expect(
      resolveGenieAuthorization(appkit, 'mcp-admin@example.test', { ...input, requestId: 'req-2' })
    ).resolves.toEqual({ transport: 'direct' });
  });
});
...
describe('the production serving transport', () => {
  ...
  it('hands the body to the API client without rebuilding it', async () => {
    ...
    expect(seen[0]?.path).toBe('/serving-endpoints/app/invocations');
...
describe('what the route actually puts on the wire', () => {
  ...
  it('sends stored attachment text, which no route test could previously observe', async () => {
    ...
    const lakebase = memoryLakebase([
      {
        ...
        extracted_text: 'Project HALCYON-7742 retires Red Dead Online on 2026-11-15.',
      },
    ]);
    const app = await startInsightsApp(agentContractTransport(captured), lakebase);
    ...
    expect(customInputs.attachment_text).toBe(
      '## halcyon-memo.txt\nProject HALCYON-7742 retires Red Dead Online on 2026-11-15.'
    );
  });
...
describe('identity and benchmark records', () => {
  it('reports execution responsibility without disclosing the application principal id', () => {
    ...
    expect(identityPayload(request({ 'x-forwarded-email': 'analyst@cmeg.example' }))).toEqual({
      signedInAs: 'analyst@cmeg.example',
  ...
  it('reports the serving principal separately once a preflight has named it', () => {
    ...
    const payload = identityPayload(request({ 'x-forwarded-email': 'analyst@cmeg.example' }));
  ...
  it('reports a mode the server established rather than a fixed literal', () => {
    recordVerifiedAccess('analyst@cmeg.example', 'holds SELECT on 10 tables');
    const payload = identityPayload(request({ 'x-forwarded-email': 'analyst@cmeg.example' }));
    ...
    expect(identityPayload(request({ 'x-forwarded-email': 'other@cmeg.example' })).executionMode).toBe(
      'service-principal'
    );
    forgetAccessDecisions();
  });
...
describe('an answered conversation is a run', () => {
  ...
  it('declares the spaces on the answer contract, so a run that used one is not drift', () => {
    ...
    const parsed = TraceSchema.parse({
      ...
      toolCalls: 1,
      stages: [],
      genie_spaces: [{ id: 'space-data', title: 'ADAPT Data' }],
    });

    expect(parsed.genie_spaces).toEqual([{ id: 'space-data', title: 'ADAPT Data' }]);
...
describe('GET /api/runs/:id/trace', () => {
  ...
  it('links a stored trace into the experiment saved in the app, not only the one in the environment', async () => {
    process.env.DATABRICKS_SERVING_ENDPOINT_NAME = 'app';
    process.env.DATABRICKS_HOST = 'https://fevm-sample.cloud.databricks.com';
        ...
        [
          {
            ...
            intent: 'active',
            updated_by: 'deployer@take-two.com',
          },
        ]
      )
    );
...
describe('the MLflow trace behind an answer', () => {
  ...
  it('recognises an MLflow trace id and links to it', () => {
    ...
    expect(reference?.url).toBe(
      'https://fevm-sample.cloud.databricks.com/ml/experiments/9998887776665554/traces' +
...
describe('a canned answer discloses that no live query produced it', () => {
  ...
  it('leaves a prose reply with no borrowed figures to disclose', async () => {
    ...
    const app = await startInsightsApp(
      () =>
        Promise.resolve({
          output: [{ content: [{ type: 'output_text', text: 'GTA Online leads the last 30 days.' }] }],
        }),
      memoryLakebase()
    );
    ...
    try {
      ...
      expect(answered.mode).toBe('live');
      expect(answered.narrative).toBe('GTA Online leads the last 30 days.');
        ...
        Promise.resolve({
          custom_outputs: {
            ...
            player_insights_forecast: {
              horizon_days: 30,
              projected_active_players: [{ title: 'GTA Online', p50: 19_400 }],
            },
          },
        }),
      'conv-honest-future'
  ...
  return {
    ...
    lakebase: {
      query: (sql: string, params: unknown[] = []) => {
        ...
        // Conversation rail rows still belong to this memory store.
...
describe('the run ledger under POST /api/insights/ask', () => {
  ...
  it('aborts a never-ending Ask at the deadline and ignores output attempted afterward', async () => {
    ...
    const store = memoryLakebase();
    const { ledger, lakebase } = lakebaseWithLedger(store);
    const app = await startInsightsApp(transport, lakebase, { servingTimeoutMs: 2_000 });
    ...
    try {
      ...
      const stored = store.messages.filter((message) => message.role === 'assistant');
      expect(stored).toHaveLength(1);
      const payload =
        typeof stored[0].response_json === 'string'
          ? (JSON.parse(stored[0].response_json) as { trace?: { stages?: { id?: string }[] } })
          : (stored[0].response_json as { trace?: { stages?: { id?: string }[] } });
      expect(payload.trace?.stages?.map((stage) => stage.id)).toEqual(['step-before-timeout']);
      expect(ledger.runs[0].terminal_message_id).toBe(stored[0].id);
  ...
  it('persists before serving and finishes after the Ask view disconnects', async () => {
    ...
    try {
      ...
      const conversationList = await app.conversations();
      expect(conversationList.conversations.some((conversation) => conversation.id === 'conv-detached')).toBe(true);
