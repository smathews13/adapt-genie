...
function adaptTags(extra: Record<string, string> = {}): Record<string, string> {
  return { application: 'ADAPT', ...extra };
}
...
describe('owner cancellation boundary', () => {
  it('cancels only the signed-in owner matching the tagged run or correlation', async () => {
    ...
    const rows = [
      row('own-run', adaptTags({ run_id: 'run-1' }), { query_text: secretSql }),
      row('own-correlation', adaptTags({ correlation_id: 'corr-1' })),
      row('other-run', adaptTags({ run_id: 'run-2' })),
      row('other-user', adaptTags({ run_id: 'run-1' }), {
        executed_as_user_name: 'other@example.com',
      }),
      row('submitter-is-not-owner', adaptTags({ run_id: 'run-1' }), {
        executed_as_user_name: 'other@example.com',
        user_name: OWNER,
      }),
      row('other-app', { application: 'SomethingElse', run_id: 'run-1' }),
      row('wrong-case-app', { application: 'other-app', run_id: 'run-1' }),
    ...
    const result = await cancelAdaptWarehouseQueries({
...
describe('admin cancellation boundary', () => {
  it('cancels all and only exactly ADAPT-tagged active queries', async () => {
    const rows = [
      row('array-tags', [
        { key: 'application', value: 'ADAPT' },
        { key: 'run_id', value: 'run-array' },
      ]),
      row('map-tags', adaptTags({ run_id: 'run-map' }), {
      ...
      row('wrong-warehouse', adaptTags(), { warehouse_id: 'warehouse-other' }),
      row('finished-despite-filter', adaptTags(), { status: 'FINISHED' }),
...
describe('bounded Query History sweep', () => {
      ...
      listQueries(input): Promise<QueryHistoryPage> {
        ...
        if (input.pageToken === 'running-page-2') {
          return Promise.resolve({
            res: [row('first-visible', adaptTags()), row('from-page-two', adaptTags())],
          });
        }
        runningPass += 1;
        if (runningPass === 1) {
          return Promise.resolve({
            res: [row('first-visible', adaptTags())],
            next_page_token: 'running-page-2',
            has_next_page: true,
          });
        }
        return Promise.resolve({
          res: [row('first-visible', adaptTags()), row('late-visible', adaptTags())],
        });
      },
  ...
  it('shares one deadline across lookup, delay, and statement cancellation', async () => {
    ...
    try {
      ...
      const pending = cancelAdaptWarehouseQueries({
        warehouseId: WAREHOUSE,
        scope: { mode: 'admin' },
        transport: { listQueries, cancelStatement: () => Promise.resolve() },
        deadlineMs: 5,
...
describe('cancel result honesty', () => {
  it('separates races, refusals, and failures without returning provider messages', async () => {
    const rows = [
      row('raced-409', adaptTags()),
      row('not-running', adaptTags()),
      row('refused-403', adaptTags()),
      row('failed-500', adaptTags()),
    ];
    ...
    const transport = createDatabricksWarehouseCancellationTransport({
      request(options) {
        ...
        return Promise.resolve(
          options.method === 'GET'
            ? {
                res: [row('statement/with space', adaptTags())],
