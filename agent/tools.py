...
def _string_literal(value: str) -> str:
    ...
    Kept local because the tag path is self-contained. Backslash first, or the
    doubled quote gets escaped again.
    """
...
_TAGS_ALTERNATIVES = (
    ...
    "list_data_assets and describe_table to find the tables. Do not tell the user "
    "their tables are untagged on the strength of this."
)
...
class PlayerInsightTools:
    ...
    def run_sql(self, sql: str) -> ToolResult:
        return self._read(sql, "orchestrator.run_sql", "run_sql")
    ...
    def list_data_assets(self, catalog: str = "", schema: str = "") -> ToolResult:
        ...
        tables = sorted(name for name in in_catalog if name.split(".")[1] == schema)
        if not tables:
            return ToolResult(
                text=(
    ...
    *,
    ...
    if not grouped:
        ...
        return (
            ...
            "tables cannot answer the question. Use list_data_assets and describe_table."
        )
