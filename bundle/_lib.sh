...
acquire_run_lock() {
  ...
  lock_dir="${TMPDIR:-/tmp}/adapt-${safe_name}.lock"
...
load_bundle_json() {
  ...
  local cache="${ADAPT_BUNDLE_JSON_CACHE:-}"
...
# ADAPT_BUNDLE_JSON_CACHE and reads the file instead of the CLI.
#
...
seed_bundle_cache() {
  [[ -n "${ADAPT_BUNDLE_JSON_CACHE:-}" ]] && return 0
  load_bundle_json
  ADAPT_BUNDLE_JSON_CACHE="$(mktemp "${TMPDIR:-/tmp}/adapt-bundle-json.XXXXXX")"
  export ADAPT_BUNDLE_JSON_CACHE
  on_exit 'rm -f "$ADAPT_BUNDLE_JSON_CACHE" "$ADAPT_BUNDLE_JSON_CACHE.key"'
  printf '%s' "$_BUNDLE_JSON" > "$ADAPT_BUNDLE_JSON_CACHE"
  _bundle_cache_key > "$ADAPT_BUNDLE_JSON_CACHE.key"
}
