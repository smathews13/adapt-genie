 ...
 *
 * ONE SOURCE. `var.lakebase_app_schema` in databricks.yml, the compatibility
 * schema environment variable the release bakes into app.yaml, the DDL that
 ...
 * resolve. The historical app.yaml value is retained as an executable
 * compatibility identifier. A source-only deploy maps that identifier to the
 * ADAPT-owned default below. Bundle releases fill the target and therefore keep
 * their configured schema, including existing compatibility stores and role
 * rows.
 */

/** The schema a direct Git deployment creates and owns. */
export const DEFAULT_APP_SCHEMA = 'adapt';

/** Executable compatibility identifier still used by existing bundle targets. */
 ...
 * which ADAPT schema the unchanged app role already owns. Keeping the
