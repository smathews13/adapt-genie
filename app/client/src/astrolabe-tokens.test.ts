...
/**
 * The ADAPT palette, checked against the things it is easiest to get wrong.
 *
 ...
 * the delivered ADAPT tokens -- names included, since an earlier pass here
 ...
 *     copy: someone builds an ADAPT surface by starting from the nearest DuBois rule.
...
describe('the ADAPT palette matches the delivered token specification', () => {
  ...
  it('agrees with the scale tokens.css already declares, step for step', () => {
    ...
    // the ADAPT set broken at the moment the DuBois set is retired, which is the
...
describe('the partials are wired into the cascade', () => {
  it('imports both ADAPT compatibility partials, so a test of them is a test of the app', () => {
