...
DISTRIBUTIONS = {"databricks": "databricks-sdk", "databricks_mcp": "databricks-mcp"}
...
def test_removed_retrieval_stack_is_absent_from_runtime_and_packaging():
    runtime = (AGENT / "agent.py").read_text()
    packaging = (AGENT / "log_model.py").read_text()
    removed_modules = (
        "semantic_" + "layer",
        "semantic_" + "layer_build",
        "semantic_" + "retrieval",
    )
    removed_symbols = (
        "Databricks" + "Vector" + "SearchIndex",
        "search_" + "semantics",
        "VECTOR_" + "SEARCH_SCOPES",
        "SEMANTIC_" + "INDEX",
    )

    assert all(not (AGENT / f"{module}.py").exists() for module in removed_modules)
    assert all(symbol not in runtime for symbol in removed_symbols)
    assert all(symbol not in packaging for symbol in (*removed_modules, *removed_symbols))
