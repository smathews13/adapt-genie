 ...
 * recurrence. `role.ts` has held the admin set -- Ask, Monitoring, Ops -- while
 * `Layout.tsx` once drew a hardcoded row that had never heard of Monitoring or
 * Ops. Both routes were registered and both URLs worked. Nobody could reach
 * either by clicking, and a deploy would have shipped two invisible pages.
 *
 ...
 * what it says.
 *
 * The explicit mock keeps this file focused on the role contract even if a
 * future review build temporarily changes the deployment-wide flag.
 *
...
const ADMIN_ORDER = ['Ask', 'Monitoring', 'Ops', 'Connections', 'Architecture'];
const CONSUMER_ORDER = ['Ask'];

describe('administrators can reach Monitoring, Ops, and Connections by clicking', () => {
  it.each(['admin', 'super_admin'] as const)('draws the full navigation for a %s, in order', (role) => {
    // The order is the claim, not just the membership. Monitoring and Ops follow
    // Ask -- the assistant first, then the two pages that describe the deployment
    // -- because that is the order an administrator reads them in.
    expect(labels(render(role))).toEqual(ADMIN_ORDER);
  });

  it.each(['admin', 'super_admin'] as const)('points a %s at the registered routes', (role) => {
    expect(hrefs(render(role))).toEqual(['/', '/monitoring', '/ops', '/connections', '/architecture']);
  });
  ...
  it('draws the same tabs inside the mobile sheet', () => {
...
describe('a consumer is not shown a door that will be shut in their face', () => {
  it('draws Ask alone and neither admin page', () => {
...
describe('a wider row still starts on the conversation rail’s edge', () => {
  ...
  it('singles out no tab at either width, so the first one is not cut off', () => {
    ...
    // padding on top of that, which left ADAPT Ask's icon on its own edge. Neither
