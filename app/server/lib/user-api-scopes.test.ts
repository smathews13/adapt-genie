 ...
 * happens: the row names the missing API permission instead of
...
const ACCESS_GUIDE_PATH = join(REPO, 'docs', 'ADAPT_Access_Guide.md');
 ...
 * block of its own inherits, i.e. what a customer / takeTwo deployment requests.
 *
...
/** Every subject the ADAPT deployment probes, including its tables. */
...
describe('the scopes the bundle declares against the scopes the probes call with', () => {
  it('accounts for every scope the customer target probes need, as declared or as staged', () => {
    const declared = effectiveScopes('customer');
    const staged = stagedScopes('customer');
  ...
  it('requests those and no more, so an added scope is a decision rather than a drift', () => {
    // The customer target inherits the shared default. Workspace read and postgres are
    // requested for the released notebook and Lakebase browsers.
    expect(effectiveScopes('customer')).toEqual([
  ...
  /**
   * The customer target carries no scope override of its own, so it inherits
   * the shared ADAPT default without maintaining a second copy.
   */
  it('carries no scope block of its own, inheriting the shared default', () => {
    expect(declaredScopes('customer')).toEqual([]);
  });

  it('has no staged scopes after releasing Lakebase browse', () => {
    expect(stagedScopes('customer')).toEqual([]);
  });
  ...
  it('never has a scope declared and staged at once', () => {
    const declared = effectiveScopes('customer');
    expect(stagedScopes('customer').filter((scope) => declared.includes(scope))).toEqual([]);
  });
   ...
   *
   * answers with coarse families including `unity-catalog`. A previous version
   * of this file pinned a coarse family AS the right answer, on that evidence.
   * The Apps API then rejected `unity-catalog`
   ...
   *
   *   databricks api post /api/2.0/apps -p "cmeg demo" \
   *     --json '{"name":"adapt","user_api_scopes":["<name>"]}'
   *
  ...
  it('names the scopes the way the Apps API does, not the way the OAuth server does', () => {
    ...
    for (const rejected of ['unity-catalog', 'catalog', 'workspace']) {
...
/**
 * The shared default a customer / takeTwo deployment inherits.
 *
 * WHY THIS IS THE GUARD THAT MATTERS. The default is now the ONE source of the
 * scope list -- cmeg inherits it rather than re-listing it (see the block
 * above). The regression this stops: the browse scopes were once CMEG-ONLY, so
 ...
 * into the variable default on 2026-08-18, so every target -- customer and cmeg
 * alike -- requests the same list. A DAB complex-variable override REPLACES the
 * default, so the only
...
describe('the shared default every customer / T2 deployment inherits', () => {
  ...
  it('requests the catalog browse scopes, not just cmeg', () => {
   ...
   * slips through. cmeg inherits this exact list.
   */
