...
`custom_inputs.request_id`; the companion app's correlation module is the other
half of this contract and holds the reasoning for why the browser mints it and
why it is not the run ledger's primary key. This module's job is narrower: put
that id where MLflow can be SEARCHED by it, so a question the app logged can be
turned into the trace of the run that answered it.
...
_QUERY_IDS: ContextVar[dict[str, str] | None] = ContextVar("query_ids", default=None)
...
def activate_query_ids(required: Any) -> None:
    """Bind this request's safe IDs for downstream SQL attribution."""

    values = {
        CORRELATION_TAG: usable(getattr(required, "request_id", "")),
        RUN_TAG: usable(getattr(required, "run_id", "")),
    }
    _QUERY_IDS.set({key: value for key, value in values.items() if value})


def current_query_ids() -> dict[str, str]:
    """Return a copy of the IDs bound to the current request context."""

    return dict(_QUERY_IDS.get() or {})


def clear_query_ids() -> None:
    """Clear request-scoped SQL tags after a turn or test."""
