...
export function actionableResourceTagResults(results: TagResult[]): TagResult[] {
  return results.filter((result) => result.billingAttribution && result.support === 'supported');
}
...
export function ResourceTagResults({
  ...
  const issues = [
    summary.permissionRequired > 0 ? `${summary.permissionRequired} need access` : '',
    summary.supportedFailed > 0 ? `${summary.supportedFailed} failed` : '',
    summary.unsupported > 0 ? `${summary.unsupported} unsupported` : '',
    summary.notApplicable > 0 ? `${summary.notApplicable} excluded` : '',
  ].filter(Boolean);
  return (
    <section className="resource-tag-result" role="status" aria-live="polite">
      <div className="resource-tag-summary">
        <div>
          ...
          {issues.length > 0 ? <span>{issues.join(' · ')}</span> : null}
        </div>
      ...
      {!hidden ? (
        <div className="settings-table-frame resource-tag-table-frame">
          <table className="settings-data-table resource-tag-table">
            ...
            <tbody>
              {actionableResourceTagResults(summary.results).map((result) => (
