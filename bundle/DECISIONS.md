...
- the Genie space instructions, which reach a reader through the model's answer
  ...
  attaches to a space it does not create, so a deploy no longer pushes this text
  and the live space is the copy that matters. `genie/adapt_poc_space.json`
...
- **Unity Catalog metadata**: table and column comments, schema and volume
  comments, and table properties, written when the tables are provisioned. They
  are query context, not inert documentation, and can reach a reader through a
  Genie answer.

Two corollaries, because the first attempt at this got both wrong. **Removing a
comment is not complying with this**: an empty definition gives Genie less useful
context; drop the provenance clause and keep the description. And **the claim has
softer forms**: "null by design" and "deliberately visible" assert that someone
constructed the rows.
...
**Decided 2026-08-15.** No ADAPT surface may fill a gap with something plausible.
`allow_unattributed_figures` relaxes exactly that
...
Recorded in the ADAPT monitoring copy rules.
...
Applies to copy a reader reaches. A missing metric may still render as an em dash
where the ADAPT surface specification calls for one, because that is a
...
| --- | --- |
| The constraints that outrank everything else on the ask and answer surfaces | The maintained ADAPT surface specification |
