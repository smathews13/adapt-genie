...
import { ADAPT_SUPER_ADMIN_EMAILS, isAdminRoute } from '../lib/admin-roles';
...
describe('User Monitoring read-model routes', () => {
  ...
  it('serves the list with one bounded Lakebase query and no warehouse/system-billing call', async () => {
    ...
    expect(summary?.sql).toContain('FROM jsonb_to_recordset($14::jsonb)');
    ...
    expect(summary?.params[13]).toContain('"email":"person@example.test"');
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        organizations: expect.arrayContaining([expect.objectContaining({ id: 'domain:example.test', count: 1 })]),
  ...
  it('builds the Databricks facet from the full roster instead of the current page', async () => {
    const rosterRows = [
      storedRow({ display_email: 'one@databricks.com', total_users: '4' }),
      storedRow({ display_email: 'two@engineering.databricks.com', total_users: '4' }),
      storedRow({ display_email: 'three@databricks.com', total_users: '4' }),
    ...
    await handlers.get('/api/monitoring/user-spend')!(
      {
        ...
        headers: { 'x-forwarded-email': 'admin@databricks.com' },
        header: (name: string) => (name.toLowerCase() === 'x-forwarded-email' ? 'admin@databricks.com' : undefined),
      } as unknown as Request,
      res
    );
    const summary = calls.find((call) => call.sql === READ_USER_SPEND_SUMMARY_QUERY);
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        users: [expect.objectContaining({ email: 'one@databricks.com' })],
        organizations: expect.arrayContaining([
          expect.objectContaining({ id: 'databricks', name: 'Databricks', logoKey: 'databricks', count: 5 }),
        ]),
  ...
  it('keeps Identity-roster organization options when combined filters return no rows', async () => {
    ...
    const summary = calls.find((call) => call.sql === READ_USER_SPEND_SUMMARY_QUERY);
    expect(summary?.params.slice(6, 9)).toEqual(['no match', 'admin', ['studio.example', 'partner.example']]);
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        users: [],
  ...
  it('computes current-period profile KPIs from the roster and two bounded spend reads', async () => {
    const { handlers, query, calls } = routes();
    const res = response();
        ...
        users: [
          expect.objectContaining({
            ...
            metrics: expect.objectContaining({
              costPerQuestion: expect.objectContaining({ value: 12.5 / 3 }),
  ...
  it('keeps best-known spend and derives core KPIs when one source is incomplete', async () => {
    ...
    expect(payload.users[0]?.metrics.costPerQuestion).toMatchObject({ value: 9.55 / 3, estimated: true });
  ...
  it('serves full spend for an immutable Identity entry that has no stored roster row', async () => {
    ...
    const summary = calls.find((call) => call.sql === READ_USER_SPEND_SUMMARY_QUERY);
    expect(JSON.parse(String(summary?.params[13]))).toContainEqual({ email, role: 'super_admin' });
      ...
      expect.objectContaining({
        ...
        users: [
          expect.objectContaining({
            email,
            total: {
              usd: { amount: 19.75, quality: 'allocated' },
              dbu: { amount: 8.5, quality: 'allocated' },
            },
          }),
        ],
      })
