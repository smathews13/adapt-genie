...
const SPECS: readonly AssetPickerSpec[] = (
  [
    ...
    },
    {
      ...
      typeLabel: 'Or type an orchestrator endpoint name',
      typeNote: '',
    },
    {
      field: 'llm-endpoint',
    ...
    {
      field: 'declared-manifest',
      levels: TABLE_LEVELS,
      pickAt: 'last',
      multi: true,
      title: 'Tables your sign-in can see',
      typeLabel: 'Or type a comma-separated table list',
      typeNote: '',
    },
    {
      field: 'lakebase',
  ...
  ] satisfies readonly AssetPickerSpec[]
).filter((spec) => !isAdaptHiddenConnection(spec.field));
...
export function rowActions(spec: AssetPickerSpec, cursor: PickerCursor, item: BrowseItem): PickerAction[] {
  ...
  if (!last) {
    actions.push({
      ...
      cursor:
        kind === 'catalogs' || kind === 'lakebase-projects'
