...
afterEach(() => {
  ...
  delete process.env.PLAYER_INSIGHTS_APP_CATALOG;
...
describe('the authoritative telemetry destination', () => {
  ...
  it('treats an unset variable without an app catalog as no destination', () => {
    // A Git-only deployment may name neither half. Empty is more honest than
    // inventing a catalog.
    expect(telemetrySchema()).toEqual('');
    expect(logsTable('')).toEqual('');
  });

  it('does not pretend telemetry is enabled when only the app catalog is baked', () => {
    process.env.PLAYER_INSIGHTS_APP_CATALOG = 'customer_catalog';
    expect(telemetrySchema()).toEqual('');
  });
  ...
  it('recovers a live destination when the artifact variable is absent', async () => {
 ...
 *
 * The statement was verified against the live cmeg table by hand rather than
