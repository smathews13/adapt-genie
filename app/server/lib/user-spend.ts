...
export const USER_SPEND_RUNS_QUERY = `
  ...
  modern_resources AS (
    ...
    WHERE resource->>'tool' IN ('data_genie', 'dictionary_genie')
  ...
  legacy_resources AS (
    ...
    WHERE jsonb_array_length(
      ...
      AND stage->>'id' ~ '(data_genie|dictionary_genie)$'
...
/**
 * Durable evidence that a person actually used this ADAPT deployment.
 * Roster, ACL, and role tables are deliberately absent.
 *
...
export function readUserInteractionEvidence(rows: readonly Record<string, unknown>[]): UserInteractionEvidence[] {
  ...
  for (const row of rows) {
    ...
    if (
      !email ||
      !isUserMonitoringIdentity(email) ||
...
function tileTotal(tile: CostTile | undefined, unit: CostBudgetUnit, days: number): number | null {
  ...
  const value =
    tile.id === 'serving-endpoint'
      ? unit === 'USD'
        ? tile.marginalAmount === undefined
          ? tile.amount
          : tile.marginalAmount
        : tile.marginalDbus === undefined
          ? (tile.dbus ?? null)
          : tile.marginalDbus
      : unit === 'USD'
        ? tile.amount
...
export function buildSpendByUser(input: {
  ...
  const questionComponentIds = new Set(['serving-endpoint', 'foundation-model', 'sql-warehouse']);
  ...
  for (const id of componentIds) {
    ...
    for (const unit of ['USD', 'DBU'] as const) {
      ...
      if (id === 'serving-endpoint') {
        ...
        weights = new Map(input.queryUsers.map((row) => [row.email, row.adaptExecutionMs]));
  ...
  const profiles: UserSpendProfile[] = users.map((email) => {
    ...
    const questionComponents = components.filter((component) => questionComponentIds.has(component.id));
    const usdParts = questionComponents.map((component) => component.usd);
    const dbuParts = questionComponents.map((component) => component.dbu);
    ...
    const partial = unattributed.some(
      (component) =>
        questionComponentIds.has(component.id) &&
        ((component.usd.amount ?? 0) !== 0 || (component.dbu.amount ?? 0) !== 0)
    );
    return {
      email,
      total: {
  ...
  const reconcile = (unit: CostBudgetUnit) => {
    ...
    const componentTotals = componentIds
      .filter((id) => questionComponentIds.has(id))
    ...
    const missing = unattributed
      .filter((component) => questionComponentIds.has(component.id))
...
/**
 * People this page should list. Workspace usernames often omit `@domain`
 * (`sam.mathews`) while still being the same person as an email on the roster.
 * Service principal ids have no dot, so they stay out.
 */
function isUserMonitoringIdentity(email: string): boolean {
  if (!email) return false;
  if (email.includes('@')) return true;
  return email.includes('.') && /[a-z]/i.test(email);
}
...
export function buildUserMonitoringPage(input: {
  ...
  const emails = [
    ...new Set([
      ...input.roles.keys(),
      ...interactions.keys(),
      ...input.runs.map((row) => row.email),
      ...input.activity.map((row) => row.email),
    ]),
  ];
  ...
  const rosterRows: UserMonitoringRow[] = emails
    .filter((email) => isUserMonitoringIdentity(email))
    .map((email) => {
  ...
  const countedRows = rosterRows.filter(
    (row) => (!search || row.email.includes(search)) && (!input.role || row.role === input.role)
  );
  ...
  const rows = authorizedRows.sort((left, right) => {
