...
describe('billing SQL contract', () => {
  ...
  it('measures exact untagged resources', () => {
    expect(query?.statement).toContain(`u.custom_tags['${BILLING_TAG_KEY}'] = 'adapt'`);
    expect(query?.statement).toContain(
...
describe('price join golden outputs', () => {
  ...
  it('reconciles the displayed component totals to Total app spend exactly once', () => {
    ...
    expect(summary.amount).toBeCloseTo(64 + 6 + 11.4, 8);
  });
...
describe('coverage, shared meters, and Genie', () => {
  ...
  it('withholds a warehouse meter until Query History proves the ADAPT share', () => {
