...
describe('the Run Explorer’s two columns', () => {
  ...
  it('gives the detail column a real pane without restoring the agent map', () => {
    // Tabs plus the view they switch sit on one sheet. ADAPT retains Overview,
    // Timeline, and Details; the retired agent-map tab must not return.
    const pane = rule(RUNS, '.run-detail');
    expect(pane).toContain('height: var(--run-explorer-pane-block-size)');
    expect(EXPLORER).toContain('className="run-detail ast-surface-primary"');
...
describe('what amber is allowed to be on the evaluation screen', () => {
  /*
   * The ADAPT amber, which is a different colour rather than a rename. It was
...
describe('the qualification ledger and the region below it', () => {
  // The three token names below are the ADAPT compatibility spellings. The values did not
