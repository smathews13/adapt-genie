...
const GRANT = 'GRANT SELECT ON TABLE `<your_catalog>`.`<your_schema>`.`silver_purchases` ' + 'TO `adapt-serving-sp`;';
...
test('reports a healthy dependency set', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /^Connected as/ })).toContainText('adapt-serving-sp');
});

test('shows the literal GRANT for an unreachable table', async ({ page }) => {
  await servePreflight(
    page,
...
test('treats a 503 report as a report, not as a dead page', async ({ page }) => {
      ...
      checks: [
        {
          ...
          label: 'Agent endpoint · adapt-orchestrator',
  ...
  await expect(page.getByLabel('Fix for Agent endpoint · adapt-orchestrator')).toHaveText('uv run python log_model.py');
});
