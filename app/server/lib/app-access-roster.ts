/**
 * Databricks App admission, read under the signed-in super administrator.
 *
 * ADAPT roles and the Databricks App ACL are deliberately different
 * authorities: Lakebase decides what somebody may do after admission; the App
 * ACL decides whether Databricks admits them at all.
 */
...
/**
 * Draw the human roster from explicit App users and overlay ADAPT roles.
 *
 * Groups and service principals stay separate because Databricks does not
 * expand their membership in the App ACL response.
 */
export function alignRosterWithAppAccess(payload: RosterPayload, snapshot: AppAccessSnapshot): RosterPayload {
  ...
  return {
    ...
    appAccessMessage: 'Databricks App permissions determine membership. ADAPT determines each member’s app role.',
