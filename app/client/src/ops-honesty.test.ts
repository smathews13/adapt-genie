...
describe('the quality of a number', () => {
  ...
  it('labels only a daily rate because the page control supplies the range context', () => {
    // A fixed-cost endpoint is billed by the hour whether anything queries
...
describe('an empty cost block', () => {
  ...
  it('never exposes unmatched workspace Genie usage as a Cost card', () => {
    ...
    expect(displayed.map((item) => item.id)).toEqual(['genie:data']);
    expect(displayed.some((item) => item.id === 'genie:unattributed')).toBe(false);
    expect(displayed.some((item) => item.id === 'genie:dictionary')).toBe(false);
  });
});
...
describe('the telemetry notice', () => {
  const input = { variable: 'ADAPT_TELEMETRY_SCHEMA', table: 'a_catalog.a_schema.otel_logs', reason: '' };
...
describe('the mark on a cost tile', () => {
  it('names a product for every ADAPT-visible billed component', () => {
...
describe('which Ops resources open in Databricks', () => {
  it('opens the cost tiles that name a workspace object', () => {
    ...
    expect(costTileWorkspaceObject({ id: 'app-compute', resourceId: 'adapt' })).toEqual({
      kind: 'app',
      name: 'adapt',
    });
  });

  it('opens the configured Genie space', () => {
  ...
  it('does not turn a workspace id into a link', () => {
  ...
  it('builds the Databricks URL for the configured Genie space', () => {
  ...
  it('opens Health identifiers the Architecture page already knows how to open', () => {
    ...
    expect(healthResourceObject({ kind: 'vector-index', name: 'a.b.c' })).toBeNull();
  });

  it('does not invent unsupported resource URLs', () => {
