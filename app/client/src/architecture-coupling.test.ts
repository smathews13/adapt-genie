...
describe('a node reports what the shared derivation says, and nothing else', () => {
  it('maps the shared detailed status to Architecture’s binary operational state', () => {
    ...
    for (const node of ARCHITECTURE_NODES) {
      ...
      const expectedLabel =
        expected === 'reachable' || (expected === 'not-checked' && reading.row.configured.trim())
          ? 'Connected'
          : expected === 'blocked'
            ? 'Disconnected'
            : expected === 'not-checked'
              ...
              : expected === 'nothing-to-reach'
                ? 'Not configured'
                : 'Unavailable';
      expect(nodeReport(node, reading).label, node.id).toBe(expectedLabel);
    }
  });
...
describe('the page has no way to derive a status for itself', () => {
  ...
  it('owns Architecture labels without copying Connections verdict words', () => {
...
describe('every node that names a dependency names a real one', () => {
   ...
   */
  it('draws the warehouse, Genie, endpoints, Lakebase and the experiment', () => {
