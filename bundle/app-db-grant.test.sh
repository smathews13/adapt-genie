...
REPO="$(cd "$HERE/.." && pwd)"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/adapt-app-db-grant-test.XXXXXX")"
...
mkdir -p "$WORK/bin" "$WORK/repo/bundle" "$WORK/repo/app/scripts"
cp "$HERE/_lib.sh" "$HERE/app-db-grant.sh" "$WORK/repo/bundle/"
cp "$REPO/databricks.yml" "$WORK/repo/databricks.yml"
touch "$WORK/repo/app/scripts/grant-app-db-access.mjs"
