...
import { executionToken, servingIdentityFields } from '../lib/execution-credential';
import {
...
/** Schema name for ownership guards; resolved from a compatibility environment identifier. */
export { APP_SCHEMA };
...
const ResourceCallSchema = z.looseObject({
  kind: z.enum(['genie-space']),
  id: z.string(),
  tool: z.enum(['data_genie', 'dictionary_genie']),
...
/**
 * Every answered turn is a run, whether it came from ADAPT chat or the Benchmark Lab.
...
async function callerReadsSharedConversations(
  ...
  await rolesReady?.();
  // ADAPT: administrators and super administrators always read every
  // conversation in the Ask rail, exactly as they already read every run in Run
  // Explorer (see `callerReadsEveryRun`). The role is resolved from the
  // authoritative Lakebase roster and an unreadable roster resolves to consumer,
  // so a storage failure narrows this to the caller rather than widening it.
  // Consumers stay owner-scoped; writes remain owner-only regardless.
  const { role } = await resolveRole(store, email);
  return opensAdminSurfaces(role);
}
...
async function cancelTaggedWarehouseQueries(input: {
  ...
  try {
    ...
    return {
      result: await cancelAdaptWarehouseQueries({
        warehouseId,
        scope: input.scope,
        transport,
      }),
...
export function conversationRunTrace(row: Record<string, unknown>, experimentId: string): RunTrace {
  ...
  return normalizeReaderAnswer({
    ...
    figures: answer.success ? answer.data.figures : [],
 ...
 *
 * The environment name is an executable compatibility identifier retained for
 ...
 * An explicit false on the compatibility environment identifier -- correctly, because a
 ...
 * cmeg target sets it to "true" so its seeded evaluation conversations are
 ...
 * and applied once.
 *
 * Set the compatibility migration variable to `verify` once the deploy step is wired and
...
export function setupInsightsRoutes(
  ...
  appkit.server.extend((app) => {
    ...
    app.use(requireAdmin(appkit.lakebase, userEmail, readGroupRole));
    ...
    app.use(requireSuperAdmin(appkit.lakebase, userEmail, readGroupRole));
    ...
    app.get('/api/identity', async (req, res) => {
      const signedInAs = userEmail(req);
      const [role, identityMetadata, roster] = await Promise.all([
        rolePayload(appkit.lakebase, signedInAs, readGroupRole),
      ...
      res.json({
        ...
        organization,
        organizations,
        // Administrators and super administrators always see everyone's
        // conversations in ADAPT, so the browser is told the rail is shared for
        // them and owner-scoped for a consumer. Derived beside the authoritative
        // stored role, it never advertises other people to a non-admin session.
        sharedConversationRail: opensAdminSurfaces(role.role),
        ...role,
        identityMetadata,
      });
    });
    ...
    /**
     * Add ADAPT's load-bearing scopes and workspace browse scope to this app
     * as the signed-in user.
     *
     * This is intentionally beside the identity route rather than under an
     * ADAPT admin prefix. CAN MANAGE on the Databricks App is the authority
    ...
    app.post('/api/app-user-api-scopes', async (req, res) => {
      ...
      const outcome = await allowAdaptUserApiScopes({
        host,
        appName,
        userToken,
    ...
    app.get('/api/conversations', async (req, res) => {
      ...
      if (!readsShared) {
        // Forged filters can never widen this set: the SQL owner predicate ran
        // before this response was shaped.
        res.json(rows);
        return;
      }

      // The full authorized rail stays in the payload so owner options and
      // counts do not collapse when one filter is selected.
    ...
    app.get('/api/saved-queries', async (req, res) => {
      await respondWithStored(
        appkit,
        res,
        'GET /api/saved-queries',
        `SELECT id, question, created_at
           FROM ${APP_SCHEMA}.saved_queries
          WHERE user_email = $1
          ORDER BY created_at DESC
          LIMIT 100`,
        [userEmail(req)]
    ...
    app.post('/api/saved-queries', async (req, res) => {
      ...
      const questionValue = body && typeof body === 'object' ? (body as { question?: unknown }).question : undefined;
      const raw = typeof questionValue === 'string' ? questionValue.trim() : '';
      if (!raw) {
        res.status(400).json({ error: 'question_required', message: 'A saved query needs a question to keep.' });
        return;
      }
      const question = raw.slice(0, 2000);
      try {
        const result = await appkit.lakebase.query(
          `INSERT INTO ${APP_SCHEMA}.saved_queries (id, user_email, question)
           VALUES ($1, $2, $3)
           RETURNING id, question, created_at`,
          [`sq-${crypto.randomUUID()}`, email, question]
        );
        res.status(201).json(result.rows[0]);
      } catch (error) {
        console.warn(`[lakebase] POST /api/saved-queries failed: ${(error as Error).message}`);
        res.status(503).json({
          error: 'saved_query_not_stored',
          message: 'This question could not be saved right now. Try again shortly.',
        });
      }
    });
    ...
    app.delete('/api/saved-queries/:id', async (req, res) => {
      ...
      try {
        const result = await appkit.lakebase.query(
          `DELETE FROM ${APP_SCHEMA}.saved_queries WHERE id = $1 AND user_email = $2 RETURNING id`,
          [req.params.id, email]
        );
        if (result.rows.length === 0) {
          res.status(404).json({ error: 'saved_query_not_found', id: req.params.id });
          return;
        }
        res.status(200).json({ id: req.params.id });
      } catch (error) {
        console.warn(`[lakebase] DELETE /api/saved-queries/:id failed: ${(error as Error).message}`);
        res.status(503).json({
          error: 'saved_query_delete_failed',
          message: 'This saved query could not be removed right now. Try again shortly.',
        });
      }
    });
    ...
    app.post('/api/runs/:id/cancel', async (req, res) => {
      ...
      await recordAdminAction(appkit.lakebase, {
        ...
        subject: runIds.join(','),
        detail: `${actor} stopped their own active ADAPT run(s): ${runIds.join(', ')}.`,
      });
      res.json({
    ...
    app.post('/api/admin/runs/cancel-all', async (req, res) => {
      ...
      await recordAdminAction(appkit.lakebase, {
        ...
        subject: 'active-adapt-runs',
        detail: `${actor} stopped a one-time snapshot of ${runIds.length} active ADAPT run(s).`,
      });
      res.json({
        targeted: runIds.length,
        cancelled: runIds.length,
    ...
    async function executeGovernedAsk(
      ...
      const ownershipPromise = readStored(
        appkit,
      ...
      try {
        ...
        try {
          ...
          if (plan) {
            ...
            reply.json({ ...planResponse, runId: admission.run?.runId ?? identity.requestId });
            return;
          }
          ...
          if (clarification) {
            ...
            reply.json({ ...clarificationResponse, runId: admission.run?.runId ?? identity.requestId });
            return;
          }
        ...
        const settlement = runStored
          ? ({ to: 'SUCCEEDED', traceId: disclosed.trace.id, messageId: disclosed.id } as const)
          : ({ to: 'PERSISTENCE_FAILED', code: 'PERSISTENCE_UNAVAILABLE', traceId: disclosed.trace.id } as const);
    ...
    app.post('/api/benchmarks/run', async (req, res) => {
       ...
       * path at all. A version logged with the compatibility authorization
       * variable has none, so those invocations would have come back refused with
