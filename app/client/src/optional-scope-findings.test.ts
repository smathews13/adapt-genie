 ...
 *
 * The live cmeg screen carried four blocks under that heading and three of them
...
describe('the split the panel is drawn from', () => {
   ...
   * fourteen optional shortfalls (catalog, schema, and twelve tables) and one
   * finding (the warehouse).
   */
  const split = splitOptionalScopeFindings([CATALOG, SCHEMA, ...TABLES, WAREHOUSE]);
  ...
  it('names each optional permission once, in the order it was met', () => {
    expect(split.optional.scopes).toEqual(['catalog.catalogs:read', 'catalog.schemas:read', 'catalog.tables:read']);
  ...
  it('keeps both halves in the order the report produced them', () => {
    const ordered = splitOptionalScopeFindings([WAREHOUSE, CATALOG, SCHEMA]);
