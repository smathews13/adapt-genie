...
/**
 * The cut the app draws: official geometry, ADAPT fills.
 *
...
describe('every outbound MLflow action carries the MLflow mark inside its anchor', () => {
  ...
  it('puts the wordmark inside remaining MLflow anchors, with theme-following ink', () => {
    const anchors = [
...
describe('the plan card marks the steps that are a call on a product', () => {
  ...
  it('keeps the agent’s own robot on the card header rather than a product logo', () => {
    // The coral mark is ADAPT's identity, not Databricks Apps'. It is the one
...
describe('Run Explorer Timeline uses the notebook vocabulary', () => {
  const notebookViz = {
    ...
    stages: [
      ...
      },
      {
        ...
        status: 'complete' as const,
        calls: 1,
        input: JSON.stringify({ sql: "SELECT COUNT(CASE WHEN title = 'NBA 2K23' THEN 1 END) AS games FROM gold" }),
        output: '',
      },
      {
        id: 'step-3',
  ...
  it('draws kind pills, matching bars, notebook event names, and kind badges', () => {
    const drawn = renderToStaticMarkup(
      <TraceTimeline variant="explorer" trace={notebookViz} question="How many NBA games do we have?" />
    );
    ...
    expect(drawn).toContain('describe_table cdp_share_prod.take_two.gold_title_daily');
  ...
  it('leaves Ask on stakeholder names, product marks, and the tile roll-up', () => {
    const drawn = renderToStaticMarkup(<TraceTimeline trace={notebookViz} question="How many NBA games do we have?" />);
