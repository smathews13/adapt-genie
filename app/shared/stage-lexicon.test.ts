...
import {
  LEGACY_ANALYSIS_STAGE_ID,
  ORCHESTRATOR_ANALYSIS_TASK,
  projectReaderStage,
  projectToolOutput,
} from './stage-lexicon';
...
describe('reader-facing stage lexicon', () => {
  it('maps the historical analysis envelope to neutral Orchestrator copy', () => {
    ...
    expect(stage.name).toBe('Orchestrator data analysis');
    expect(stage.input).toBe(ORCHESTRATOR_ANALYSIS_TASK);
    expect(stage.output).toBe('Completed governed data analysis with Genie.');
