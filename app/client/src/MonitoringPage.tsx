 ...
 * a question puts ADAPT Ask's `AnswerCard` in a centered modal over that list,
 ...
 *  - The answer body, which is ADAPT Ask's own `AnswerCard`. An admin reading
...
export function FilterRow({
  ...
  return (
    <div className="monitoring-filters">
      ...
      />
      {/* The ADAPT-specific one, and the one worth more than the others: every
...
/**
 * The stored answer as a shape ADAPT Ask's own card will render, or null.
 *
...
export function PersonSpend({
  ...
  const averageDailyBase =
    authoritative?.averageDaily.state === 'value' ? authoritative.averageDaily : core.averageDaily;
  const averageDaily =
    averageDailyBase.state === 'value'
      ? { ...averageDailyBase, subtitle: '' }
      : { ...averageDailyBase, subtitle: 'Average not available yet' };
  const appShare =
...
function ProfileIdentityBadges({ role }: { role: Role }) {
  return (
        ...
        <div className="user-profile-modal-header-content">
          ...
          <div className="user-profile-modal-user">
            <div className="user-profile-modal-identity-row">
              ...
              <ProfileIdentityBadges role={panel.role} />
            </div>
...
export function PersonPanelShell({
  ...
  onBack?: () => void;
  backLabel?: string;
  identitySeed?: Pick<UserMonitoringRow, 'role' | 'organization'> | null;
}) {
  if (state.status === 'ready') {
    return (
      <PersonPanel
      ...
      <header className="user-profile-modal-header">
        <div className="user-profile-modal-header-content">
          ...
          <div className="user-profile-modal-identity-row">
            ...
            {identitySeed ? <ProfileIdentityBadges role={identitySeed.role} /> : null}
          </div>
...
export function UserMonitoringPanel({
  ...
  const changed = Boolean(browser.search || browser.role || browser.organizations.length > 0);
            ...
            <div className="monitoring-users-list" role="list" aria-label="Users ordered by attributable spend">
              ...
              {payload.users.map((row) => (
                ...
                >
                  ...
                  <span className="monitoring-user-organization">
                    <span className="monitoring-users-mobile-label">Organization</span>
...
export function MonitoringPage() {
  ...
  const [profileIdentitySeed, setProfileIdentitySeed] = useState<
    (Pick<UserMonitoringRow, 'email' | 'role' | 'organization'> & { scope: string }) | null
  >(null);
  ...
  const userBrowserKey =
    userBrowser.open && !drawer.person
      ? `users|v${USER_MONITORING_SCHEMA_REVISION}|${identityEpoch}|${userWindow.from}|${userWindow.to}|${userBrowser.unit}|${userBrowser.search}|${userBrowser.role}|${userBrowser.organizations.join(',')}|${userBrowser.cursor}|25`
      : '';
  ...
  const updateUserBrowser = (
    updates: Partial<Pick<typeof userControls, 'search' | 'role' | 'organizations' | 'unit' | 'range'>>
  ) => {
  ...
  return (
    <div className="page-shell monitoring-page">
      ...
      <MonitoringBody
        ...
        now={now}
        onOpen={open}
        onOpenPerson={showsAdminSurfaces(outlet?.role.state ?? 'failed') ? openPersonPanel : undefined}
        onOpenUsers={showsAdminSurfaces(outlet?.role.state ?? 'failed') ? openUsers : undefined}
      ...
      {userBrowser.open && !drawer.person && !drawer.question ? (
        <UserMonitoringPanel
          ...
          onClear={() => updateUserBrowser({ search: '', role: '', organizations: [] })}
          onNext={(cursor) =>
            setUserControls((current) => {
