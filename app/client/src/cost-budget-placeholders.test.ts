...
describe('cost budget placeholders', () => {
  ...
  it('keeps a successful zero optional-resource read as an editable zero baseline', () => {
    const vector = tile({
      id: 'legacy-addon',
      amount: 0,
      dbus: 0,
  ...
  it('uses mutually exclusive SQL and Genie allocations, each tile basis, and the selected unit', () => {
    const payload = {
      ...
      tiles: [
        ...
        tile({ id: 'legacy-addon', amount: 3, dbus: 5, basis: 'per-day' }),
    ...
    const refreshed = {
      ...payload,
      tiles: payload.tiles.map((item) => (item.id === 'legacy-addon' ? { ...item, amount: 10, dbus: 8 } : item)),
    };
