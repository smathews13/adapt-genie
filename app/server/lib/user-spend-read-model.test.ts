...
describe('fast read semantics', () => {
  ...
  it('scopes non-admins in SQL and preserves zero separately from missing values', async () => {
    ...
    expect(READ_USER_SPEND_SUMMARY_QUERY).toContain('WHERE NOT $13::boolean');
    expect(READ_USER_SPEND_SUMMARY_QUERY).toContain('AND ($5::boolean OR lower(roster.email) = lower($6))');
    expect(READ_USER_SPEND_SUMMARY_QUERY).toContain('FROM jsonb_to_recordset($14::jsonb)');
    ...
    expect(READ_USER_SPEND_SUMMARY_QUERY).toContain('cardinality($9::text[]) = 0');
