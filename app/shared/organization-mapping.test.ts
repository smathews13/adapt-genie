...
describe('organization mapping', () => {
  ...
  it.each([
    ...
    ['producer@take2games.com', 'take-two-interactive'],
    ['artist@2k.com', '2k'],
    ['designer@rockstargames.com', 'rockstar-games'],
    ['developer@rockstarlondon.com', 'rockstar-games'],
  ...
  it('derives compact digit-aware marks from neutral registrable labels', () => {
    ...
    expect(organizationMonogramForDomain('2k.example')).toBe('2K');
  ...
  it('decodes counted filter options without weakening mapping validation', () => {
    const option = { ...organizationsForEmails(['one@databricks.com'])[0], count: 4 };
