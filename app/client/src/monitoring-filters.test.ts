...
describe('the User Monitoring modal lives in the URL', () => {
  ...
  it('moves browser to profile and back without losing search, role, organization, unit, or cursor', () => {
    const browser =
      'users=1&userSearch=ada&userRole=admin&userOrganization=2k%2Crockstar-games&userUnit=USD&userCursor=next';
    ...
    expect(returned.get('userOrganization')).toBe('2k,rockstar-games');
    expect(returned.get('userCursor')).toBe('next');
    expect(userBrowserFromParams(returned).organizations).toEqual(['2k', 'rockstar-games']);
  });
  ...
  it('round-trips organization multiselect filters and clears stale cursors', () => {
    const written = params(
      withUserBrowserFilters('users=1&range=7d&userCursor=stale', {
        search: 'sam',
        role: 'admin',
        organizations: ['take-two-interactive', '2k'],
        unit: 'DBU',
      })
    );
  ...
  it('closes the whole modal without clearing Monitoring filters or period', () => {
    const closed = params(
      closedUserMonitoring('range=24h&outcome=partial&users=1&who=a%40b.test&userOrganization=2k&userUnit=DBU')
    );
