...
describe('step attribution and reconciliation', () => {
  it('aggregates multiple calls and genuine retries while deduplicating exported copies by span id', () => {
    const first = span('call-1', 'orchestrator.llm.step-1', {
      input_tokens: 100,
      output_tokens: 10,
      total_tokens: 110,
      cache_read_input_tokens: 25,
    });
    const retry = span('call-2', 'orchestrator.llm.step-1', {
      input_tokens: 120,
      output_tokens: 12,
      total_tokens: 132,
  ...
  it('matches the redacted CMEG shape and excludes its aggregate orchestrator span', () => {
    const cmeg = [
      span(
        'aggregate',
        'orchestrator.loop',
