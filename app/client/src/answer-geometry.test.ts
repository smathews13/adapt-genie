...
describe('the caveats change how loudly they are said and nothing else', () => {
  ...
  it('draws Caveats as a comfortably spaced surface box', () => {
    ...
    // that could not be read as type at 11px; ADAPT's warning family is
...
describe('the plan card says which state it is in, in the colour that state means', () => {
  it('waits in the warning family and resolves to the positive one', () => {
    ...
    // #FFAB00 could not be read as type at 11px, and ADAPT's #8A6A38 on
...
describe('an entity in the prose is a link that says where it goes', () => {
  it('uses the chrome-yellow table family with a pinned 1px dotted rule', () => {
    ...
    expect(links).toContain(
      'entity-table entity-table-link font-medium underline decoration-dotted decoration-1 underline-offset-2'
    );
    expect(ANSWER_CSS).toMatch(/\.entity-table-link\s*\{[^}]*color:\s*var\(--ast-entity-table-fg\)/);
  });
...
describe('the closing note and the feedback row', () => {
  ...
  it('keeps the shared AI caveat without routine permission narration', () => {
    expect(CARD).toContain('<AIAnalysisCaveat className="ai-note" />');
    expect(CARD).not.toContain('dataAccessDisclosure');
    expect(CARD).not.toContain('data-access-note');
  });
...
describe('the card holds text it did not write', () => {
  ...
  it('keeps sources as one bullet list rather than a nested card', () => {
    ...
    // Sources and Caveats share one type size so the two support panels read as
    // one family; the Sources list now sits on the same 13px rung Caveats uses.
    expect(ruleFor(BODY_CSS, '.source-list {')).toContain('font-size: var(--ast-fs-13)');
  });
...
describe('the two cards do not reach past the token block', () => {
  ...
  it('spends no token that is not the ADAPT palette’s', () => {
    ...
    // ADAPT tokens do not own. A denylist only ever finds the tokens whoever
    ...
    for (const [name, source] of [
      ...
      expect(foreign, `${name} draws from the ADAPT tokens`).toEqual([]);
    }
  });
