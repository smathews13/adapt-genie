...
export function SettingsPage({
  ...
  const form =
    active === 'appearance'
      ...
      : active === 'watchlist'
        ? WATCHLIST_SETTINGS_FORM_ID
        : active === 'starterQuestions'
          ? ASK_STARTERS_SETTINGS_FORM_ID
          : active === 'egress'
            ? EGRESS_SETTINGS_FORM_ID
            : active === 'experimental' && BENCHMARK_LAB_ENABLED
              ? BENCHMARK_SETTINGS_FORM_ID
              : undefined;
  const experimentalSaveOnly = active === 'experimental' && !BENCHMARK_LAB_ENABLED;
   ...
   */
  const saveDisabled = settingsSaveDisabled(saving, dirtyCount, Boolean(form) || experimentalSaveOnly);
    ...
    >
              ...
              <div className="settings-pane">
                ...
                <table className="exp-feature-table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <ExperimentalFeatureName kind="genie-code-mcp">
                          Genie Code / Genie MCP
                      ...
                      <td className="exp-feature-status">
                        <ExperimentalStatus on={showsGenieCodeMcp(draftFeatures)} />
                      </td>
                      <td className="exp-feature-control">
                        <div className="exp-feature-control-inner">
                          <Switch
                            ...
                            aria-label="Enable Genie Code and Genie MCP"
                          />
                        </div>
                      </td>
                    </tr>
      ...
      <footer className="settings-modal-footer">
        ...
        <div className="settings-footer-actions">
          ...
          <Button
            type={experimentalSaveOnly ? 'button' : 'submit'}
            data-variant="primary"
            className="settings-save"
            form={experimentalSaveOnly ? undefined : form}
            ...
            onClick={() => {
              setPressed(true);
              if (!experimentalSaveOnly) return;
              void (async () => {
                setSaveState({ kind: 'saving' });
                try {
                  await commitExperimental();
                  setSaveState({ kind: 'saved', count: dirtyCount });
                } catch (error) {
                  setSaveState({
                    kind: 'failed',
                    message: error instanceof Error ? error.message : 'The settings were not saved.',
                  });
                }
              })();
            }}
          >
            <AdaptBusyButtonContent busy={saving} label="Save" busyLabel="Saving" />
          </Button>
        </div>
      </footer>
      {discardOpen ? (
