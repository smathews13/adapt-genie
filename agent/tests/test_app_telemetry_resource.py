...
That already happened once in this repo with an unused search endpoint, which
billed by the hour for five days while nothing queried it.
...
APP_RESOURCE = ROOT / "resources" / "adapt_app.app.yml"
TELEMETRY_RESOURCE = ROOT / "resources" / "adapt_telemetry.schema.yml"
...
def test_the_base_app_omits_empty_destinations():
    ...
    app = app_document()["resources"]["apps"]["adapt_app"]
...
def test_the_telemetry_schema_has_an_app_owned_default():
    assert app_document()["variables"][SCHEMA_VAR]["default"] == "adapt_telemetry"
...
def test_no_telemetry_is_declared_at_the_top_level():
    ...
    for name in (DESTINATIONS_VAR, SCHEMA_VAR):
        assert name not in top_level, (
            f"{name} belongs in resources/adapt_app.app.yml beside the "
...
def test_the_server_is_told_where_the_telemetry_tables_are():
    ...
    assert SCHEMA_ENV in app_yaml_env(), (
        f"{SCHEMA_ENV} is missing from the companion app manifest, so the "
...
def test_the_telemetry_variable_ships_empty():
    ...
    Same rule as the neighboring compatibility variables: this tree is
    published, so the value is resolved at release time for the target being
    released rather than written down here. Empty is read by the server as
    telemetry being off, which is one of its ordinary states and not an error.
    """
