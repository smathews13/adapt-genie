...
#
# THE VOCABULARY FOR THE WHOLE DEPLOYMENT, not only for this module. The
# companion app's settings registry uses these same five strings and has a test
# that parses this file to prove it uses no other, because both ends read them
# to decide whether to offer an edit box for a value that a form cannot actually
# change.
# ---------------------------------------------------------------------------
...
def _missing_message(missing: list[str], profile_name: str) -> str:
    ...
    return (
        f"Missing required ADAPT configuration: {', '.join(missing)}."
        f"{asked_for}\n\n"
