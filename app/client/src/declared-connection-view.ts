 ...
 * chose SQL warehouse, Volume, or Model endpoint got the
...
export const ADDABLE_KINDS: ReadonlyArray<{
  ...
  { id: 'metric-view', kind: 'unity-catalog', label: 'Metric view', browse: 'table' },
  ...
  { id: 'agent-endpoint', kind: 'agent', label: 'Agent endpoint', browse: 'serving-endpoint' },
  { id: 'mcp-server', kind: 'agent', label: 'MCP server', browse: null },
...
export function isDeclaredUnityCatalogConnection(connection: {
  ...
  return (
    ...
    connection.resourceType === 'metric-view' ||
...
export function pickerForAddKind(kindId: string): AssetPickerSpec | null {
  ...
  return chosen?.browse ? ADD_CONNECTION_PICKERS[chosen.browse] : null;
}
