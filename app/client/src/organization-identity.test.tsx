...
describe('organization identity assets', () => {
  it('renders each canonical local mark with the manifest label and stable id', () => {
    for (const organization of ORGANIZATION_MANIFEST) {
      ...
      const expectedAsset = ORGANIZATION_LOGO_IMAGES[organization.logoKey] ?? ORGANIZATION_LOGOS[organization.logoKey];
      expect(markup).toContain(expectedAsset);
  ...
  it('renders Databricks identities as the naked official symbol', () => {
    const markup = renderToStaticMarkup(
      <OrganizationAvatar organization={organizationForEmail('sam.mathews@databricks.com')} />
    );
    expect(markup).toContain('roster-organization-mark--databricks');
    expect(markup).toContain(ORGANIZATION_LOGOS.databricks);
    expect(ORGANIZATION_CSS).toMatch(
      /\.roster-organization-mark--databricks[^}]*border:\s*0[^}]*background:\s*transparent/s
    );
  });
...
describe('organization user badges', () => {
  it.each([
    ['sam.mathews@databricks.com', 'databricks', 'sam.mathews'],
    ['producer@take2games.com', 'take-two-interactive', 'producer'],
    ['artist@2k.com', '2k', 'artist'],
    ['designer@rockstargames.com', 'rockstar-games', 'designer'],
