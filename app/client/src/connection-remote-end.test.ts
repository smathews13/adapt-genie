...
describe('an unset value is not an unchecked one', () => {
  // An unset optional AI Gateway route is valid. "Not checked" over an empty
  // value would promise an answer nobody is coming with.
  it('reads an unset optional route as nothing to reach', () => {
    expect(statusOf('llm-gateway')).toBe('nothing-to-reach');
  });
...
describe('a check that ran outranks all of it', () => {
  ...
  it('counts a measured value as a remote end even with nothing configured', () => {
    expect(hasRemoteEnd(row('llm-gateway', { actual: 'a-route' }))).toBe(true);
  });

  it('keeps an authoritative checked row in the canonical list even when it is normally local configuration', () => {
    const payload: SettingsPayload = {
      resources: [row('experiment-id', { configured: '123' })],
      drift: [],
      status: 'ok',
      appBuildSha: '',
      modelBuildSha: '',
    ...
    const readings = readingsById(readConnections(payload, [check('experiment-id', 'ok')]));
    expect(readings.get('experiment-id')?.status).toBe('reachable');
  });
});
