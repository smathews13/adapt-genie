...
export async function deleteRuntimeSettingsDefaults(client: LakebaseReader): Promise<ResolvedRuntimeSettings> {
  await client.lakebase.query(`DELETE FROM ${RUNTIME_SETTINGS_TABLE} WHERE id = $1`, [KEY]);
  forgetRuntimeSettings();
  return { settings: DEFAULT_RUNTIME_SETTINGS, revision: 0, source: 'default', canReset: false };
}
