 ...
 * previous read, or apportioned by an invented ratio. Where a component cannot
 * be attributed, its tile says so and shows no number, because a component nobody could attribute
 ...
 * that does not name this deployment's own endpoint, warehouse, and app
 ...
 */
export const COST_COMPONENTS = ['serving-endpoint', 'sql-warehouse', 'genie', 'lakebase', 'app-compute'] as const;
 ...
 *
 * Every field may be empty, and empty is handled rather than defaulted.
 */
export interface CostIdentifiers {
  ...
  warehouseId: string;
  /** Bound Lakebase project/branch/endpoint path from `LAKEBASE_ENDPOINT`. */
  lakebaseEndpoint?: string;
  ...
  /**
   * Whether this app's own tag assignment is `system_billing=adapt`.
   *
...
export function readResourceActivityRows(rows: readonly Record<string, unknown>[]): ResourceActivity[] {
  const allowed = new Set<ResourceActivity['tileId']>(['genie:data', 'genie:dictionary']);
  ...
  for (const row of rows) {
    ...
    const calls = Number(row.attributed_calls);
 ...
 * number, which is honest. An admin asking what this thing costs must not be
 * shown a workspace-wide total as though it belonged to ADAPT.
 *
...
const TILED_PRODUCTS = new Set(['MODEL_SERVING', 'SQL', 'LAKEBASE', 'APPS']);
const PRODUCT_REASONS: Record<string, string> = {
  ...
  SQL: 'Warehouse billing rows are allocated only by complete ADAPT Query History execution-time share.',
  APPS: 'Measured by exact app name. App tag presence is a separate organizational signal.',
  GENIE: GENIE_LLM_UNAVAILABLE,
  LAKEBASE: `Measured from LAKEBASE billing rows carrying ${billingTagPair()}.`,
...
export function canAsk(component: CostComponent, ids: CostIdentifiers): boolean {
  ...
  // Lakebase billing identifies the product but not its project in
  // usage_metadata. The app-owned tag is the documented attribution key, and
  // the binding proves this deployment actually has Lakebase configured.
  if (component === 'lakebase') return Boolean(ids.lakebaseEndpoint);
 ...
 * that id is not a Genie space, so the tile has nothing to open.
 */
export function resourceIdFor(component: CostComponent, ids: CostIdentifiers): string {
  switch (component) {
    ...
    case 'app-compute':
      return ids.appName;
    case 'lakebase':
      return ids.lakebaseEndpoint ?? '';
    case 'genie':
      return '';
  }
}

function resourceKindFor(component: CostComponent, _ids: CostIdentifiers): CostResourceKind | '' {
  switch (component) {
    case 'serving-endpoint':
...
export function buildCostStatement(ids: CostIdentifiers, range: CostRange): CostStatement | null {
  ...
  for (const component of covered) {
    ...
    if (component !== 'lakebase') bind(marker, ids[matcher.parameter] as string, matcher.type);
    const predicate =
      component === 'lakebase'
        ? `u.workspace_id = :workspaceId AND u.custom_tags['${BILLING_TAG.key}'] = '${BILLING_TAG.value}'`
        : matcher.column === null
  ...
  if (canAsk('lakebase', ids)) {
    resourcePredicates.push(
      `(u.billing_origin_product = 'LAKEBASE' AND u.custom_tags['${BILLING_TAG.key}'] = '${BILLING_TAG.value}')`
    );
  }
  ...
  for (const [product, reason] of Object.entries(PRODUCT_REASONS)) {
    ...
    if (product === 'MLFLOW' || product === 'LAKEBASE') {
      products.push({
        ...
        pricedRows: 0,
        unpricedRows: 0,
        tiled: TILED_PRODUCTS.has(product),
        reason,
      });
    }
  }
 ...
 * after it.
 *
 * `variable` is '' where there is genuinely nothing to set. A remedy a reader
...
> = {
  ...
  lakebase: {
    label: 'Lakebase',
    quality: 'real',
    population: 'Tagged ADAPT project',
    basis: 'total-in-range',
...
export function buildTiles(
  ...
  _resourceActivity: readonly ResourceActivity[] = [],
  ...
  for (const component of COST_COMPONENTS) {
    ...
    const tile = componentTile(component, ids, byComponent, warehouseAttribution);
    tiles.push(
...
function componentTile(
  ...
  const evidence = {
    billingRows,
    astrolabeQueries: component === 'sql-warehouse' ? warehouseAttribution.adaptQueries : null,
  ...
  if (component === 'sql-warehouse') {
    ...
    return withMeta({
      ...base,
      amount: (amount * warehouseAttribution.adaptExecutionMs) / warehouseAttribution.totalExecutionMs,
      dbus:
...
const UNKNOWN_QUESTION_PARTS: readonly Omit<Extract<QuestionCostPart, { quality: 'unknown' }>, 'amount' | 'quality'>[] =
  [
    ...
    },
    {
      ...
      unavailable: 'Lakebase is billed as tagged project usage and cannot be joined to one query or run.',
    },
  ];
...
export function buildQuestionAttribution(
  ...
  const servingSpend =
    servingTile?.marginalAmount === undefined ? servingTile?.amount : servingTile.marginalAmount;
