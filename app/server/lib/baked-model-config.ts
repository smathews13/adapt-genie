 ...
 * on the model version as `model_config` — foundation model and declared tables
 * — and the app can read them the same way it
...
function implicitServedModel(endpoint: unknown): { entityName: string; version: string } {
  const config = asRecord(asRecord(endpoint).config);
  const entities = Array.isArray(config.served_entities)
    ? config.served_entities
    : Array.isArray(config.servedEntities)
      ? config.servedEntities
      : [];
  if (entities.length !== 1) return { entityName: '', version: '' };
  const entity = asRecord(entities[0]);
  return {
    entityName: text(entity.entity_name ?? entity.entityName ?? entity.model_name ?? entity.modelName),
    version: text(entity.entity_version ?? entity.entityVersion ?? entity.model_version ?? entity.modelVersion),
  };
}
...
export function parseModelConfigDocument(source: string): Record<string, unknown> {
  ...
  if (trimmed.startsWith('{')) {
    try {
      ...
      if ('llm_endpoint' in record || 'declared_manifest' in record) {
        return record;
      }
    } catch {
      return {};
...
/**
 * MLflow 3 Logged Model artifacts do not live under the source run. Follow the
 * logged-model artifact URI through Workspace Files to the served MLmodel.
 */
 ...
 *
 * Cached briefly by the exact served model version so a Connections refresh is
 * not three MLflow round trips. The endpoint is still read on every call:
 * promotion changes its route without changing its name, and stale model
 * configuration is worse than that one inexpensive control-plane read.
...
export async function readBakedModelConfig(
  ...
  try {
    const endpoint = await (input.readEndpoint ?? describeEndpoint)(endpointName);
    const parsed = parseServedModel(endpointName, endpoint);
    const served =
      parsed.entityName && parsed.version
        ? { entityName: parsed.entityName, version: parsed.version }
        : implicitServedModel(endpoint);
