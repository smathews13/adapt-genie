...
describe('app budget guard UI', () => {
  ...
  it('keeps every Cost budget mutation branded and exposes its busy state', () => {
    const source = readFileSync(new URL('./CostBudgets.tsx', import.meta.url), 'utf8');
    expect(source).toContain("aria-busy={busy === 'approve' || undefined}");
    expect(source).toContain("aria-busy={busy === 'revoke' || undefined}");
    expect(source).toContain('aria-busy={saving || undefined}');
    expect(source.match(/<AdaptBusyButtonContent/g)).toHaveLength(3);
    expect(source).not.toContain('<ConceptFlicker');
  });
