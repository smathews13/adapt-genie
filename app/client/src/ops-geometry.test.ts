...
describe('latency baseline filter states', () => {
  it('uses semantic color states without dim opacity or geometry changes', () => {
    ...
    expect(RULES).not.toContain(".ops-latency-trend-filter[aria-pressed='true']::before");
...
describe('the figures line up', () => {
  ...
  it('keeps the TREND pills the same 32px height as search and Refresh', () => {
    ...
    expect(RULES).not.toContain('.ops-latency-trend-filter::before');
  });

  it('keeps trend filters inline beside the title', () => {
    expect(rule('.ops-latency-head-controls')).toMatch(/flex-wrap:\s*nowrap/);
    const filters = rule('.ops-latency-trend-filters');
    expect(filters).toMatch(/display:\s*inline-flex/);
    expect(filters).toMatch(/flex:\s*0 0 auto/);
    expect(filters).toMatch(/flex-wrap:\s*nowrap/);
    expect(filters).not.toMatch(/padding|overflow-x|border-bottom/);
    expect(RULES).not.toContain('.ops-latency-toolbar');
  });
});
