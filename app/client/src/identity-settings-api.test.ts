...
import { HumanRosterError, writeGroupRoleMapping, writeHumanRoster } from './identity-settings-api';
...
describe('human roster API feedback', () => {
  ...
  it('posts an existing workspace group mapping through the super-admin route', async () => {
    const request = vi.fn(() => Promise.resolve(answer(PAYLOAD)));
    vi.stubGlobal('fetch', request);
    await expect(writeGroupRoleMapping('existing-team', 'admin')).resolves.toEqual(PAYLOAD);
    expect(request).toHaveBeenCalledWith(
      '/api/users/groups',
      expect.objectContaining({
        method: 'POST',
        credentials: 'same-origin',
        body: JSON.stringify({ groupName: 'existing-team', role: 'admin' }),
      })
    );
  });

  it('preserves the server explanation when a group mapping dependency is unavailable', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(() =>
        Promise.resolve(
    ...
    const failure = await writeGroupRoleMapping('existing-team', 'admin').catch((cause: unknown) => cause);

    expect(failure).toMatchObject({ kind: 'unavailable', status: 503 });
    expect((failure as Error).message).toBe('SCIM could not confirm that workspace group. No mapping was saved.');
  });
