...
/** A readable first name for ADAPT's empty-state greeting, when the identity has one. */
export function signedInFirstName(signedInAs: string | null | undefined): string | null {
  const value = signedInAs?.trim() ?? '';
  if (!value || value === IDENTITY_UNAVAILABLE || value === IDENTITY_RESOLVING) return null;
  const local = localPart(value);
  if (isOpaqueId(local)) return null;
  const first = local.split(SEPARATORS).find((token) => token.length > 0);
  if (!first) return null;
  const glyphs = Array.from(first);
  return glyphs[0].toLocaleUpperCase() + glyphs.slice(1).join('');
}
