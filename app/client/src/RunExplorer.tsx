 ...
 * the analysis envelope or SQL wrapper as an agent stage instead of a tool stage. */
/**
 ...
 * agent records a tree, not a list. A historical analysis envelope can span
 * the whole data-analysis phase, and inside it sit the `step-n` model turns and,
 * inside those, every tool call the model asked for. Adding the parent to its
 * own children charges the run for the parent span plus a second copy of
 * everything that happened during it, and the historical name matches the pattern
 * above, so it was always in the sum.
 *
 * So parents are dropped and only the innermost data stages are counted, which
 * is also the honest reading of the label: the parent span includes the model
...
export function RunExplorer() {
  ...
  // `?conversation=`, so clicking through from ADAPT Ask hid every other run in
