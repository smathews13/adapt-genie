...
const asAlice = { 'x-forwarded-email': 'alice@cmeg.example' };
...
describe('source security invariants', () => {
  it('derives shared reads from the authoritative role, and never from the caller alone', () => {
    ...
    // ADAPT reads the rail by role, exactly as Run Explorer does. The role comes
    // from the authoritative Lakebase roster, so an unreadable roster resolves to
    // consumer and narrows the read rather than widening it.
...
describe('what the rail reads', () => {
  it('scopes to the caller when the flag is unset', async () => {
    ...
    expect(read.params).toEqual(['alice@cmeg.example']);
  });
  ...
  it('keeps a consumer self-only when the flag is exactly true', async () => {
    ...
    try {
      expect(
        (
          await app.fetch('/api/conversations?owners=bob%40cmeg.example&owners=alice%40cmeg.example', {
            headers: asAlice,
          })
        ).status
      ).toBe(200);
  ...
  it('keeps a consumer message read self-only when the flag is exactly true', async () => {
    ...
    expect(read.params).toEqual(['conv-bob', 'alice@cmeg.example', 51]);
  ...
  it.each(['admin', 'super_admin'] as const)('lets a %s list and open shared conversations', async (role) => {
    ...
    try {
      await app.fetch('/api/conversations?owners=bob%40cmeg.example', { headers: asAlice });
      await app.fetch('/api/conversations/conv-bob/messages', { headers: asAlice });
    } finally {
      await app.close();
    }
    ...
    expect(messages.params).toEqual(['conv-bob', 'alice@cmeg.example', 51]);
  });
  ...
  it('fails closed when the authoritative role store cannot be read', async () => {
    ...
    try {
      expect((await app.fetch('/api/conversations?owners=bob%40cmeg.example', { headers: asAlice })).status).toBe(200);
    } finally {
      await app.close();
    }
...
describe('what the flag deliberately does not widen', () => {
    ...
    const app = await startApp({
      query(sql: string) {
        ...
        return /user_email\s+FROM player_insights\.conversations/i.test(sql)
          ? Promise.resolve({ rows: [{ user_email: 'bob@cmeg.example' }] })
  ...
  it('still scopes attachment reads to the owner', async () => {
    ...
    expect(read?.sql).toContain('c.user_email = $2');
    expect(read?.params).toEqual(['conv-bob', 'alice@cmeg.example', false]);
  });
  ...
  it('lets an admin poll and read attachments for a shared conversation', async () => {
    ...
    expect(run?.params).toEqual(['conv-bob', 'alice@cmeg.example', true]);
    expect(attachments?.params).toEqual(['conv-bob', 'alice@cmeg.example', true]);
  });
  ...
  it('will not record feedback against a guessed message id for a consumer', async () => {
    ...
    expect(write?.params[2]).toBe('alice@cmeg.example');
...
describe('what the app says about itself at boot', () => {
  it('includes only sanitized organization mappings on /api/identity', async () => {
    process.env.PLAYER_INSIGHTS_ORGANIZATIONS = JSON.stringify([
      { domain: 'ROCKSTARGAMES.COM', name: 'Rockstar Games', monogram: 'R*' },
    ...
    try {
      ...
      expect(payload.organizations).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: 'rockstar-games', domain: 'rockstargames.com', logoKey: 'rockstar' }),
      ...
      expect(malformed.organizations).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({ id: 'take-two-interactive' }),
          expect.objectContaining({ id: '2k' }),
          expect.objectContaining({ id: 'rockstar-games' }),
        ])
      );
