...
import type { Answer, Conversation, ConversationMessage } from './app-types';
import { ConversationRailRunStatus } from './ConversationRailRunStatus';
import { claimConversationTitle } from './conversation-rail';
...
describe('first-answer terminal ordering', () => {
  it('settles the durable ledger run after polling replaces the browser correlation id', () => {
    const correlationId = 'corr-browser-1';
    const durableRunId = 'run-server-1';
    beginLiveAsk({ conversationId: CONVERSATION, question: 'Who led?' });
    identifyLiveAsk(CONVERSATION, correlationId);
    updateActiveConversationRuns((current) =>
      trackActiveConversationRun(current, CONVERSATION, {
        run_id: durableRunId,
        state: 'RUNNING',
    ...
    const terminal = terminalSettlementForResponse(answer(), { runStored: true, runId: durableRunId });
    expect(terminal.runId).toBe(durableRunId);
    expect(terminal.state).toBe('SUCCEEDED');
    expect(readActiveConversationRuns().get(CONVERSATION)?.status.run_id).toBe(durableRunId);
    settleAskDisplay(CONVERSATION, correlationId, terminal);

    expect(readActiveConversationRuns().get(CONVERSATION)?.status.state).toBe('SUCCEEDED');
