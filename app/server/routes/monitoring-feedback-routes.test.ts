...
describe('Monitoring feedback SQL truth', () => {
  ...
  it('pairs the answer to a real question and returns only submitted thumbs', () => {
    ...
    expect(MONITORING_FEEDBACK_QUERY).toContain('q.content <> $11');
    expect(MONITORING_FEEDBACK_QUERY).toContain("direction IN ('up', 'down')");
  });

  it('combines question, user, comment, direction, role, and organization filters', () => {
    for (const parameter of ['$6', '$7', '$8', '$9', '$10']) {
  ...
  it('joins role evidence without starting from the account roster', () => {
    ...
    expect(MONITORING_FEEDBACK_QUERY).not.toContain('sp_assignments');
