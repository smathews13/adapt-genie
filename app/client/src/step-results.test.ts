 ...
 * the agent's rendered retrieval outcome, whose heading is
  ...
  it('separates fully qualified names, metadata, and trailing prose', () => {
    const parsed = structuredTableResult(
      [
        ...
        '- catalog.schema.two  [franchise: 2K]',
      ...
      sections: [
        { kind: 'prose', text: 'DATA OVERVIEW' },
        {
          ...
          tables: [
            ...
            { name: 'catalog.schema.two', metadata: ['franchise: 2K'] },
          ],
        },
...
const DATA_GENIE = [
  'Asking Genie space Revenue Metrics (d00dfeedd00dfeedd00dfeedd00dfeed).',
  '',
...
const DICTIONARY_GENIE = [
  'Asking Genie space Revenue Definitions (badc0ffebadc0ffebadc0ffebadc0ffe).',
  '',
...
const GOVERNED_SEARCH = [
  'GOVERNED SEARCH RESULTS. These are descriptions and definitions, not data. Nothing here is a measurement and ' +
  ...
  '',
  '[table] example_catalog.sales_demo.silver_customer_profiles (uncertified)',
  'Table example_catalog.sales_demo.silver_customer_profiles. Validated customer profiles with explicit email eligibility and identity scope.',
  'Columns:',
  '- player_id (string)',
  '- cdp_customer_id (string)',
  '- label (string)',
  '- identity_confidence (decimal(5,4))',
  '- signup_date (date)',
  '- snapshot_date (date)',
  '',
  '[table] example_catalog.sales_demo.data_dictionary (uncertified)',
  'Table example_catalog.sales_demo.data_dictionary. Business definitions, sensitivity, and usage guardrails for curated sales fields',
  'Columns:',
  '- table_name (string)',
  '- column_name (string)',
  '',
  ...
  '',
  'What appears above was filtered by a cached snapshot of grants from when this governed dataset was built. It is ' +
...
const FINDINGS = [
  ...
  'Note: `player_id` is scoped per label, so if a player exists under multiple labels they may be counted more ' +
    ...
    '`cdp_customer_id`) may be more appropriate — let me know if you would like that breakdown.',
].join('\n');
...
describe('a data_genie result', () => {
  ...
  it('names the space that answered, with its id', () => {
    expect(result?.space).toEqual({ name: 'Revenue Metrics', id: 'd00dfeedd00dfeedd00dfeedd00dfeed' });
  });
  ...
  it('is not a definition card when the lookup returned several rows', () => {
    const many = DICTIONARY_GENIE.replace(
      ...
      [
        ...
        'silver_player_profiles | cdp_customer_id | Cross-label customer key. | Aggregate only.',
      ].join('\n')
    );
...
describe('a governed asset search result', () => {
  const result = semanticResult(GOVERNED_SEARCH);

  it('reads the entries the search matched, in order', () => {
    expect(result?.entries.map((entry) => entry.name)).toEqual([
      'example_catalog.sales_demo.silver_customer_profiles',
      'example_catalog.sales_demo.data_dictionary',
    ]);
  ...
  it('reads each entry’s certification, description and columns apart from each other', () => {
    ...
    expect(first.description).toBe('Validated customer profiles with explicit email eligibility and identity scope.');
    expect(first.columns.slice(0, 2)).toEqual([
      { name: 'player_id', type: 'string' },
      { name: 'cdp_customer_id', type: 'string' },
    ]);
  ...
  it('keeps a notice it has not been told about, rather than dropping it silently', () => {
    const unverified = GOVERNED_SEARCH.replace(
      '13 further entr(y/ies) matched and were left out to stay inside the result budget. Search again with a narrower question or a kind filter if none of the above is the right one.',
...
describe('a table name at row width', () => {
  it('elides the catalog and keeps the object apart from the schema', () => {
    expect(collapsedName('example_catalog.sales_demo.silver_customer_profiles')).toEqual({
      lead: '…sales_demo',
      object: 'silver_customer_profiles',
    });
  });
...
describe('an agent step’s findings', () => {
  ...
  it('reads a step with no pairs and no note as prose, unchanged', () => {
    expect(reportSections('search_tagged_assets')).toEqual([{ kind: 'prose', text: 'search_tagged_assets' }]);
  });
...
describe('a recorded statement', () => {
  ...
  it('breaks one line into a line per clause, and not per predicate', () => {
    expect(sqlClauseLines(sqlStatements(SQL)[0])).toEqual([
      ...
      'FROM `example_catalog`.`sales_demo`.`data_dictionary`',
      "WHERE `column_name` ILIKE '%player_id%' AND `business_definition` IS NOT NULL;",
    ]);
  });

  it('does not break inside a literal or a quoted name that holds a keyword', () => {
    const quoted = "SELECT `where` FROM t WHERE label = 'from Rockstar'";
    expect(sqlClauseLines(quoted)).toEqual(['SELECT `where`', 'FROM t', "WHERE label = 'from Rockstar'"]);
  });
