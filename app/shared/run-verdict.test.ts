...
import {
  ...
  completedWithGovernedRetries,
  displayedStageStatus,
  OPTIONAL_ANALYSIS_CLIP_NOTE,
...
const FIGURES = [{ label: 'GTA unique players', value: 6655, display: '6,655' }];
...
describe('the verdict a caveat cannot steal', () => {
  it('qualifies a completed answer when the same governed tool recovered on retry', () => {
    const stages = [
      { id: 'step-1-1-data_genie', status: 'partial' },
      { id: 'step-2-1-data_genie', status: 'complete' },
      { id: 'synthesis', status: 'complete' },
    ];
    expect(answerRunVerdict({ stages, narrative: TABLE })).toBe('complete');
    expect(completedWithGovernedRetries(stages)).toBe(true);
  });

  it('does not call unrelated later work a governed-data retry', () => {
    expect(
      completedWithGovernedRetries([
        { id: 'step-1-1-data_genie', status: 'failed' },
        { id: 'step-2-1-run_sql', status: 'complete' },
        { id: 'synthesis', status: 'complete' },
      ])
    ).toBe(false);
  });
  ...
  it('keeps a 12-table catalog listing Complete when optional analysis detail was clipped', () => {
    ...
    expect(OPTIONAL_ANALYSIS_CLIP_NOTE.test('Optional detail was clipped at the analysis handoff bound.')).toBe(true);
    expect(
      OPTIONAL_ANALYSIS_CLIP_NOTE.test(
        'Optional tail was clipped at the analysis handoff bound, so some metadata fields may be incomplete.'
      )
    ).toBe(true);
    const historicalStageAcronym = ['D', 'S', 'F'].join('');
    expect(
      OPTIONAL_ANALYSIS_CLIP_NOTE.test(`Optional detail was clipped at the ${historicalStageAcronym} handoff bound.`)
    ).toBe(true);
    expect(WRITER_STOPPED_CAVEAT.test('Optional detail was clipped at the analysis handoff bound.')).toBe(false);
    expect(
      answerRunVerdict({
  ...
  it('keeps a markdown catalog listing Complete when there is no pipe table', () => {
    expect(
      answerRunVerdict({
        stages: [
          { id: 'orchestrator-analysis', status: 'partial' },
