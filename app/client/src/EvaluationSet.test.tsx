...
describe('Evaluation set', () => {
  ...
  it('lays matching turns as a four-across card grid without reason tags', () => {
    const questions = [
      'how many users used either a rockstar game or a 2k game this year?',
      'show me GTA player trends',
    ];
  ...
  it('renders tags as capsules and per-case guidelines in the editor', () => {
    const markup = renderToStaticMarkup(
      <EvaluationSet
        lab={model(workspace([row({ id: 'case_001', question: 'How many active players?' })]), {
          expandedId: 'case_001',
        })}
      />
    );
