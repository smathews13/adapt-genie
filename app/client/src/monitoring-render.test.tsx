...
const TEST_ORGANIZATIONS: readonly OrganizationMapping[] = [
  {
    ...
    domainSuffixes: ['rockstargames.com'],
    name: 'Rockstar Games',
    monogram: 'R*',
    logoKey: 'monogram',
    ariaLabel: 'Organization: Rockstar Games',
    fallback: 'monogram',
  },
  {
    id: '2k',
    ...
    monogram: '2K',
    logoKey: 'monogram',
    ariaLabel: 'Organization: 2K Games',
    fallback: 'monogram',
  },
  {
...
describe('the detail modal', () => {
  ...
  it('keeps the user drilldown above the answer on a run that recorded no trace id', () => {
    const rendered = text(
  ...
  it('renders the answer with ADAPT Ask\u2019s own card when nothing is conditioned', () => {
    const markup = render(<QuestionDrawer detail={detail()} onClose={() => {}} canOpenUser />);
    const rendered = text(markup);
  ...
  it('uses one shared user badge as the canonical profile link without duplicate controls or possessive copy', () => {
    const markup = renderAt(
      <QuestionDrawer detail={detail({ askedBy: 'sam.mathews@example.test' })} onClose={() => {}} canOpenUser />,
    ...
    expect(markup).not.toContain('lucide-user-round');
    expect(markup).toContain(
      'aria-label="Open user overview for User sam.mathews@example.test; organization example.test"'
    );
    expect(markup).toContain('href="/monitoring?range=7d&amp;userSearch=sam&amp;who=sam.mathews%40example.test"');
    expect(
      occurrences(
        markup,
        'aria-label="Open user overview for User sam.mathews@example.test; organization example.test"'
    ...
    expect(rendered).toContain('sam.mathews');
    expect(rendered).not.toMatch(/see sam\.mathews|sam\.mathews['’]s? activity|this person['’]s activity/i);
  });

  it('aligns and wraps the user badge without widening the modal', () => {
  ...
  it('keeps the user badge non-interactive when User Monitoring is unavailable', () => {
    const markup = render(
      <QuestionDrawer detail={detail({ askedBy: 'sam.mathews@example.test' })} onClose={() => {}} canOpenUser={false} />
    );

    expect(markup).toContain(
      'class="identity-chip identity-chip--compact organization-user-badge question-attribution-user"'
    );
    expect(markup).not.toContain('aria-label="Open user overview for sam.mathews"');
    expect(markup).not.toContain('who=sam.mathews%40example.test');
  });
...
describe('the User Monitoring browser', () => {
  ...
  const payload = {
    userMonitoring: {
      ...
      unit: 'USD',
      state: 'partial',
      reason: 'Optional coverage is partial.',
      organizations: [
        {
          ...testOrganizationForEmail('person@rockstargames.com'),
          count: 1,
        },
        {
      ...
      users: [
        {
          email: 'ada.reader@rockstargames.com',
          role: 'admin',
          organization: testOrganizationForEmail('ada.reader@rockstargames.com'),
  ...
  it('renders one centered dialog with filters and spend-ordered user rows', () => {
    ...
    expect(markup).toContain('data-organization-id="rockstar-games"');
    ...
    expect(markup).toContain('monitoring-user-organization');
    expect(visible).toContain('Rockstar Games');
...
describe('the per-user panel', () => {
  ...
  it('shows the authorized current role', () => {
  ...
  it('says what a row filter is, not what it did to a run', () => {
    ...
    // No claim about rows lost, which ADAPT cannot know.
