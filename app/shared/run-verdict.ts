...
const RETRYABLE_GOVERNED_TOOLS = new Set(['data_genie', 'dictionary_genie', 'query_named_table', 'run_sql']);

function governedTool(stage: VerdictStage): string {
  const id = typeof stage.id === 'string' ? stage.id.trim() : '';
  const tool = /^step-\d+-\d+-(.+)$/.exec(id)?.[1] ?? '';
  return RETRYABLE_GOVERNED_TOOLS.has(tool) ? tool : '';
}

/**
 * Whether a governed read recovered after an earlier attempt ended short.
 *
 * The detailed stages remain untouched. This is only the final outcome
 * qualifier, and requires a later successful call to the same governed tool;
 * an unrelated successful synthesis cannot turn a refused read into a retry.
 */
export function completedWithGovernedRetries(stages: readonly VerdictStage[]): boolean {
  const unresolved = new Set<string>();
  for (const stage of stages) {
    const tool = governedTool(stage);
    if (!tool) continue;
    if (stage.status === 'partial' || stage.status === 'failed') unresolved.add(tool);
    if (stage.status === 'complete' && unresolved.has(tool)) return true;
  }
  return false;
}
...
/**
 * App-generated optional analysis clipping, not a terminal outcome.
 *
 ...
 * also contains an internal stage name or "clipped".
 */
export const OPTIONAL_ANALYSIS_CLIP_NOTE =
  /^(?:(?:\*\*)?package note:(?:\*\*)?\s*)?optional (?:tail|detail|metadata|bounded diagnostic) (?:was|were) clipped at the (?:(?:[a-z]{3}|analysis) )?handoff bound(?:, so (?:some )?metadata fields may be incomplete)?[.!]?$/i;
 ...
 * `partial` because optional analysis detail was clipped is not: the listing
 ...
 * clipped optional analysis detail is recorded as synthesis `partial` while the
 ...
 * optional analysis detail was clipped on a catalog listing.
 */
 ...
 * 12-table catalog listing Partial whenever optional analysis detail was clipped.
 */
