...
describe('shared generic tool-call label', () => {
  ...
  it('is reused by Run Explorer, Answer process, and Monitoring without adding Ask-side run metrics', () => {
    ...
    expect(read('HomePage.tsx')).not.toContain('<ToolCallsLabel');
  });
});
...
describe('public trace evidence and pre-stage copy', () => {
  ...
  it('uses the same exact planning phrase before stage one without duplicating progress in the insight rail', () => {
    ...
    expect(read('HomePage.tsx')).toContain('<LiveProgress');
    expect(read('HomePage.tsx')).toContain('className="trace-inspector insight-rail"');
    expect(read('HomePage.tsx')).not.toContain('AgentPathConstellation');
    expect(read('HomePage.tsx')).not.toContain('currentStage={currentStage}');
  });
});
