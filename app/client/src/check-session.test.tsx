...
import { checkedAtOf, forgetChecks, recallChecks, rememberChecks, type CheckSession } from './check-session';
...
describe('coming back to the tab after the checks have run', () => {
  ...
  it('does not restore removed run-limit or summary strips', () => {
    rememberChecks(ranAt());
    const markup = pageMarkup();
    expect(markup).not.toContain('data-testid="architecture-loop-tiles"');
...
describe('the store outlives a route change and nothing else', () => {
  it('is not written to storage that outlives the running app', () => {
    ...
    const source = readFileSync(fileURLToPath(new URL('./check-session.ts', import.meta.url)), 'utf8').replace(
      /\/\*[\s\S]*?\*\//g,
      ' '
    );
