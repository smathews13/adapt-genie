...
function ExecutionValue({ identity }: { identity: NonNullable<DeploymentIdentity['identity']> }) {
  ...
  if (mode === 'app_service_principal') {
    return <span>{identity.identityMetadata?.app.displayName || 'ADAPT'} app</span>;
  }
        ...
        {failed ? ( // One line, in the shape of every other value on the card: a chip that
          ...
          <div className="identity-overview">
            ...
            <section className="identity-section" aria-labelledby="identity-app-heading">
              ...
              <div className="identity-section-grid">
                <Fact label="Display name">
                  <span>{metadata?.app.displayName || 'ADAPT'}</span>
                </Fact>
                ...
                <Fact label="Execution">{identity ? <ExecutionValue identity={identity} /> : null}</Fact>
              </div>
            </section>
