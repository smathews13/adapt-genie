...
describe('the run-label repair migration', () => {
  it('recreates the missing table for databases whose older version ledger drifted', () => {
    const migration = MIGRATIONS.find((entry) => entry.name === 'repair run label overrides');
    expect(migration?.version).toBe(42);
    expect(migration?.statements.join('\n')).toContain('CREATE TABLE IF NOT EXISTS');
    expect(migration?.statements.join('\n')).toContain('run_label_overrides');
