/**
 * That opening ADAPT Ask asks for each list exactly once, and asks for both at
 * the same time.
 *
...
describe('what the page is handed', () => {
  ...
  it('sends normalized owner filters and reads server-computed matches', async () => {
    const calls: string[] = [];
    vi.stubGlobal('fetch', (url: string) => {
      calls.push(url);
    ...
    expect(calls).toEqual(['/api/conversations?owners=alice%40example.com']);
 ...
 *
 * `conversationLoading` is not a spinner on the rail. While it is true ADAPT Ask
...
describe('a slow run list does not hold the page shut', () => {
  ...
  it('is the read ADAPT Ask clears its composer gate on', () => {
