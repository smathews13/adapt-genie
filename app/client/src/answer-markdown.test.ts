...
const CATALOG = 'example_catalog.sales_demo';
...
describe('the blocks the agent actually writes', () => {
  ...
  it('keeps nested findings lines as items of the same list, so they keep the same dots', () => {
    ...
    const source = [
      '## DATA PACKAGE',
      '',
      '- **Interpretation:** Rolling 90-day daily player trend for GTA titles.',
      '- **Sources used:**',
      '  - `gold_title_daily_summary` daily aggregate.',
      '  - `search_tagged_assets` used for initial table discovery.',
      ...
      '  Weekly trend (most recent 4 full weeks, GTAO):',
    ...
    expect(blockText([blocks[1]])).toContain('Weekly trend (most recent 4 full weeks, GTAO):');
  });
  ...
  it('lifts a bulleted Reference / Metadata label to the same rank as Gold', () => {
    const source = [
      ...
      '- `governed_asset_list`: List of governed entries.',
    ].join('\n');
    const blocks = parseAnswerMarkdown(source);
  ...
  it('reads bold wrapping a code span, so a schema name is not starred backticks', () => {
    const source = 'All 12 declared tables live in **`example_catalog.sales_demo`** and are available to query.';
    const nodes = answerInline(source, [], []);
    expect(visible(nodes)).toBe(
      'All 12 declared tables live in example_catalog.sales_demo and are available to query.'
    );
...
describe('the tables the agent writes', () => {
  ...
  it('drops a data_genie dump from the prose and still finds the table after it', () => {
    const source = [
      'data_genie({"question": "For the title \\"Red Dead Redemption 2\\", distinct players by platform"})',
      '',
...
describe('the rendered Markdown is styled as part of an answer', () => {
  ...
  it('draws a backticked name as the info chip, which is what is inside the backticks', () => {
    ...
    // in. It was `--db-code-bg`, an rgba of a grey that is in no ADAPT family.
