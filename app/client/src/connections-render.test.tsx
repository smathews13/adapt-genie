...
describe('inline build status descriptions', () => {
  ...
  it('keeps the endpoint free of filler and places exporter context before its copy control', () => {
    ...
    expect(endpointMarkup).not.toContain('Serves this ADAPT deployment.');
...
describe('Build and telemetry date badges', () => {
  ...
  it('renders Last deployed as one date badge followed by the existing user badge', () => {
    ...
    const user = markup.indexOf('identity-chip identity-chip--compact organization-user-badge');
    ...
    expect(text(markup)).toContain('sam');
  });
...
describe('the deployment source links', () => {
  ...
  it('opens each destination from a real anchor, in a new tab', () => {
    ...
    expect(markup).toContain('<a class="deployment-fact-link" href="https://workspace.example.com/apps/adapt"');
...
describe('one panel for the failures, not one panel per failure', () => {
  ...
  it('prints a shell remedy once for the group that shares it', () => {
    ...
    const rendered = renderGroups([
      check('optional-catalog', 'unverified', {
        label: 'Optional catalog',
        detail: 'HTTP 403. This app does not ask for `optional.catalog:read`.',
 ...
 * an optional catalog. Each carried the same paragraph again with a different
...
describe('optional permissions are not a thing to fix', () => {
  ...
  /**
   * The live screen's optional shortfalls: two catalog scopes and twelve
   * tables. All fourteen are optional.
   */
  const LIVE = [
  ...
  it('draws no fix block for a shortfall in an optional permission', () => {
    ...
    // The optional names are on the screen and not
    // inside the panel.
  ...
  it('reports the optional shortfall neutrally, with no cause and no fix', () => {
    ...
    // Fourteen: the twelve tables, catalog, and schema.
...
describe('a connection row', () => {
  ...
  it('lets administrators edit immediate and staged connection values', () => {
    ...
    expect(adminLockedKind).toMatch(/data-affordance/);
    expect(adminLockedWarehouse).toMatch(/data-affordance/);
  ...
  it('wires a picker and Change action for every administrator-editable connection', () => {
    ...
    for (const id of ['assets-volume', 'experiment-id'] as const) {
    ...
    expect(pickerForField('lakebase')).not.toBeNull();
    expect(PAGE_SOURCE).toContain('<LakebaseBindingManager');
  });
...
describe('the Unity Catalog tables section', () => {
  ...
  it('renders the shared modal shell without the retired inline picker controls', () => {
    ...
    expect(text(markup)).toContain('Declaring an asset records intended scope only.');
    expect(text(markup)).toContain('Live Genie curation remains the model manifest source of truth');
    expect(text(markup)).toContain('until the agent is logged again');
  ...
  it('lists an added table once with a compact connected badge and no row loader', () => {
    ...
    expect(text(tableMarkup)).toContain('Connected');
  ...
  it('keeps added catalogs and schemas in the one Unity Catalog table', () => {
    ...
    expect(text(ucMarkup)).toContain('In scope · added in ADAPT');
  ...
  it('counts exact declarations without inferring catalog or schema descendants', () => {
    ...
    ).toEqual({ label: 'Connected', selectable: false });
  });
  ...
  it('draws a row per declared table with binary status and freshness', () => {
    ...
    expect(rendered).not.toContain('@databricks.com');
  });
