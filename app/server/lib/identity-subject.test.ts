...
function userToken(claims: Record<string, unknown> = {}) {
  return jwt({ sub: 'alice@take-two.example', exp: NOW + 3600, ...claims });
}

describe('reading a token', () => {
  it('reads the claims of a base64url JWT', () => {
    expect(readTokenClaims(jwt({ sub: 'alice@take-two.example' }))).toEqual({
      sub: 'alice@take-two.example',
    });
  });
  ...
  it('collects every subject the token names rather than picking one', () => {
    expect(tokenSubjects(jwt({ sub: '4212345', email: 'alice@take-two.example' }))).toEqual([
      '4212345',
      'alice@take-two.example',
    ]);
  });

  it('ignores a subject claim that is present and empty', () => {
    expect(tokenSubjects(jwt({ sub: '   ', email: 'alice@take-two.example' }))).toEqual(['alice@take-two.example']);
  });
});
...
describe('comparing two identities', () => {
  ...
  it('treats a difference of case as the same person', () => {
    expect(sameSubject('Alice@Take-Two.Example', 'alice@take-two.example')).toBe(true);
  });

  it('does not treat a plus-address or another domain as the same person', () => {
    expect(sameSubject('alice+work@take-two.example', 'alice@take-two.example')).toBe(false);
    expect(sameSubject('alice@elsewhere.example', 'alice@take-two.example')).toBe(false);
  });
});

describe('binding a token to the signed-in user', () => {
  it('binds a token issued to the person the request is from', () => {
    const binding = bindTokenToUser(userToken(), 'alice@take-two.example', { now: NOW });
    expect(binding).toEqual({ kind: 'bound', subject: 'alice@take-two.example' });
  ...
  it('binds when the address arrives on a claim other than sub', () => {
    const binding = bindTokenToUser(
      jwt({ sub: '4212345', upn: 'alice@take-two.example', exp: NOW + 60 }),
      'alice@take-two.example',
      { now: NOW }
    );
  ...
  it('refuses a token issued to somebody else', () => {
    const binding = bindTokenToUser(userToken({ sub: 'bob@take-two.example' }), 'alice@take-two.example', { now: NOW });
    expect(binding).toEqual({ kind: 'mismatch', subject: 'bob@take-two.example' });
  ...
  it('refuses an expired token before it looks at whose it is', () => {
    const binding = bindTokenToUser(
      userToken({ sub: 'bob@take-two.example', exp: NOW - 3600 }),
      'alice@take-two.example',
      { now: NOW }
    );
  ...
  it('tolerates a clock difference rather than reporting an outage as an expiry', () => {
    const within = bindTokenToUser(userToken({ exp: NOW - CLOCK_SKEW_SECONDS + 5 }), 'alice@take-two.example', {
      now: NOW,
    });
    expect(within.kind).toBe('bound');
    const beyond = bindTokenToUser(userToken({ exp: NOW - CLOCK_SKEW_SECONDS - 5 }), 'alice@take-two.example', {
      now: NOW,
    });
  ...
  it('reports an absent token as absent rather than as a mismatch', () => {
    expect(bindTokenToUser(null, 'alice@take-two.example').kind).toBe('absent');
    expect(bindTokenToUser('   ', 'alice@take-two.example').kind).toBe('absent');
...
describe('a subject that cannot be compared is unproven, not wrong', () => {
  it.each([
    ...
    const binding = bindTokenToUser(token, 'alice@take-two.example', { now: NOW });
    expect(binding.kind).toBe('unverifiable');
    expect(bindingFailure(binding)).toBeNull();
  });

  it('does not let unverifiable count as verified', () => {
    const binding = bindTokenToUser('dapi0123', 'alice@take-two.example', { now: NOW });
    expect(isVerified(binding)).toBe(false);
  });
  ...
  it('still refuses when an incomparable claim sits beside somebody else\u2019s address', () => {
    const binding = bindTokenToUser(
      jwt({ sub: '4212345', email: 'bob@take-two.example', exp: NOW + 60 }),
      'alice@take-two.example',
      { now: NOW }
    );
    expect(binding).toEqual({ kind: 'mismatch', subject: 'bob@take-two.example' });
...
describe('what gets written down', () => {
  it('names both addresses on a mismatch, which is the whole finding', () => {
    const binding = bindTokenToUser(userToken({ sub: 'bob@take-two.example' }), 'alice@take-two.example', { now: NOW });
    const line = describeBinding(binding, 'alice@take-two.example');
    expect(line).toContain('alice@take-two.example');
    expect(line).toContain('bob@take-two.example');
  ...
  it('never puts the token or any other claim in the log line', () => {
    ...
    for (const email of ['alice@take-two.example', 'bob@take-two.example']) {
