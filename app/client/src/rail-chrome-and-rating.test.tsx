...
const QUESTION = strip(partial('question-attribution.css'));
...
describe('a conversation with a run in flight', () => {
  it('shows the durable latest-turn status without restoring live harness chrome', () => {
    const row = HOME.slice(HOME.indexOf('<span className="conversation-item-head">'));
    expect(row.slice(0, row.indexOf('</span>'))).toContain('conversation-status');
    expect(row.slice(0, row.indexOf('</span>'))).toContain('summary.status');
    expect(HOME).not.toContain('<ConversationRailRunStatus');
...
describe('the unified question attribution bubble', () => {
  it('keeps the question and asker inside one tailed surface', () => {
    expect(HOME).toContain('<QuestionAttributionBubble');
    expect(rule(QUESTION, '.question-attribution-surface')).toContain('border: 1px solid var(--ast-border-input)');
    expect(rule(QUESTION, '.question-attribution-surface::after')).toContain('right: 22px');
  });

  it('keeps the bubble in normal flow above its answer', () => {
    ...
    expect(row).toContain('margin-bottom: 22px');
  });
...
describe('the settings pane uses elevated semantic glass', () => {
  ...
  it('is elevated below the stronger account-menu surface', () => {
    ...
    // Account and settings overlays both use opaque semantic surfaces so the
    // static background cannot compete with their content.
    expect(rule(DARK, "html[data-theme='dark'] .account-menu")).toContain('var(--background)');
