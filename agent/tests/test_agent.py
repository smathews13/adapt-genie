...
PLANNER_PREFIX = "You are ADAPT's planner"
...
def test_sources_are_the_tables_the_run_read_and_nothing_else():
    ...
    assert cited == [PROFILES, SUMMARY_180D]
...
def test_a_stage_payload_past_the_field_ceiling_is_clipped_and_says_so():
    ...
    tool_stage = next(stage for stage in stages(ask(build(llm, tools))) if stage["kind"] == "tool")
...
def plan_for(question=PLAN_QUESTION, **kwargs):
    ...
    plan = runtime._discovered_plan(question, [], "")
...
def test_the_takeaway_leads_and_the_narrative_does_not_repeat_it():
    ...
    assert "Never\nuse a transport or section label" in SYNTHESIS_INSTRUCTIONS
    assert '"Returned:"' in SYNTHESIS_INSTRUCTIONS


def test_findings_are_asked_for_as_two_to_four_distinct_bullets():
    ...
    assert "2-4 short bulleted claims" in SYNTHESIS_INSTRUCTIONS
...
def test_a_short_answer_is_still_allowed_to_stay_short():
    ...
    assert "If fewer than 2 distinct claims exist" in SYNTHESIS_INSTRUCTIONS
...
def test_null_optional_synthesis_sections_validate_as_empty():
...
def test_malformed_synthesis_salvages_prose_without_printing_raw_json():
    malformed = json.dumps(
        {
            "takeaway": "NBA 2K26 sold 58 units.",
            "narrative": "- **58 units** were sold.",
            "figures": [{"label": "Units"}],
        }
    )
    synthesis = agent._salvaged_synthesis(malformed, "## DATA PACKAGE")
    assert synthesis.takeaway == "NBA 2K26 sold 58 units."
    assert synthesis.narrative == "- **58 units** were sold."
    assert not synthesis.narrative.startswith("{")
    assert synthesis.caveats[0] == agent.SALVAGED_CAVEAT


def test_salvage_uses_evidence_when_narrative_is_missing():
    synthesis = agent._salvaged_synthesis(
        '{"takeaway": null}', "## DATA PACKAGE\n- **Findings / data:** **58 units** were sold."
    )
...
def test_inventory_cleanup_keeps_answer_text_without_a_blank_separator():
    cleaned = agent._without_internal_answer_transcript(
        "Relevant tables in this Genie space are:\n"
        "- `catalog.schema.daily`: daily metrics\n"
        "- `catalog.schema.players`\n"
        "**Bookings increased 12%.**"
    )
    assert cleaned == "**Bookings increased 12%.**"
...
class TestTheInternalPackageIsNotShownAsAnAnswer:
    """The loop's evidence package is internal. When a turn ran out
    ...
    def test_the_apparatus_sections_do_not_reach_the_reader(self):
        narrative, _ = reader_facing_evidence(PACKAGE)
    ...
    def test_a_label_with_no_body_leaves_nothing_behind(self):
        ...
        The loop may emit the lead-in whether or not it has anything to put after it,
        so a section has to be dropped on its BODY being empty rather than on its
        name being one this path keeps.
        """

        narrative, caveats = reader_facing_evidence(PACKAGE)
    ...
    def test_limits_become_caveats_rather_than_prose(self):
        ...
        _, caveats = reader_facing_evidence(PACKAGE)
    ...
    def test_an_overview_is_already_prose_and_only_loses_its_heading(self):
        ...
        narrative, caveats = reader_facing_evidence(overview)
...
def test_the_figures_and_the_names_are_asked_to_be_bolded():
    ...
    assert "table and column names" not in SYNTHESIS_INSTRUCTIONS
...
# Mirrors the companion app's live-answer schema.
...
def test_a_run_records_the_genie_space_it_put_the_question_to():
    ...
    llm = ScriptedLlm([Call("data_genie", {"question": "figures"})], "Done.")
    tools = configured(data_genie_space_title="ADAPT Data")
    ...
    assert spaces == [{"id": "data", "title": "ADAPT Data"}]
...
def test_orchestrator_prefers_direct_governed_reads_before_genie():
...
def test_orchestrator_trace_has_no_intermediate_agent_stage():
    response = ask(
        build(
            ScriptedLlm(
                [Call("data_genie", {"question": "active players"})],
    ...
    trace = stages(response)
    assert trace[0]["id"] == "orchestrator"
    assert trace[1]["id"] == "step-1"
...
def test_the_question_is_asked_once_when_the_last_turn_is_the_plan():
    ...
    messages = llm.loop_calls[0]["messages"]
    analysis_requests = [m["content"] for m in messages if m["role"] == "user"]
    assert len(analysis_requests) == 1
    assert f"Analysis request:\n{question}" in analysis_requests[0]
    assert "Here is the plan I propose." in analysis_requests[0]
...
def test_a_repeated_question_earlier_in_the_conversation_is_kept():
    ...
    assert len(analysis_requests) == 1
    assert f"Analysis request:\n{question}" in analysis_requests[0]
    assert analysis_requests[0].count(question) == 2
...
def test_the_endpoint_reports_only_current_adapt_configuration():
    outputs = preflight(build(ScriptedLlm())).custom_outputs
    keys = {entry.get("key") for entry in outputs["configuration"]}

    assert "semantic_" + "index" not in keys
