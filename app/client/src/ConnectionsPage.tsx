...
// The one status recipe. Named as a meaning here, painted with ADAPT's compatibility tokens.
          ...
          <p className={`deployment-fact-text${row.dateRange ? ' deployment-fact-dates' : ''}`} title={row.title}>
            ...
            {row.identity ? (
              <span className="deployment-identity">
                by <OrganizationUserBadge identity={row.identity} />
              </span>
            ) : null}
          </p>
        ) : null}
                ...
                <TableRow id={`declared-table-row-${entry.connection.id}`} tabIndex={-1}>
                  <TableCell className="connections-table-name">
                    <span className="connections-table-name-stack">
                      <span className="connections-table-name-line">
              ...
              return (
                ...
                whole sentence is still here in a title. */}
                    <TableCell className="connections-table-detail" title={reachability.title}>
                      {declared?.pending ? null : (
                        <>
                          {isRequestedEntity(check.name, requestedEntity) ? (
                            <span className="connections-table-arrival">
                              Linked from the answer you followed here.{' '}
                            </span>
                          ) : null}
                          {reachability.row}
...
export function unityCatalogAssetScopeState(
  ...
  if (resourceType === 'table' && managedTableNames.some((name) => normalizedConnectionValue(name) === normalized)) {
    return { label: 'Connected', selectable: false };
  }
...
export function ConnectionRow({
   ...
   */
  // A save already has a compact loader in its button. Suppress the row-level
  // refresh treatment during that same write so one action never draws two
  // simultaneous animations.
  const restating = !saving && refreshing && status !== 'nothing-to-reach';
...
export function ConnectionsPage() {
   ...
   * primary and row-local ADAPT loaders. A re-run leaves cached answers on
  ...
  }, [firstRun, payload, session?.load?.settings]);
  /**
  ...
  const readings = useMemo(() => {
    const rows = payload ? readConnections(payload, reported) : connectionPlaceholderReadings(reported);
    return rows.filter((reading) => !isAdaptHiddenConnection(reading.resource.id));
  }, [payload, reported]);
  /**
  ...
  const groups = useMemo(
    () =>
      groupConnections(
        readings,
        (reading) => refreshing || connectionResourceLoadState(reading, session, firstRun) === 'loading'
      ),
    [firstRun, readings, refreshing, session]
  );
  /**
