...
describe('timeout boundary', () => {
  ...
  it('ends the client session, blocks retry bootstrap, and shows the concise return action', async () => {
    ...
    expect(markup).toContain('Your ADAPT session ended after inactivity. Return to sign in to continue.');
    ...
    expect(markup).not.toMatch(/Sign out of ADAPT|workspace session|identity-provider|authenticate you again/);
  ...
  it('keeps upstream-session detail off the timeout card', () => {
    ...
    expect(accountMenu).toContain('Sign out of ADAPT');
