...
export function resourceStates(input: {
  ...
  return CONNECTED_RESOURCES.map((resource) => {
    ...
    let configured = entry ? displayValue(entry.value) : '';
    let configuredFrom = entry ? (text(entry.source) ?? '') : '';
    if (!configured && resource.appEnvVar) {
    ...
    if (resource.id === 'lakebase-schema') {
      ...
      if (!configuredFrom) configuredFrom = 'app-environment';
    }
 ...
 */
const TRUSTED_PROVENANCE = new Set([ARTIFACT, 'app-environment', 'app-default', 'data-contract']);
