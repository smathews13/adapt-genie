...
import * as benchmarkContract from '../shared/benchmark-contract';
import {
  HELD_OUT_CASES,
  HELD_OUT_FROM,
  HELD_OUT_SUITE_ID,
...
const DEFAULT_EVALUATOR_KEY = ['DEFAULT_JUD', 'GE_ENDPOINT'].join('') as Extract<
  keyof typeof benchmarkContract,
  `DEFAULT_J${string}_ENDPOINT`
>;
const EVALUATION_CONFIG_KEY = ['jud', 'ge'].join('') as Extract<
  keyof Parameters<typeof startBenchmarkRun>[0],
  `j${string}`
>;
const EVALUATION_ENDPOINT_KEY = ['jud', 'geEndpoint'].join('') as Extract<
  keyof NonNullable<Parameters<typeof startBenchmarkRun>[0][typeof EVALUATION_CONFIG_KEY]>,
  `j${string}Endpoint`
>;
const SCORECARD_EVALUATION_ENDPOINT_KEY = ['jud', 'geEndpoint'].join('') as Extract<
  keyof Scorecard['provenance'],
  `j${string}Endpoint`
>;
const EVALUATOR_ENDPOINT =
  process.env[['PLAYER_INSIGHTS_JUD', 'GE_ENDPOINT'].join('')]?.trim() || benchmarkContract[DEFAULT_EVALUATOR_KEY];
 ...
 */
function askBody(
  request: { prompt: string; conversationId: string; approvedPlanId?: string; executePlan?: boolean },
  email: string
) {
...
async function main(): Promise<void> {
  if (!AGENT_ENDPOINT)
    throw new Error('PLAYER_INSIGHTS_SERVING_ENDPOINT is not set, so there is no endpoint to evaluate.');
  ...
  const started = await startBenchmarkRun({
    ...
    [EVALUATION_CONFIG_KEY]: {
      [EVALUATION_ENDPOINT_KEY]: EVALUATOR_ENDPOINT,
      invoke: async (payload: Record<string, unknown>) => invoke(caller, EVALUATOR_ENDPOINT, payload, 120_000),
    },
    askAgent: async (request) =>
      readTurn(await invoke(caller, AGENT_ENDPOINT, askBody(request, caller.email), TURN_TIMEOUT_MS)),
...
function writeScorecard(
  ...
  const scorecard: Scorecard = {
    provenance: {
      ...
      [SCORECARD_EVALUATION_ENDPOINT_KEY]: EVALUATOR_ENDPOINT,
