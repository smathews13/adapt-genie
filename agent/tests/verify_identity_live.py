...
it: it needs a real token and a deployed endpoint.
    ...
    uv run python tests/verify_identity_live.py --endpoint adapt-orchestrator
...
or set the compatibility probe-table environment variable. It must be granted to
the human and NOT to the app's service principal, with no `account users` grant
anywhere above it.
...
What the probe does NOT yet cover is the end-to-end path. ADAPT queries governed
Genie spaces, so it cannot be pointed at an arbitrary throwaway table. The probe
therefore measures the two principals directly rather than through a question.
Closing that last gap means adding the probe table to a governed Genie space,
which is a decision for whoever owns the catalog, not something to do quietly
here.
...
To run it, get approval first, then:
    ...
    databricks service-principals create --display-name adapt-identity-negative
...
NOBODY = "not-the-invoker@adapt.invalid"
...
def probe_read(host: str, token: str, warehouse: str, table: str) -> tuple[bool, str]:
    ...
    Runs the read directly because ADAPT queries governed Genie spaces and cannot
    be pointed at an arbitrary table. This measures the grant itself and leaves
    the end-to-end path to the identity cases above.
    """
...
def main() -> int:
    ...
    print(
        ...
        "denial there is evidence rather than wording. ADAPT cannot query this arbitrary "
        "probe through its governed Genie route, so the end-to-end read is not exercised."
        if probe_usable
