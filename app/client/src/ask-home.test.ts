...
describe('the ask home is the geometry the mockup gives it', () => {
  ...
  it('holds the transcript off the rails by a token rather than by a retyped clamp', () => {
    ...
    // across partials -- the transcript's padding among them -- so the box a
    // reader types into and the column it appears in lined up only by having been
    // retyped identically. That is the failure --app-header-pad-x was made a token
  ...
  it('keeps the chrome the transcript scrolls under opaque, and only that chrome', () => {
    ...
    for (const selector of ["html[data-theme='dark'] .app-header", "html[data-theme='dark'] .composer"]) {
      ...
      expect(rule, `${selector} is opaque`).toMatch(/background:\s*var\(--ast-surface-(?:solid|chrome|primary)\)/);
      expect(rule, `${selector} does not frost`).toMatch(/backdrop-filter:\s*none/);
    }
    // ADAPT uses a static reading surface in the rail, with no constellation
    // inspector or frosted sky behind it.
    const rail = body("html[data-theme='dark'] .conversation-rail", dark);
    expect(rail).toMatch(/background:\s*var\(--ast-surface-primary\)/);
    expect(rail).toMatch(/background-image:\s*none/);
    expect(rail).toMatch(/backdrop-filter:\s*none/);
  });
  ...
  it('bounds the transcript’s measure, so the widest windows have a page margin', () => {
    ...
    // And it fills the track up to that cap rather than shrinking to its content.
    // On a grid item `margin-inline: auto` turns off the default stretch, so
    // without this the column was as wide as whatever the answer happened to hold
    // -- the measure for a card with a wide table, hundreds of pixels short of it
    // for one with a chart. `width: 100%` makes the width the window's to decide,
    // not the content's, so the same answer surface is one width across answers.
  ...
  it('makes the empty state a headline, the ADAPT starter questions, and an in-flow composer', () => {
    /*
     * The empty state is a headline, four ADAPT starter-question cards, and the
     * composer. While there is no transcript and no load in flight, the page
     * marks the main column `is-empty` and returns the composer from its fixed
     * seat to normal flow under the headline. Once a question is appended, that
     * class leaves and the fixed transcript control comes back.
     *
     * The starters are the ADAPT `.ask-starter` cards, not the retired
     * `.prompt-grid`: each submits through the same ask() the composer calls.
     * Comments are stripped because ask.css keeps the retired selector's name in
     * the explanation of why THAT grid must stay gone.
     */
    ...
    // The ADAPT starter cards are present, styled, and each carries a real prompt.
    expect(home).toContain('className="ask-starter"');
    expect(home).toContain('void ask(starter.question)');
    expect(home).toContain("fetch('/api/ask-starters')");
    expect(home).toContain('DEFAULT_ASK_STARTER_SETTINGS.questions');
    expect(home).not.toContain('useSavedQueries');
    expect(home).not.toContain('Saved queries');
    expect(body('.ask-starters')).toMatch(/grid-template-columns/);
    // The retired hand-asking grid stays retired; the starters are a fresh selector.
  ...
  it('leads the composer caveat with the mark, in a row that can hold one', () => {
    ...
    const home = withoutComments(HOME_PAGE);
    expect(home).toMatch(
      /<AstrolabeMark size=\{13\} \/>\s*ADAPT can make mistakes\. Sources and caveats are included\./
    );
  ...
  it('uses the entire harness rail as one navy panel', () => {
    ...
    // A FLAT FILL, NOT A STAR FIELD. The starfield spackle this column used to
    // carry was removed with the rest of the stars theme, so the panel -- and the
    // idle `.trace-empty` that has no surface of its own -- is one solid navy.
    expect(inspector).not.toMatch(/--ast-sky-spackle/);
...
describe('the run says which of four things it is doing', () => {
  ...
  it('states all four rather than leaving two of them to one Badge variant', () => {
    ...
    expect(new Set(PAINTED.map(([, selector]) => selector)).size).toBe(3);
...
describe('the two marks that sign a transcript', () => {
  ...
  it('unifies every reader question with compact organization attribution', () => {
    ...
    expect(body('.user-message .question-attribution-user .identity-chip-text')).toMatch(/overflow:\s*visible/);
    expect(HOME_PAGE).toContain('canOpenUser={adminSharedRail}');
  });
});
...
describe('the inspector while a run is still going', () => {
  ...
  it('draws no agent-path constellation anywhere on the Ask page', () => {
    // The Live Agent harness panel was removed for the customer POC: the Ask page
    // no longer exposes the numbered agent path, its metrics, or the constellation
    // that drew it. The answer pane reports progress in plain text via LiveProgress.
    expect(HOME_PAGE).not.toContain('<AgentPathConstellation');
    expect(HOME_PAGE).not.toContain("import { AgentPathConstellation } from './AgentConstellation'");
    ...
    expect(HOME_PAGE).not.toContain('className="insight-harness"');
    expect(HOME_PAGE).not.toContain('Agent path');
  });

  it('shows every live run step to consumers as well as admins', () => {
    const liveSeat = HOME_PAGE.slice(HOME_PAGE.indexOf('<LiveProgress'), HOME_PAGE.indexOf('</CardContent>'));
    expect(liveSeat).toContain('stages={liveStages}');
  ...
  it('does not relabel a stored Complete turn as Partial only because it was truncated', () => {
    ...
    expect(HOME_PAGE).not.toContain("summary.truncated === true ? 'partial'");
  });
  ...
  it('still tracks the running step for the status pill, with no path to ring', () => {
    // The numbered agent path (and the `railActiveIndex` frontier that used to ring
    // its newest card) went with the harness panel. What survives is `runningStep`:
    // the step in progress is the number the pill's failure label needs -- the step
    // a run DIED inside is a different claim from how far it got.
    expect(HOME_PAGE).toMatch(/const runningStep = runningStepNumber\(liveStages\);/);
    expect(HOME_PAGE).toMatch(/runningStep,/);
    // The ring is gone: no `railActiveIndex`, and nothing derives a frontier from
    // `railStages` any more.
    expect(HOME_PAGE).not.toMatch(/railActiveIndex/);
    expect(HOME_PAGE).not.toMatch(/railStages/);
  });
  ...
  it('counts the step in progress off one clock, and stops it when the run ends', () => {
    ...
    // LiveProgress in the answer column is the one surface that reads this clock
    // now that the harness constellation is gone.
...
describe('below 1180px the finished run is still reachable', () => {
  ...
  it('keeps the way into a run on admin surfaces only', () => {
    expect(HOME_PAGE).toMatch(/showAdminTrace \? \([\s\S]{0,180}className="trace-summary-link"/);
    expect(HOME_PAGE).toMatch(/Explore full run/);
  });

  it('says the lost-write disclosure from one constant at its one remaining site', () => {
    // With the harness panel removed the composer strip is the only place that
    // shows this disclosure, so the sentence lives in a single constant read once
    // there -- a shortened copy could not diverge from a second because there is
    // no second any more.
    expect(HOME_PAGE).toMatch(/const RUN_NOT_STORED =/);
    expect(HOME_PAGE).toMatch(/will not be here when you \s*'?\s*\+?\s*'?come back/);
    expect(HOME_PAGE.match(/RUN_NOT_STORED/g)?.length).toBe(2);
  });
  ...
  it('keeps the composer in the center column flow when the insight rail leaves', () => {
    expect(NARROW).not.toMatch(/\.composer\s*\{[^}]*(?:left|right|bottom|position:\s*fixed)/);
    expect(body('.composer')).toMatch(/position:\s*static/);
  });
});
