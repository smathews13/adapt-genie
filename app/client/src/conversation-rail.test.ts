...
import { claimConversationTitle, ownerKey, railOwnership, signedInOwner } from './conversation-rail';
...
describe('a conversation is named by the question, the moment it is asked', () => {
  it('replaces the placeholder rather than waiting for an answer to do it', () => {
    ...
    const before = [stored('conv-1', PLACEHOLDER_CONVERSATION_TITLE, ME)];
...
describe('the rename happens where the question is sent, not where the reply is read', () => {
  ...
  it('stamps the reader onto the row only when a question is sent', () => {
    const start = HOME_PAGE.slice(
      HOME_PAGE.indexOf('function startNewConversation()'),
      HOME_PAGE.indexOf('function focusQuestionInput()')
    );
    expect(start).not.toContain('unaskedConversation');
...
describe('who a rail row belongs to is decided once, for the count and the watermark both', () => {
  ...
  it('treats one address in two capitalisations as one person', () => {
    // Not what the deployment stores today -- every row on cmeg carries the same
...
describe('the signed-in address is an address, or it is nothing', () => {
  ...
  it('does not put an untouched new conversation in the rail', () => {
    const start = HOME_PAGE.slice(
      HOME_PAGE.indexOf('function startNewConversation()'),
      HOME_PAGE.indexOf('function focusQuestionInput()')
    );
    expect(start).not.toContain('claimConversationTitle');
    expect(start).not.toContain('unaskedConversation');
  });

  it('puts an attachment-only conversation in the rail after the server stores it', () => {
    ...
    expect(upload.indexOf('if (!response.ok || !payload)')).toBeLessThan(upload.indexOf('unaskedConversation'));
    expect(upload).toContain('rememberSelectedConversation(conversationId)');
  });
});
...
describe('the rail draws one answer to "whose is this", not two', () => {
  ...
  it('attributes answers to the selected conversation owner', () => {
    expect(HOME_PAGE).toMatch(
      /const asker = useMemo\(\(\) => \{\s*const owner = conversations\.find\(\(item\) => item\.id === conversationId\)\?\.user_email;/
    );
    expect(HOME_PAGE).toContain("return typeof owner === 'string' && owner.trim() ? owner : identity.signedInAs;");
    expect(HOME_PAGE).toContain('asker={asker}');
  });
  ...
  it('filters on the normalised key, so a selection survives a change of case', () => {
    expect(HOME_PAGE).toContain('rail.owners.map((owner) => owner.key)');
    expect(HOME_PAGE).toContain('ownerFilters.filter((key) => present.has(key))');
