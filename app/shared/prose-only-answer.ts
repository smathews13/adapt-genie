...
/*
 * ---- The analysis package is apparatus, and it was reaching the card ----
 *
 * `agent.py` already knows this. `reader_facing_findings` splits the analysis
 ...
 * back and serves it as the answer. On a run that ended during analysis, "whatever
 ...
 * clipped at an internal handoff bound" -- which reads as the answer, and reads as an
...
/*
 * One `- **Name:**` lead-in. The generated package is inconsistent about whether the colon
...
/**
 * Splits the orchestrator package into the prose a reader is shown and the caveats.
 *
...
export function readerFacingFindings(findings: string): ReaderFacingFindings {
  ...
  for (const body of bodiesOf(CAVEAT_SECTIONS)) {
    // One caveat per line: the orchestrator writes these as its own nested bullets,
...
export function proseOnlyAnswer(id: string, prose: string, recordedStages: readonly unknown[] = []): ProseOnlyAnswer {
   ...
   * is the one asymmetry here and it is deliberate. When a run ends in analysis
