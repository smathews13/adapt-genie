...
const APP = `${HOST}/api/2.0/apps/adapt`;
...
function agentReport(overrides: Record<string, unknown> = {}) {
  return {
    ...
    configuration: [
      ...
      { key: 'dictionary_genie_space_title', value: 'ADAPT Dictionary' },
    ],
    checks: [
      {
        id: 'sql-warehouse',
...
describe('POST /api/access-verification with a token', () => {
  ...
  it('probes the declared tables and Genie spaces from this release, not from a serving ping', async () => {
    ...
    process.env.PLAYER_INSIGHTS_DATA_GENIE_TITLE = 'ADAPT Data';
    process.env.PLAYER_INSIGHTS_DICTIONARY_GENIE_TITLE = 'ADAPT Dictionary';
    const calls = stubStatements(() => ({ ok: true }));
    ...
    expect(genieOn(body)[0].label).toMatch(/ADAPT/);
