 ...
 *
 * `--ast-radius-control` and `--ast-radius-card` are the ADAPT compatibility names for the
 * same two corners, and the token test holds each equal to its
 * `--radius-*` twin while both sets are live. So a rule written in the ADAPT
