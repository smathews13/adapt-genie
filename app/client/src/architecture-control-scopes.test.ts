...
describe('the runtime bounds illuminate only what the agent actually bounds', () => {
  it('maps reasoning steps to the orchestrator and foundation model', () => {
    expect(ARCHITECTURE_CONTROL_SCOPES.maxSteps.nodes).toEqual(['agent-endpoint', 'llm-endpoint']);
    expect(ARCHITECTURE_CONTROL_SCOPES.maxSteps.edges).toEqual(['agent-endpoint->llm-endpoint']);
    expect(nodeControlBounds('genie-data')).not.toContain('maxSteps');
  });

  it('maps the tool-call cap to Genie and the warehouse it uses', () => {
    expect(ARCHITECTURE_CONTROL_SCOPES.maxToolCalls.nodes).toEqual(['genie-data', 'sql-warehouse']);
    expect(ARCHITECTURE_CONTROL_SCOPES.maxToolCalls.edges).toEqual([
      'agent-endpoint->genie-data',
      'genie-data->sql-warehouse',
    ]);
    expect(nodeControlBounds('llm-endpoint')).not.toContain('maxToolCalls');
    expect(edgeControlBounds('agent-endpoint', 'llm-endpoint')).not.toContain('maxToolCalls');
  });
