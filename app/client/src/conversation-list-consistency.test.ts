...
const conversations: Conversation[] = Array.from({ length: 6 }, (_, index) => ({
  id: `conv-${index + 1}`,
  title: ['NBA spike', 'Compare active players', 'GTA 5'][index % 3],
  updated_at: `2026-08-20T17:4${index}:00Z`,
  user_email: 'sam.mathews@databricks.com',
}));
...
describe('Ask and Run Explorer use one conversation set', () => {
  ...
  it('keeps every stored id even when titles repeat and one run is complete', () => {
    const askIds = railOwnership(conversations, 'sam.mathews@databricks.com').entries.map(
  ...
  it('adds the owner chips up to the All chip, which is the Explorer’s total', () => {
    ...
    const shared: Conversation[] = [
      ...conversations,
      { id: 'conv-7', title: 'GTA 5', updated_at: '2026-08-20T17:50:00Z', user_email: 'colleague@databricks.com' },
    ];
    const rail = railOwnership(shared, 'sam.mathews@databricks.com');
