...
async function main() {
  ...
  if (!adminEmails) {
    ...
    console.log(
      '\n  note  the generated build/deploy/app.yaml now carries administrator addresses.\n' +
        ...
        '        afterwards with:  git restore app/build/deploy/app.yaml'
    );
  }
  ...
  // Postgres schema the app owns. Authored as adapt in app.yaml; the
  ...
  await writeDeployAppYaml({
    ...
    env: [
      ...
      ...(catalog ? [{ name: 'PLAYER_INSIGHTS_APP_CATALOG', value: `'${catalog}'` }] : []),
