...
function statusFor(reading: ConnectionReading): string {
  const failed = reading.status === 'blocked' || reading.status === 'refused' || reading.status === 'unreachable';
  if (!failed && (reading.status === 'reachable' || Boolean(clean(reading.row.configured)))) {
    return PRIMARY_CONNECTION_LABEL.connected;
  }
