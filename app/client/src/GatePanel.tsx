 ...
 *     sentence.
 *  3. A reader who WAS an administrator a moment ago is moved to ADAPT Ask and
...
/**
 * The sentence carried to ADAPT Ask by the move above, or ''.
 *
 ...
 *
 * In the layout rather than in ADAPT Ask so that it is drawn wherever the move
 * lands, and so that ADAPT Ask -- which is the largest file in this client -- does
...
export function RoleLostNotice() {
  ...
  if (!said) return null;
  return (
    <div className="role-lost-notice" role="alert">
      <p>{said}</p>
    </div>
  );
}
