        ...
        <p className="live-step-head">
          ...
          {step.status !== 'complete' && (
            <Badge variant="outline" className={astPill(step.status)}>
              {step.status === 'running' ? <AdaptBusyMark className="live-step-busy-mark" /> : null}
              {step.status}
            </Badge>
          )}
        </p>
...
export function LiveProgress({
   ...
   * `buildLiveRun` for why. They are still declared because ADAPT Ask holds the
  ...
  /*
   * Follow only this list's scrollTop. `scrollIntoView` is intentionally not
   * used: it can move every scrollable ancestor and pull the whole transcript
   * away from what the reader was viewing. A reader who scrolls this list up
   * owns it until they return to its bottom.
   *
   * The follow is released only on an UPWARD scroll by the reader, not whenever
   * the container happens to sit short of the bottom. A programmatic scroll to a
   * freshly appended row lands short of the bottom for a frame while the row
   * lays out, and a naive gap check read that as "the reader is elsewhere" and
   * stopped following — which is why a run appeared to freeze a few steps in.
   */
  useFollowEffect(() => {
