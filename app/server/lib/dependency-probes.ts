 ...
 * that do not map directly to app declarations, and the Apps update API rejects
...
export interface ProbeSubject {
   ...
   * ANSWERS IS NOT NECESSARILY CURRENT, so reachability alone must not be
   * presented as freshness.
   *
 ...
 *
 * Databricks answers a scope-limited call with a named required scope, which is
 * the only place in the whole exchange the distinction is visible, so it is read rather than
 ...
 * better one where it speaks. Unity Catalog refusals on the
...
export async function probeConnections(input: {
  ...
  if (subjects.length === 0) return [];
  const settle = (checks: PreflightCheck[]) => withManifestRollup(checks);
  if (!input.host) {
    return settle(
      unaskedChecks(
        subjects,
  ...
  return settle(await runProbes(subjects, options));
}
