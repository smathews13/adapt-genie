...
describe('IdentityCard', () => {
  ...
  it('uses the official Databricks organization mark for Databricks email domains', () => {
    const identity: PanelIdentity = {
      ...IDENTITY,
      signedInAs: 'sam.mathews@databricks.com',
      organizations: [],
    };
  ...
  it('compacts only the Identity-card role badge without clipping Super admin', () => {
    ...
    expect(textOf({ identity, failed: false })).toContain('ADAPT role Super admin');
    ...
    const read: DeploymentIdentity = {
      identity: {
        ...
        identityMetadata: {
          ...
          app: { displayName: 'ADAPT', resourceName: '', workspaceHost: '', workspaceId: '' },
          servicePrincipal: {
  ...
  it('renders a future resource kind by its concrete binding name', () => {
    const identity: PanelIdentity = {
      ...
      identityMetadata: {
        ...
        servicePrincipal: {
          ...
          attachedResources: [
            {
              resourceKey: 'example-feature-store',
              resourceType: 'feature_store',
              displayIdentifier: 'example-feature-store',
              permission: 'CAN_READ',
            },
          ],
        },
    ...
    expect(markup).toContain('Feature store · example-feature-store');
    expect(markup).toContain(
      'aria-label="Feature store attached resource · example-feature-store · binding example-feature-store · permission CAN_READ"'
    );
...
describe('execution responsibility', () => {
  it('names app execution while keeping the live application identity separate', () => {
    ...
    const markup = renderToStaticMarkup(<IdentityCard read={read} />);
    expect(text).toContain('Execution ADAPT app');
