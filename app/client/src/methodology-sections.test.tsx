...
describe('shared Cost and Forecasting methodology structure', () => {
  it('orders populated groups and omits empty groups', () => {
      ...
      <MethodologySections
        groups={[
          ...
          { title: 'Genie usage', rows: [{ label: 'Allowance', detail: '150 DBU' }] },
          {
            ...
            rows: [{ label: 'Optional add-on', detail: 'Measured endpoint share' }],
          },
        ]}
      />
    );
    ...
    expect(markup).toContain('Optional add-on');
  });
  ...
  it('is the one structure rendered by both Cost and Forecasting disclosures', () => {
    ...
    expect(cost).not.toContain('Exact optional endpoint billing');
