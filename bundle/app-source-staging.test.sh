...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/adapt-app-staging.XXXXXX")"
...
reject_path() {
  ...
  if validate_app_staging_path "$path" adapt-genie sam@example.com >/dev/null 2>&1; then
    fail "$label"
  else
    pass "$label"
  fi
...
check "exact ADAPT staging path is accepted" validate_app_staging_path \
  "/Workspace/Users/sam@example.com/adapt-genie-src" \
  "adapt-genie" "sam@example.com"
...
reject_path "wrong actor is refused" "/Workspace/Users/other@example.com/adapt-genie-src"
reject_path "wrong app is refused" "/Workspace/Users/sam@example.com/other-app-src"
reject_path "nested path is refused" "/Workspace/Users/sam@example.com/releases/adapt-genie-src"
reject_path "relative traversal is refused" "/Workspace/Users/sam@example.com/../adapt-genie-src"
reject_path "retired real-src variant is refused" \
  "/Workspace/Users/sam@example.com/adapt-genie-real-src"
reject_path "retired app-source variant is refused" \
  "/Workspace/Users/sam@example.com/adapt-genie-app-source"
...
case "$group ${1:-}" in
  ...
  "workspace get-status")
    if [[ -d "$MOCK_REMOTE" ]]; then
      ...
      if [[ "${MOCK_MISSING_PATH_STYLE:-code}" == "message" ]]; then
...
export MOCK_SOURCE_PATH="/Workspace/Users/sam@example.com/adapt-genie-src"
...
if clean_and_import_app_source "$SOURCE" "$MOCK_SOURCE_PATH" adapt-genie "cmeg demo" \
  >/dev/null 2>&1; then
  databricks apps deploy adapt-genie --source-code-path "$MOCK_SOURCE_PATH" \
    --mode SNAPSHOT --profile "cmeg demo" >/dev/null
else
...
check "profile containing a space stays one argument" \
  grep -q -- '|--profile|cmeg demo$' "$MOCK_LOG"

rm -rf "$MOCK_REMOTE"
export MOCK_MISSING_PATH_STYLE=message
check "CLI path-not-found message is treated as an empty staging location" \
  clean_and_import_app_source "$SOURCE" "$MOCK_SOURCE_PATH" adapt-genie "cmeg demo"
unset MOCK_MISSING_PATH_STYLE
...
if clean_and_import_app_source "$SOURCE" "$MOCK_SOURCE_PATH" adapt-genie "cmeg demo" \
  >/dev/null 2>&1; then
  databricks apps deploy adapt-genie --profile "cmeg demo" >/dev/null
fi
...
check "release sources the guarded staging helper" \
  grep -q 'source .*app-source-staging.sh' "$APP_RELEASE"
check "release explicitly requests SNAPSHOT mode" \
  grep -q 'apps deploy "\$APP_NAME".*--mode SNAPSHOT' "$APP_RELEASE"
check "release replaces the validated staging directory" \
  grep -q 'clean_and_import_app_source "\$DEPLOY_TREE" "\$SRC_PATH" "\$APP_NAME" "\$PROFILE"' "$APP_RELEASE"
check "release computes the source manifest" \
  grep -q 'app_source_manifest_summary "\$DEPLOY_TREE"' "$APP_RELEASE"
check "staging helper cannot stop or delete the active app" \
  sh -c "! grep -Eq 'databricks apps (stop|delete)' '$HERE/app-source-staging.sh'"
