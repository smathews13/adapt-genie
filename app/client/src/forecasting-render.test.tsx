...
describe('Forecasting visibility and placement', () => {
  it('keeps Forecasting visible by default with Cost', () => {
    const markup = renderOps({ ...NO_EXPERIMENTS });
    expect(markup).toContain('ops-cost-heading');
    expect(markup).toContain('data-testid="ops-forecasting"');
  ...
  it('renders editable assumptions and all deterministic horizons', () => {
    ...
    expect(markup).not.toContain('Legacy add-on cost per table per day');
  ...
  it('includes attributable Foundation Model and Lakebase spend in reconciled forecast subtotals', () => {
    ...
    payload.tiles.push(
      ...
      },
      {
        id: 'lakebase',
        label: 'Lakebase',
        resourceId: 'projects/adapt/branches/production',
        quality: 'real',
        amount: 7,
        dbus: 3.5,
        basis: 'total-in-range',
    ...
    const result = calculateForecast(baseline, baseline.defaults);
    const week = result.horizons.find((horizon) => horizon.days === 7)!;
    expect(week.components.map((component) => component.id)).toEqual(
      expect.arrayContaining(['foundation-model', 'lakebase'])
    );
    expect(week.total).toBe(week.components.reduce((sum, component) => sum + (component.amount ?? 0), 0));
  });

  it('keeps configured but unpriced Foundation Model and Lakebase rows visible without treating them as zero', () => {
    const payload = cost();
    payload.tiles.push(
      {
        id: 'foundation-model',
        ...
        population: 'Interactive Ask tokens',
        attribution: 'unavailable',
        unavailable: 'Model billing is unavailable.',
        remedy: '',
        note: '',
      },
      {
        ...
        population: 'Tagged ADAPT project',
        attribution: 'unavailable',
        unavailable: 'Lakebase billing is unavailable.',
        remedy: '',
        note: '',
      }
    );
    ...
    expect(result.components).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: 'foundation-model', dailyAmount: null }),
        expect.objectContaining({ id: 'lakebase', dailyAmount: null }),
      ])
    );
    expect(result.horizons[0].components).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: 'foundation-model', amount: null }),
        expect.objectContaining({ id: 'lakebase', amount: null }),
      ])
    );
  });
  ...
  it('renders loading, unavailable, and partial states without inventing totals', () => {
    ...
    expect(loading).toContain('adapt-loader--panel');
